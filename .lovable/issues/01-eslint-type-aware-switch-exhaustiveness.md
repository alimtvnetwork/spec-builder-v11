...
In `eslint.config.js`, the rule `@typescript-eslint/switch-exhaustiveness-check` was added without enabling typed linting (`parserOptions.project` or `parserOptions.projectService: true`). Furthermore, the files glob `**/*.{ts,tsx}` captures `.ts` files inside `spec/` (e.g. `spec/11-spec-management-software/16-api/types.ts`) which are not included in `tsconfig.app.json` or `tsconfig.node.json`.
