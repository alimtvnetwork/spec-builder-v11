...
The Link Manager WordPress plugin architecture (`spec/30-wp-plugin/link-manager/`) is defined as a specification-only project with **30 spec files**, **29 database tables**, and **34 PHP entity classes**. It utilizes a three-tier storage system: main DB (29 tables), per-content history DBs, and system snapshots. The plugin provides **110+ REST API endpoints** under the `lm/v1` namespace across 13 core services.
