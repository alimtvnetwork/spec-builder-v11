...
- Replaced ~345 `ctx context.Context` → `context stdctx.Context` across 9 files in `spec/11-spec-management-software/05-features/25-ai-enhancements/`:
...
- Replaced ~70 raw `os.*` calls with `pathutil` wrappers across 7 files:
  - **spec/14-wp-plugin-builder**: `06-project-management` (~5 fixes), `08-spec-processing` (~3 fixes), `07-code-generation` (~5 fixes), `09-preset-learning` (~3 fixes), `13-testing-strategy` (~1 fix), `05-rag-system` (~1 fix)
  - **spec/05-seedable-config-architecture**: `00-overview` (~3 fixes)
...
- Replaced ~7 raw `os.*` calls with `pathutil` wrappers across 2 files:
  - **spec/12-nexus-flow-cli**: `01-core-specification` (~4 fixes — parseInput, writeOutput), `02-standalone-architecture` (~3 fixes — CodeExecStage, embedded DB extraction)
- `spec/24-nexus-flow-cli/` and `spec/06-split-db-architecture/` now have **zero raw `os.*` violations**
...
- Replaced ~294 `ctx context.Context` → `context stdctx.Context` across 9 files:
  - **spec/22-ai-bridge-cli/01-backend/51-vector-database-integration** (~43 fixes — VectorStore interface, EmbeddingProvider interface, OllamaEmbedder, EmbedBatch)
  - **spec/22-ai-bridge-cli/01-backend/40-gsearch-context-integration** (~36 fixes — GSearchExecutor interface, ContextFetcher, ContextIntegrationService, HtmlBlogSearchExecutor)
  - **spec/22-ai-bridge-cli/01-backend/09-agentic-mode** (~20 fixes — Executor, PromptAnalyzer, AgenticExecutor, executeSearch)
  - **spec/22-ai-bridge-cli/01-backend/56-observability** (~15 fixes — OllamaHealthChecker, LlamaCppHealthChecker, RAGStoreHealthChecker, InitTracer)
  - **spec/22-ai-bridge-cli/01-backend/16-ai-seo-error-codes** (~5 fixes — RetryWithBackoff)
  - **spec/22-ai-bridge-cli/01-backend/31-revision-feedback-system** (~5 fixes — BlogHandler.GenerateWithFeedback)
  - **spec/30-wp-plugin/wp-plugin-publish/01-backend/05-plugin-service** (~90 fixes — full Service interface + impl + scanner + hasher + watcher + validator)
  - **spec/30-wp-plugin/wp-plugin-publish/03-implementation/30-plugin-service-impl** (~80 fixes — full Service interface + crud + scanner + mappings)
  - **spec/30-wp-plugin/wp-plugin-publish/02-frontend/26-ui-patterns** (~1 fix — Service.Create)
...
- Replaced ~120 `ctx context.Context` → `context stdctx.Context` across 3 files:
  - **spec/30-wp-plugin/wp-plugin-publish/03-implementation/31-sync-service-impl** (~45 fixes — Service interface, CheckSync, CheckAllSites, CheckAllPlugins, GetFileChanges, RecordFileChange, MarkSynced, ClearChanges)
  - **spec/30-wp-plugin/wp-plugin-publish/03-implementation/33-watcher-service-impl** (~35 fixes — Service interface, TriggerScan, ScanAfterGitPull, ScanAll, InitializeCache, performScan)
  - **spec/30-wp-plugin/wp-plugin-publish/03-implementation/34-git-service-impl** (~40 fixes — Service interface, Pull, PullAll, GetStatus, Build, PullAndBuild, PullAndBuildAll, GetConfig, UpdateConfig, runGitCommand)
...
- Replaced ~60 `ctx context.Context` → `context stdctx.Context` across 5 files:
  - **spec/22-ai-bridge-cli/01-backend/01-architecture** (~10 fixes — BackendAdapter interface, SelectBackend)
  - **spec/22-ai-bridge-cli/01-backend/50-long-chain-command-system** (~5 fixes — ParallelExecutor, ParallelFetcher interfaces)
  - **spec/22-ai-bridge-cli/01-backend/52-research-mode** (~16 fixes — ResearchService, ResearchGatherer interfaces)
  - **spec/22-ai-bridge-cli/01-backend/32-tool-delegation** (~5 fixes — DelegationHealthChecker.Start, checkOne)
  - **spec/22-ai-bridge-cli/01-backend/03-startup-modes** (~5 fixes — runCmd, executeStreaming, daemon shutdown)
- All 5 files now have **zero `ctx context.Context` violations**

### Wave 37 (2026-03-12) — Context Naming in spec/11-spec-management-software
...
### Wave 38 (2026-03-12) — Context Naming in spec/30-wp-plugin (remaining files)
...
- **spec/30-wp-plugin module is now fully cleared** of `ctx context.Context` violations

### Wave 39 (2026-03-12) — Context Naming in spec/24-nexus-flow-cli
...
- All 3 files now have **zero `ctx context.Context` violations**
- **spec/24-nexus-flow-cli module is now fully cleared** of `ctx context.Context` violations

### Wave 40 (2026-03-12) — Context Naming in spec/26-ai-transcribe-cli
...
- All 5 files now have **zero `ctx context.Context` violations**
- **spec/26-ai-transcribe-cli module is now fully cleared** of `ctx context.Context` violations
...
### Wave 41 (2026-03-12) — Context Naming in spec/11-spec-management-software (remaining files)
...
- **spec/11-spec-management-software module is now fully cleared** of `ctx context.Context` violations

### Wave 42 (2026-03-12) — Context Naming in spec/09-gsearch-cli, spec/17-ai-research, spec/18-error-resolution, spec/21-brun-cli
- Remediated **~120 `ctx context.Context`** → `context stdctx.Context` across **13 files**:
  - **spec/20-gsearch-cli/01-backend/20-trend-analyzer-implementation** (~1 fix — GitHubCollector.Collect)
  - **spec/20-gsearch-cli/01-backend/64-stealth-scraping** (~3 fixes — DialTLSContext closure, StealthScraper.Scrape)
  - **spec/20-gsearch-cli/01-backend/44-serp-position-tracking** (~4 fixes — enrichWithAuthority, enrichWithContact)
  - **spec/20-gsearch-cli/01-backend/43-faq-discovery-ai-overview** (~1 fix — extractPaaFromSerp)
  - **spec/20-gsearch-cli/05-ai-bridge-integration** (~2 fixes — executeGSearch, CommandContext)
  - **spec/20-gsearch-cli/01-backend/42-multi-engine-search** (~3 fixes — searchWithRetry, doSearch, context.Done/Err)
  - **spec/20-gsearch-cli/01-backend/15-error-codes** (~3 fixes — executeWithRetry, context.Done/Err)
  - **spec/20-gsearch-cli/01-backend/52-bi-implementation-guide** (~1 fix — SearchEngine.Search interface)
  - **spec/60-ai-research/02-complete-ai-database-and-framework-ecosystem-guide** (~1 fix — myEmbeddingFunc + import alias)
  - **spec/60-ai-research/05-rag-memory-training-and-go-implementation-guide** (~4 fixes — ProcessQuery, LoadMemoryVariables)
  - **spec/04-error-resolution/06-error-handling/go-delegation-fix** (~4 fixes — fetchFromDelegatedServer, executeDelegatedRequest)
  - **spec/04-error-resolution/06-error-handling/readme** (~6 fixes — doRequest, context.WithValue, BuildErrorResponse, injectDelegatedServer)
  - **spec/04-error-resolution/10-apperror-package/readme** (~12 fixes — GetById, List, ListActive, CheckAll, GetCached, adapter patterns)
  - **spec/21-brun-cli/01-backend/01-core-architecture** (~1 fix — Executor.Execute interface)
  - **spec/22-ai-bridge-cli/01-backend/58-rubric-validation/12-self-validation-engine** (~6 fixes — ValidateAndReturn, Generate, judge, SaveAttempt)
- Also updated **spec/21-brun-cli/99-consistency-report** BR-I10 status to ✅ Fixed
- **spec/09-gsearch-cli**, **spec/17-ai-research**, **spec/18-error-resolution** modules now fully cleared
...
- Remediated **~95 `(*T, error)` tuple return violations** across **4 files** in `spec/11-spec-management-software/05-features/06-ai-integration/`
- **`06-ai-integration/` is now fully cleared** of `(*T, error)` tuple return violations (all 6 files)

### Wave 49 (2026-03-13) — Cat 3 Tuple Return Remediation (spec/22-ai-bridge-cli)
...
- **`spec/22-ai-bridge-cli/` is now fully cleared** of actionable `(*T, error)` tuple violations (only EXEMPTED stdlib patterns remain: MarshalJSON, DialContext)

## Remaining Hotspots (by category)

### Cat 1 — `ctx context.Context` (163 remaining in 9 files)
- **Non-actionable**: `spec/02-coding-guidelines/03-golang/`, `spec/02-coding-guidelines/01-cross-language/`, `spec/61-how-app-issues-track/` (anti-pattern examples and issue documentation)
- **EXEMPTED**: `spec/22-ai-bridge-cli/01-backend/29-gsearch-url-extraction` (2 stdlib interface violations marked EXEMPTED)
- **Cleared (entire modules)**: `spec/11-spec-management-software/`, `spec/04-error-resolution/`, `spec/20-gsearch-cli/`, `spec/21-brun-cli/`, `spec/22-ai-bridge-cli/` (all but 2 exempted), `spec/24-nexus-flow-cli/`, `spec/30-wp-plugin/`, `spec/31-wp-plugin-builder/`, `spec/25-spec-reverse-cli/`, `spec/26-ai-transcribe-cli/`, `spec/60-ai-research/`

### Cat 2 — `fmt.Errorf` — Verified Scan (2026-03-13)

**spec/11-spec-management-software/ density breakdown (783 matches in 36 files):**
...
|--------|---------|-------|--------|
| `spec/26-ai-transcribe-cli/` | **230** | **8** | 🔴 Highest density — enum Parse + STT providers |
| `spec/21-brun-cli/` | **123** | **6** | 🔴 High — enum Parse + integration API retry loops |
| `spec/06-split-db-architecture/` | **85** | **3** | 🔴 High — DB init, export/import |
| `spec/02-coding-guidelines/03-golang/` | **77** | **7** | 🟡 Non-actionable (anti-pattern examples, template code) |
| `spec/22-ai-bridge-cli/` | **73** | **10** | 🟡 Mostly exempted (stdlib interfaces, changelog refs) |
| `spec/31-wp-plugin-builder/` | **70** | **1** | 🔴 High — 14 enum Parse functions in `15-enum-architecture.md` |
| `spec/32-wp-seo-publish-cli/` | **65** | **2** | 🔴 High — 11 enum Parse + variable system |
| `spec/30-wp-plugin/` | **55** | **4** | 🔴 High — split-db, publish service, seedable config |
| `spec/02-coding-guidelines/01-cross-language/` | **53** | **4** | 🟡 Non-actionable (❌ anti-pattern examples in guidelines) |
| `spec/25-spec-reverse-cli/` | **50** | **1** | 🔴 High — 10 enum Parse functions in `12-enum-architecture.md` |
| `spec/07-seedable-config-architecture/` | **38** | **2** | 🟡 Medium — validators + seed loading |
| `spec/28-shared-cli-frontend/` | **35** | **2** | 🟡 Medium — settings service type getters |
| `spec/61-how-app-issues-track/` | **22** | **1** | 🟡 Non-actionable (audit documentation) |
| `spec/04-error-resolution/` | **15** | **2** | 🟡 Non-actionable (anti-pattern examples + UnmarshalJSON exemption) |
| `spec/27-license-manager/` | **5** | **1** | ✅ Zero violations (consistency report text only) |
| `spec/01-general-spec/` | **0** | 0 | ✅ Clear |
| `spec/50-powershell-integration/` | **0** | 0 | ✅ Clear |
| `spec/03-error-code-registry/` | **0** | 0 | ✅ Clear |
| `spec/60-ai-research/` | **0** | 0 | ✅ Clear |
| `spec/53-e2-activity-feed/` | **0** | 0 | ✅ Clear |
| `spec/08-generic-enforce/` | **0** | 0 | ✅ Clear |

**Actionable remediation targets (non-spec/02), by priority:**
1. `spec/26-ai-transcribe-cli/` — 230 matches (enum Parse + provider code)
2. `spec/21-brun-cli/` — 123 matches (enum Parse + retry loops)
3. `spec/06-split-db-architecture/` — 85 matches (DB operations)
4. `spec/31-wp-plugin-builder/` — 70 matches (enum Parse only)
5. `spec/32-wp-seo-publish-cli/` — 65 matches (enum Parse + variable system)
6. `spec/30-wp-plugin/` — 55 matches (split-db, publish, config)
7. `spec/25-spec-reverse-cli/` — 50 matches (enum Parse only)
8. `spec/07-seedable-config-architecture/` — 38 matches (validators)
9. `spec/28-shared-cli-frontend/` — 35 matches (settings service)

### Cat 3 — `(*T, error)` tuples (~8 remaining matches in ~1 directory — `spec/01-general-spec/`)
- **Cleared modules**: `spec/11-spec-management-software/`, `spec/04-error-resolution/`, `spec/28-shared-cli-frontend/`, `spec/06-split-db-architecture/`, `spec/07-seedable-config-architecture/`, `spec/20-gsearch-cli/`, `spec/21-brun-cli/`, `spec/22-ai-bridge-cli/`, `spec/24-nexus-flow-cli/`, `spec/30-wp-plugin/`, `spec/31-wp-plugin-builder/`, `spec/25-spec-reverse-cli/`, `spec/26-ai-transcribe-cli/`, `spec/60-ai-research/`, `spec/32-wp-seo-publish-cli/`, `spec/61-how-app-issues-track/`, `spec/02-coding-guidelines/01-cross-language/`, `spec/02-coding-guidelines/03-golang/`
- **Triaged as non-actionable**: `spec/02-coding-guidelines/03-golang/`, `spec/02-coding-guidelines/01-cross-language/`, `spec/61-how-app-issues-track/` — all remaining are ❌ anti-pattern examples, issue documentation tables, or prose references
- **Wave 78** (2026-03-14): Remediated 5 actionable violations in `spec/04-error-resolution/06-error-handling/` (delegation fix: `fetchFromDelegatedServer`, `buildDelegatedEnvelope`, `executeDelegatedRequest` triple→outcome struct, `buildDelegatedErrorEnvelope`) and `spec/04-error-resolution/06-error-handling/readme.md` (`doRequest`). Annotated 1 exemption in `spec/04-error-resolution/03-debugging-guides/02-debugging-go.md` (DBOperation callback). Triaged `spec/61-how-app-issues-track/` (5 matches — all ❌ anti-pattern examples or issue tables, non-actionable). Triaged JS/TS matches (`console.error`, `logger.error`) as non-actionable.
- **Wave 81** (2026-03-14): Triaged `spec/01-general-spec/` — 1 actionable violation remediated (`GetTyped[T]` tuple → `apperror.Result[T]` in `01-coding-standards-foundation.md`), ~16 JS/TS `console.error`/`this.failBackupJob` matches triaged as non-actionable (not Go tuple returns). **Category 3 is now 100% COMPLETE across the entire project.**

### Cat 4 — Raw `os.*` (259 remaining in 19 files)
- **Highest density**: `spec/02-coding-guidelines/03-golang/` + `spec/02-coding-guidelines/01-cross-language/` + `spec/33-wp-plugin-development/` (~200 — mostly ❌ anti-pattern examples, not actionable)
- **Remaining actionable**: `spec/28-shared-cli-frontend/` (~1)
- **Cleared**: `spec/06-split-db-architecture/`, `spec/07-seedable-config-architecture/`, `spec/21-brun-cli/`, `spec/24-nexus-flow-cli/`, `spec/30-wp-plugin/`, `spec/11-spec-management-software/`, `spec/31-wp-plugin-builder/`

### Cat 5 — `os.IsNotExist` (78 remaining in 3 files)
- **All remaining**: `spec/02-coding-guidelines/03-golang/` + `spec/61-how-app-issues-track/` (guideline ❌ anti-pattern examples and issue documentation — **not actionable**)
- **Cleared**: All application-level modules

### Cat 8 — `interface{}` + `map[string]any` (994 + 625 = ~1,619 remaining in ~88 files)
- **Wave 79** (2026-03-14): Remediated **8 Cat 8 violations** across 3 files in `spec/11-spec-management-software/05-features/`: `22-golang-search-cli/19-authority-credibility-scoring` (2 — `map[string]interface{}` params → typed `ConfidenceThresholds` struct), `24-code-generation-system/05-parallel-executor` (1 — `map[string]interface{}` template context → typed `PromptTemplateContext` struct), `24-code-generation-system/22-loop-validation` (5 — `map[string]interface{}` event payloads → typed event structs: `SpecLoopCompletedEvent`, `BuildLoopStartedEvent`, `BuildLoopIterationEvent`, `BuildLoopCompletedEvent`, `ParallelBuildCompletedEvent`).
- **Wave 80** (2026-03-14): Remediated **20 Cat 8 violations** and annotated **1 exemption** across 4 files in `spec/11-spec-management-software/14-microservices/`: `02-specmanager` (13 — `map[string]any` error contexts → typed `PathErrorContext`, `PathPatternErrorContext`, `PathWithRootsErrorContext`, `EnumErrorContext` structs), `06-nexus-flow` (2 — `interface{}` goroutine param + `map[string]interface{}` iteration data → typed `IterationItem` struct), `09-nexus-flow-standalone-architecture` (2 — `map[string]interface{}` Config → typed `StageConfigParams` struct + 1 EXEMPTED Wails `Bind` framework boundary), `12-ai-bridge-cli` (3 — `map[string]interface{}` AppError.Details → typed `AppErrorDetails` struct + `WithDetails` signature updated).
...
- **Wave 82** (2026-03-14): Triaged `spec/02-coding-guidelines/01-cross-language/` — **23 matches across 5 files, ALL non-actionable**: `00-master-coding-guidelines` (7 — ❌ anti-pattern examples with matching ✅ corrections, prose rule statements, checklist items), `01-issues-and-fixes-log` (4 — issue documentation describing the violation pattern), `03-casting-elimination-patterns` (9 — ❌/✅ pattern pairs + `typecast.CastOrFail`/`CastSliceOrFail` utility definitions which accept `[]interface{}` at parse boundaries by design), `05-cross-spec-contradiction-checks` (1 — contradiction table entry), `13-strict-typing` (2 — prohibition rule prose). **`spec/02-coding-guidelines/01-cross-language/` fully cleared — zero actionable violations.**
- **Wave 83** (2026-03-14): Triaged `spec/22-ai-bridge-cli/` (6 matches — all compliance-annotation comments, zero actionable) and `spec/30-wp-plugin/` (0 matches). **Both modules fully cleared.**
- **Wave 84** (2026-03-14): Remediated **~15 Cat 8 violations** and annotated exemptions across 3 sub-modules in `spec/11-spec-management-software/`:
  ...
  - **All three sub-modules now fully cleared.** `spec/11-spec-management-software/` is 100% cleared of actionable Cat 8 violations.
- **Wave 85** (2026-03-14): Triaged and remediated `spec/20-gsearch-cli/` (4 files, ~20 matches): 1 actionable remediated — `63-captcha-handling.md` (8 matches — `map[string]interface{}` CapSolver API payload → typed `CapSolverTask` + `CapSolverCreateRequest` structs). 3 non-actionable files: `21-settings-service.md` (4 — rule prose "no interface{}"), `45-contact-extraction.md` (1 — compliance annotation comment), `43-faq-discovery-ai-overview.md` (7 — already EXEMPTED external JSON-LD parse boundary). **`spec/20-gsearch-cli/` fully cleared.**
- **Wave 86** (2026-03-14): Triaged 3 non-application modules — **ALL non-actionable, zero remediation needed**:
  - **`spec/08-generic-enforce/`** (~63 matches across 5 files): ALL are rule-prose definitions, ❌/✅ anti-pattern examples, type erasure hierarchy documentation, and audit tables documenting violations in external codebases. This IS the specification that defines the Cat 8 rules — every match is intentional pedagogical content.
  - **`spec/60-ai-research/`** (~35 matches across 5 files): ALL are ALLOWED/EXEMPTED external API boundaries (OpenAI raw JSON payloads, Qdrant `NewValueMap` API, LangChain Go `chain.Call` API requiring `map[string]any`/`map[string]interface{}`). Research docs demonstrating third-party library integration patterns — not application code.
  - **`spec/02-coding-guidelines/03-golang/`** (~12 matches across 2 files): ALL are rule-prose (❌ anti-pattern examples with ✅ corrections, exception documentation for logger variadic params and third-party library interfaces, common mistakes section). This IS the Go standards specification — every match is intentional.
  - **All three modules fully cleared.**
- **Wave 87** (2026-03-14): Triaged and remediated 3 modules:
  - **`spec/26-ai-transcribe-cli/`** (7 files, ~9 matches): 4 actionable remediated — `01-architecture.md` (1 — `Event.Data map[string]interface{}` → typed `EventData` struct with Text/IsFinal/Confidence/Provider/Reason/ErrorCode fields), `06-voice-commands.md` (3 — `CustomCommand.Parameters map[string]any` → typed `CommandParameters` struct, `ExecutionResult.Data map[string]any` → typed `ExecutionResultData` struct, `map[string]any` webhook payload → typed `WebhookPayload` struct). 1 annotated exemption: `07-voice-cloning.md` (XTTS external API payload). 4 already exempt/non-actionable: `03-stt-providers.md` (2 — already EXEMPTED OpenAI/Deepgram WebSocket APIs), `08-database-schema.md` (1 — already EXEMPTED sync.Map), `15-settings-service.md` (1 — ASCII diagram text, fixed to `SettingValue`). **Fully cleared.**
  - **`spec/21-brun-cli/`** (1 file, 4 matches): ALL non-actionable — rule prose documenting "No `interface{}` or `any` usage" policy and typed accessor patterns. **Fully cleared.**
  - **`spec/28-shared-cli-frontend/`** (2 files, 6 matches): 3 actionable remediated — `09-deploy-folder.md` (`CrashReport.Context map[string]interface{}` → typed `CrashContext` struct with Path/Method/Component/SessionId fields, `ReportCrash` signature updated, inline `map[string]interface{}` literal → `CrashContext{}` constructor). 3 non-actionable: `03-settings-service.md` (rule prose). **Fully cleared.**
- **Wave 88** (2026-03-14): **FINAL WAVE — Category 8 closed out.** Remediated **~12 actionable violations** and triaged **~29 residual matches** across all remaining un-triaged modules:
  - **`spec/04-error-resolution/`** (5 actionable): `02-debugging-go.md` (3 — `Response.Data interface{}` → generic `Response[T any]`, `respondSuccess` → generic `[T any]`, `map[string]interface{}` health response → typed `HealthStatus` struct). `06-error-handling/readme.md` (2 — `DelegatedRequestServer.RequestBody/Response interface{}` → `json.RawMessage`).
  - **`spec/11-spec-management-software/`** (7 actionable, ~20 triaged): `18-realtime/02-sse-streaming.md` (1 — `SSEEvent.Data interface{}` → generic `SSEEvent[T any]`). `24-code-generation-system/26-questioning-system.md` (2 — `DefaultValue/Answer interface{}` → typed `AnswerValue` union struct). `22-golang-search-cli/19-authority-credibility-scoring.md` (2 — `.([]interface{})` assertions → `typecast.CastSliceOrFail[string]`). `04-coding-guidelines/05-seedable-config-pattern.md` (2 — GORM `map[string]interface{}` Update → typed `SettingUpdate` struct, SeedFile.Values annotated EXEMPTED at JSON parse boundary). `06-error-management/00-overview.md` (2 — AppError.Details annotated EXEMPTED at error infrastructure boundary). `22-settings-service-implementation.md` (8 — annotated with Cat 8 Triage Note header, EXEMPTED as settings framework boundary, superseded by SettingValue pattern). `10-research/01a-e2e-integration-tests.md` (2 — annotated EXEMPTED wiremock test framework). All others already exempt (jwt callback, gorm.Expr, Migratable marker interface).
  - **`spec/01-general-spec/`** (9 matches): ALL non-actionable — rule prose, ❌/✅ pattern tables, ALLOWED LangChain example, checklist items.
  - **`spec/07-seedable-config-architecture/`** (3 matches): ALL non-actionable — rule prose ("No interface{}", typed struct comments).
  - **`spec/04-error-resolution/`** (remaining): changelog prose, delegation fix description — non-actionable.
  - **🏁 CATEGORY 8 IS 100% COMPLETE. Zero actionable violations remain across the entire specification tree.**
- **Cleared modules (Cat 8)**: ALL modules in `spec/` — `spec/01-general-spec/`, `spec/11-spec-management-software/` (all sub-modules), `spec/04-error-resolution/`, `spec/28-shared-cli-frontend/`, `spec/07-seedable-config-architecture/`, `spec/20-gsearch-cli/`, `spec/21-brun-cli/`, `spec/22-ai-bridge-cli/`, `spec/30-wp-plugin/`, `spec/26-ai-transcribe-cli/`, `spec/60-ai-research/`, `spec/61-how-app-issues-track/`, `spec/02-coding-guidelines/01-cross-language/`, `spec/02-coding-guidelines/03-golang/`, `spec/08-generic-enforce/`
...
Previously identified duplicate numeric prefixes for `23-` (GSearch) and `16-` (root level) have been resolved by renaming them to `61-movie-search.md` and `spec/27-license-manager/` respectively.
