...
| Module | Scope | Status |
|--------|-------|--------|
| AI Bridge CLI (`spec/22-ai-bridge-cli/`) | ~75 YAML/JSON tag violations | ✅ |
| Microservices (`spec/11-spec-management-software/14-microservices/`) | ~200 struct tags (largest cluster) | ✅ |
| GSearch CLI | JSON key + struct tag alignment | ✅ |
| Exam Manager (`spec/30-wp-plugin/`) | JSON keys + PHP log context keys | ✅ |
