...
GSearch CLI implements a comprehensive enum-based architecture (spec/20-gsearch-cli/01-backend/58-enum-architecture.md, error range 7900-7919) for type-safe configuration. All platform, provider, engine, and mode selections use typed enums from the `internal/enums/` directory structure. All packages use the `type` suffix convention with no underscores, single `variantLabels` table with PascalCase values, `Label()` delegates to `String()`, `Parse()` uses `strings.EqualFold()`.
...
- Specification: `spec/20-gsearch-cli/01-backend/58-enum-architecture.md`
