...
**Date:** 2026-02-06  
**Auditor:** AI  
**Scope:** `spec/06-split-db-architecture/`, `spec/07-seedable-config-architecture/`  
**Files Reviewed:** 12  
**Status:** Complete
...
|---|---------|-------------|----------|
| I-01 | `spec/07-seedable-config-architecture/00-overview.md` lines 258-287 | **Database table names use `snake_case`.** Tables are named `config_meta`, `settings`, `settings_history` with `snake_case` column names (`seed_version`, `created_at`). This directly violates the project-wide PascalCase database naming convention established in `spec/01-general-spec/04-advanced/03-database-conventions-advanced.md` and enforced in Split DB specs. | 🔴 Critical |
| I-02 | `spec/07-seedable-config-architecture/03-rag-validation-helpers.md` lines 19-30 | **Error code mapping inconsistency.** The helper maps error names like `RAG_CHUNK_SIZE_INVALID` to code `AB-9301`, using the `AB-NNNN` prefix format. But the actual error codes in Go implementations (per `spec/22-ai-bridge-cli/`) use plain integers (9301). The prefix is never used in runtime code. | 🟡 Warning |
| I-03 | `spec/06-split-db-architecture/00-overview.md` lines 198-199 | **`Status` field uses string type.** The `Projects.Status` column is `TEXT DEFAULT 'active'` with comment "active, archived, deleted" — this should reference `pipeline_status.Variant` or a shared `entity_status.Variant` enum per the enum specification. Same issue in `Databases.Status` (line 220). | 🟡 Warning |
| I-04 | `spec/06-split-db-architecture/01-cli-examples.md` lines 54-55 | **AI Bridge chat session path mismatch.** CLI examples show `data/{app}/ai/chat/001-{id}.db` but the memory context and `spec/22-ai-bridge-cli/01-backend/12-database-architecture.md` use the updated path `data/{app}/rag/chat/{company}/{seq}-{sessionId}.db` with company-scoping. The CLI examples document is outdated. | 🔴 Critical |
| I-05 | `spec/07-seedable-config-architecture/06-validation-data-seeding.md` lines 230-238 | **`ValidationCategory` uses `string` type instead of enum.** The spec defines `type ValidationCategory string` with const values — this should use the byte-based `Variant` pattern per `spec/17-enum-specification/`. Same issue with `SeoKey`, `RagKey`, `FaqKey`, `SearchKey` types. | 🟡 Warning |
| I-06 | `spec/06-split-db-architecture/04-rbac-casbin.md` lines 190-196 | **`RbacLevel` uses `string` type.** Defines `type RbacLevel string` with const values `"root"`, `"app"`, `"company"` — should use byte-based enum pattern. | 🟡 Warning |
| I-07 | `spec/06-split-db-architecture/05-user-scoped-isolation.md` lines 240-243 | **`ScopeLevel` uses `string` type.** Defines `type ScopeLevel string` with `"app"` and `"company"` — should use byte-based enum pattern. | 🟡 Warning |
| I-08 | `spec/07-seedable-config-architecture/02-rag-chunk-settings.md` lines 122-127 | **Error code numbering mismatch.** Validator uses codes AB-9301 through AB-9306 for chunk validation, but the error code mapping in `03-rag-validation-helpers.md` uses AB-9301 through AB-9310 with different assignments (e.g., `9303` is "overlap too large" in helpers but "chunk size not multiple" in chunk settings). The chunk size validator assigns 9301=range check and 9303=multiple check, while the helpers assign 9301=range and 9302=multiple. | 🟡 Warning |
| I-09 | `spec/06-split-db-architecture/02-reset-api-standard.md` lines 309-315 | **BRun reset error range mismatch.** States BRun reset errors are at 6401-6409, but BRun's allocated range is 7100-7599 per the error code registry. Reset errors should be at 7401-7409 or within 7100-7599. | 🟡 Warning |
...
| File | Has AC? |
|------|---------|
| `spec/06-split-db-architecture/00-overview.md` | ❌ |
| `spec/06-split-db-architecture/01-cli-examples.md` | ❌ |
| `spec/06-split-db-architecture/02-reset-api-standard.md` | ❌ |
| `spec/06-split-db-architecture/03-database-flow-diagrams.md` | ❌ |
| `spec/06-split-db-architecture/04-rbac-casbin.md` | ❌ |
| `spec/06-split-db-architecture/05-user-scoped-isolation.md` | ❌ |
| `spec/07-seedable-config-architecture/00-overview.md` | ❌ |
| `spec/07-seedable-config-architecture/02-rag-chunk-settings.md` | ❌ |
| `spec/07-seedable-config-architecture/03-rag-validation-helpers.md` | ❌ |
| `spec/07-seedable-config-architecture/04-rag-validation-tests.md` | ❌ (has test patterns but no formal AC) |
| `spec/07-seedable-config-architecture/05-rag-test-coverage-matrix.md` | ❌ |
| `spec/07-seedable-config-architecture/06-validation-data-seeding.md` | ❌ |
...
|----------|--------|----------------|
| 🔴 P0 | Convert all table/column names in `spec/07-seedable-config-architecture/00-overview.md` from `snake_case` to PascalCase (`config_meta` → `ConfigMeta`, `seed_version` → `SeedVersion`, etc.) | `00-overview.md` |
| 🔴 P0 | Update AI Bridge chat session paths in `spec/06-split-db-architecture/01-cli-examples.md` to match the current company-scoped pattern `data/{app}/rag/chat/{company}/{seq}-{sessionId}.db` | `01-cli-examples.md` |
| 🟡 P1 | Convert `RbacLevel`, `ScopeLevel`, `ValidationCategory`, `SeoKey` etc. from `string` types to byte-based `Variant` enums per `spec/17-enum-specification/` | `04-rbac-casbin.md`, `05-user-scoped-isolation.md`, `06-validation-data-seeding.md` |
