...
// EXEMPTED: Config file keys — mapstructure tags must match YAML config key names
type Config struct {
    Version    string             `mapstructure:"version"`
    WorkDir    string             `mapstructure:"workDir"`
    LogDir     string             `mapstructure:"logDir"`
    Profiles   map[string]Profile `mapstructure:"profiles"`
    Apps       map[string]AppDef  `json:"applications" mapstructure:"applications"` // EXEMPTED: Config file key
...
func (c *Copier) copyDir(src, dst string) *apperror.AppError {
    ...
    walkErr := filepath.Walk(src, func(path string, info os.FileInfo, err error) error {
        ...
        isIgnored := c.isIgnored(info.Name())
        ...
        return c.processEntry(src, dst, path, info)
    })
...
func (c *Copier) isIgnored(name string) bool {
    for _, pattern := range c.ignore {
        isMatch, _ := filepath.Match(pattern, name)
...
func (c *Copier) processEntry(src, dst, path string, info os.FileInfo) error {
    relPath, _ := filepath.Rel(src, path)
    dstPath := filepath.Join(dst, relPath)

    if info.IsDir() {
        return pathutil.EnsureDirWithMode(dstPath, info.Mode())
    }
...
func (c *Copier) copyFile(src, dst string) error {
    ...
    dstFile, err := pathutil.Create(dst)
    if err != nil {
        return err
    }
...
func NewBuildCmd(cfg *config.Config) *cobra.Command {
    ...
    cmd := &cobra.Command{
        ...
        RunE: func(cmd *cobra.Command, args []string) error {
            profile, isFound := cfg.Profiles[profileName]
            isMissing := !isFound
