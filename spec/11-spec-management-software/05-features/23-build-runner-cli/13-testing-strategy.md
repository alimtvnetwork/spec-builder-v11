...
func (e *TestEnv) WriteConfig(t *testing.T, cfg *config.Config) {
    ...
    pathutil.WriteFile(e.ConfigPath, data, 0644)
}
...
func (m *MockRunner) Execute(context stdctx.Context, cmd string, args []string, workDir string) apperror.Result[RunOutput] {
    ...
    return m.findMatchingResponse(context, cmd)
}

func (m *MockRunner) findMatchingResponse(context stdctx.Context, cmd string) apperror.Result[RunOutput] {
    for pattern, resp := range m.responses {
        isMatch := matchesPattern(cmd, pattern)

        if isMatch {
            return m.executeResponse(context, resp)
        }
    }
...
func (m *MockRunner) executeResponse(context stdctx.Context, resp *MockResponse) apperror.Result[RunOutput] {
    hasDelay := resp.Delay > 0
    ...
    if hasDelay {
        ...
        case <-context.Done():
            return apperror.Fail[RunOutput](
                apperror.Wrap(context.Err(), "context canceled").WithSkip(1),
            )
        }
    }

    if resp.Err != nil {
        return apperror.Fail[RunOutput](resp.Err)
    }
...
func (m *MockExecutor) Execute(context stdctx.Context, cmd *Command) apperror.Result[Result] {
    m.ExecuteCalls = append(m.ExecuteCalls, ExecuteCall{Command: cmd})

    if result, isFound := m.ExecuteResults[cmd.Script]; isFound {
...
func TestWaitForPort(t *testing.T) {
    ...
    err := manager.WaitForPort(context, port)
...
func TestWaitForPortTimeout(t *testing.T) {
    ...
    context, cancel := stdctx.WithTimeout(stdctx.Background(), 500*time.Millisecond)
    defer cancel()
    
    // Wait for port that never opens
    err := manager.WaitForPort(context, 59999)
    assert.Error(t, err)
}
```
...
func TestCopyModeClear(t *testing.T) {
    ...
    pathutil.WriteFile(filepath.Join(src, "file1.txt"), []byte("content1"), 0644)
    pathutil.WriteFile(filepath.Join(src, "file2.txt"), []byte("content2"), 0644)
    
    // Create pre-existing destination
    pathutil.EnsureDir(dst)
    pathutil.WriteFile(filepath.Join(dst, "old.txt"), []byte("old"), 0644)
    ...
    assert.True(t, pathutil.IsMissing(filepath.Join(dst, "old.txt")))
    
    // New files should exist
    assert.True(t, pathutil.Exists(filepath.Join(dst, "file1.txt")))
}
...
func TestCopyModeOverride(t *testing.T) {
    ...
    // Create source
    pathutil.EnsureDir(src)
    pathutil.WriteFile(filepath.Join(src, "file.txt"), []byte("new"), 0644)
    
    // Create destination with same file
    pathutil.EnsureDir(dst)
    pathutil.WriteFile(filepath.Join(dst, "file.txt"), []byte("old"), 0644)
    pathutil.WriteFile(filepath.Join(dst, "keep.txt"), []byte("keep"), 0644)
    ...
    assert.True(t, pathutil.Exists(filepath.Join(dst, "keep.txt")))
}
...
func TestCopyWithIgnorePatterns(t *testing.T) {
    ...
    pathutil.WriteFile(filepath.Join(src, "index.js"), []byte("code"), 0644)
    pathutil.WriteFile(filepath.Join(src, "node_modules", "pkg", "lib.js"), []byte("lib"), 0644)
    ...
    assert.True(t, pathutil.Exists(filepath.Join(dst, "index.js")))
    
    // node_modules should be ignored
    assert.True(t, pathutil.IsMissing(filepath.Join(dst, "node_modules")))
}
```
...
func TestLoadValidConfig(t *testing.T) {
    ...
    pathutil.WriteFile(configPath, []byte(configJson), 0644)
...
func TestBuildFlowSuccess(t *testing.T) {
    ...
    result, err := eng.ExecuteProfile(context, "backend")
...
func TestBuildFlowWithPreCommands(t *testing.T) {
    ...
    context := stdctx.Background()
    _, err := eng.ExecuteProfile(context, "backend")
...
func TestCheckGoErrors(t *testing.T) {
    ...
    context := stdctx.Background()
    result, err := checker.Check(context, "./cmd/app")
...
func TestCheckNodeErrors(t *testing.T) {
    ...
    context := stdctx.Background()
    result, err := checker.Check(context, "./frontend")
...
func TestHealthCheckSuccess(t *testing.T) {
    ...
    context := stdctx.Background()
    err := checker.CheckUrl(context, server.URL+"/health", 200)
...
func TestWaitForHealthy(t *testing.T) {
    ...
    context := stdctx.Background()
    err := checker.WaitForHealthyUrl(context, server.URL+"/health", app.HealthCheck)
