...
func (s *ModelSelector) SelectModel(context stdctx.Context, diagramType DiagramType, preferences *UserPreferences) apperror.Result[SelectedModel] {
	...
	for _, fallbackId := range fallbacks {
		fbResult := s.getModel(fallbackId)
		if fbResult.HasError() {
			continue
		}
		
		model := fbResult.Value()
...
func (s *ModelSelector) isModelAvailable(context stdctx.Context, modelId string) bool {
	s.mu.RLock()
	...
	available := s.healthCheck.Check(context, modelId)
...
func (s *ModelSelector) ListModels(context stdctx.Context) []ModelInfo {
	...
	for _, cfg := range s.config.Models {
		...
		available := s.isModelAvailable(context, cfg.Id)
...
func (h *ModelHealthChecker) Check(context stdctx.Context, modelId string) bool {
	...
	req, err := http.NewRequestWithContext(context, httpmethod.Get.String(), endpoint, nil)
	if err != nil {
		return false
	}
...
func (h *ModelHealthChecker) CheckAll(context stdctx.Context) map[string]bool {
	results := make(map[string]bool)
	
	for provider := range h.endpoints {
		results[provider] = h.checkProvider(context, provider)
	}
	
	return results
}

func (h *ModelHealthChecker) checkProvider(context stdctx.Context, provider string) bool {
	endpoint, ok := h.endpoints[provider]
	if !ok {
		return false
	}
...
- [../06-ai-integration/03-model-management.md](../06-ai-integration/07-llm-server-management.md) - Model management
