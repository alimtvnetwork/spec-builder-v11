...
export function PlanApprovalPanel({
  ...
  const isExecuting = plan.status === PlanStatus.Executing;
  const isPaused = plan.status === PlanStatus.Paused;
  const isFailed = plan.status === PlanStatus.Failed;
  const isComplete = plan.status === PlanStatus.Completed;
...
export function StepCard({
  ...
  const canEdit = planStatus === PlanStatus.Draft || 
    (planStatus === PlanStatus.Paused && step.status === StepStatus.Pending);
  const canExecute = step.status === StepStatus.Ready && 
    (planStatus === PlanStatus.Approved || planStatus === PlanStatus.Paused);
  const canSkip = step.status === StepStatus.Pending || step.status === StepStatus.Ready;
  const canRetry = step.status === StepStatus.Failed;
  
  return (
    <Card className={cn(
      'transition-all duration-200',
      isCurrentStep && 'ring-2 ring-primary shadow-md',
      step.status === StepStatus.Completed && 'opacity-70',
      step.status === StepStatus.Skipped && 'opacity-50',
    )}>
...
- [32-plan-execution.md](./32-plan-execution.md) - Execution engine
- [34-mermaid-integration.md](./34-mermaid-integration.md) - Diagram display
