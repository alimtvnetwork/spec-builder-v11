...
export class AudioSyncQueue {
  ...
  cancel(id: string): boolean {
    ...
    if (!op || op.status === SyncStatus.Completed) return false;
  ...
  private restoreQueue(): void {
    ...
    if (entries) {
      ...
      for (const op of this.queue.values()) {
        if (op.status === SyncStatus.Uploading || op.status === SyncStatus.Transcribing) {
  ...
  const refreshState = useCallback(() => {
    ...
    setState({
      ...
      isSyncing: allOps.some(op => 
        op.status === SyncStatus.Uploading || op.status === SyncStatus.Transcribing
      ),
      pendingCount: allOps.filter(op => op.status === SyncStatus.Pending).length,
      failedCount: allOps.filter(op => op.status === SyncStatus.Failed).length,
      operations: allOps,
    });
  }, [queue]);
...
func (h *AudioHandler) InitUpload(w http.ResponseWriter, r *http.Request) {
	...
	if err := pathutil.EnsureDir(tempDir); err != nil {
...
func (h *AudioHandler) UploadChunk(w http.ResponseWriter, r *http.Request) {
	...
	out, err := pathutil.Create(chunkPath)
	if err != nil {
...
func (h *AudioHandler) CompleteUpload(w http.ResponseWriter, r *http.Request) {
	...
	if err := pathutil.EnsureDir(filepath.Dir(finalPath)); err != nil {
		http.Error(w, "Failed to create directory", http.StatusInternalServerError)
		return
	}
	
	finalFile, err := pathutil.Create(finalPath)
	if err != nil {
	...
	for i := 0; i < session.TotalChunks; i++ {
		...
		chunk, err := pathutil.Open(chunkPath)
		if err != nil {
	...
	pathutil.RemoveAll(session.TempDir)
...
- [Voice Resilience](./20-voice-resilience.md)
- [Audio Capture](./21-audio-capture.md)
- [Transcription Service](./22-transcription-service.md)
- [Sync Queue](./12-sync-queue.md)
