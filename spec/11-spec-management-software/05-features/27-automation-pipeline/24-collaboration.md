...
CREATE INDEX IdxCollabPipeline ON CollaborationSession(PipelineId);
CREATE INDEX IdxCollabActive ON CollaborationSession(IsActive);
```
...
CREATE INDEX IdxParticipantSession ON SessionParticipant(SessionId);
CREATE INDEX IdxParticipantUser ON SessionParticipant(UserId);
```
...
CREATE INDEX IdxOpSession ON CollaborationOperation(SessionId);
CREATE INDEX IdxOpServerSeq ON CollaborationOperation(SessionId, ServerSeq);
CREATE INDEX IdxOpTarget ON CollaborationOperation(TargetType, TargetId);
```
          ...
          <Tooltip>
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    participant.status === ParticipantStatus.Active && "bg-success",
                    participant.status === ParticipantStatus.Idle && "bg-warning",
                    participant.status === ParticipantStatus.Disconnected && "bg-muted"
                  )}
                />
              </div>
            </TooltipTrigger>
            <TooltipContent>
              ...
              <p className="text-xs text-muted-foreground">
                {participant.status === ParticipantStatus.Active ? 'Active now' :
                 participant.status === ParticipantStatus.Idle ? 'Idle' : 'Disconnected'}
              </p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
