...
function GenerationPipeline({
  ...
  return (
    <div className="space-y-6">
      ...
      {stages.find(s => s.status === ExecutionStatus.Running) && (
        <Card>
          ...
          <CardContent>
            <Progress 
              value={stages.find(s => s.status === ExecutionStatus.Running)?.progress || 0 * 100} 
            />
          </CardContent>
        </Card>
      )}
      ...
      {stages
        .filter(s => s.status === ExecutionStatus.Completed && s.output)
        .map(stage => (
