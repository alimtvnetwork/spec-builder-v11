...
type File struct {
    ...
    // Single column index
    Name string `gorm:"type:text;index:IdxFileName"`
    
    // Unique index
    Slug string `gorm:"type:text;uniqueIndex:IdxFileSlug"`
}
```
...
func BackupDatabase(dbPath string) apperror.Result[string] {
    ...
    dst, err := pathutil.Create(backupPath)
    if err != nil {
        return "", err
    }
    defer dst.Close()
...
- [Relationships](./04-relationships.md) — Foreign key constraints
