...
This document specifies unit tests for the `ConfigValidator` component defined in [09-seeding-configuration.md](../../07-seedable-config-architecture/00-overview.md) section 9.2.1. Tests cover range validation, format validation, cross-key dependencies, cron parsing, and path validation.

**Cross-References:**
- [Seeding Configuration](../../07-seedable-config-architecture/00-overview.md) - ConfigValidator implementation
- [Import/Export System](../05-features/03-project-management/01-import-export-system.md) - Uses validated config
- [Path Manager](../05-features/02-file-management/02-path-manager.md) - Path validation patterns
...
// ImportConfigFixture holds typed import configuration for testing
type ImportConfigFixture struct {
    MaxFileSizeBytes       int64    `json:"import.maxFileSizeBytes"`
    MaxZipSizeBytes        int64    `json:"import.maxZipSizeBytes"`
    AllowedExtensions      []string `json:"import.allowedExtensions"`
    MaxFilesPerArchive     int      `json:"import.maxFilesPerArchive"`
    MaxConcurrentJobs      int      `json:"import.maxConcurrentJobs"`
    JobTimeoutSeconds      int      `json:"import.jobTimeoutSeconds"`
    TempDirectory          string   `json:"import.tempDirectory"`
    CleanupTempAfterSecs   int      `json:"import.cleanupTempAfterSeconds"`
    PrdSectionSeparator    string   `json:"import.prdSectionSeparator"`
    AutoGenerateProjectJson bool    `json:"import.autoGenerateProjectJson"`
}

var ValidImportConfig = ImportConfigFixture{
    MaxFileSizeBytes:       104857600,  // 100MB
    MaxZipSizeBytes:        524288000,  // 500MB
    AllowedExtensions:      []string{".md", ".json"},
    MaxFilesPerArchive:     500,
    MaxConcurrentJobs:      3,
    JobTimeoutSeconds:      300,
    TempDirectory:          "./data/import-temp",
    CleanupTempAfterSecs:   3600,
    PrdSectionSeparator:    "\n## ",
    AutoGenerateProjectJson: true,
}

// ExportConfigFixture holds typed export configuration for testing
type ExportConfigFixture struct {
    RetentionDays          int    `json:"export.retentionDays"`
    MaxConcurrentJobs      int    `json:"export.maxConcurrentJobs"`
    JobTimeoutSeconds      int    `json:"export.jobTimeoutSeconds"`
    OutputDirectory        string `json:"export.outputDirectory"`
    IncludeChecksum        bool   `json:"export.includeChecksum"`
    CompressionLevel       int    `json:"export.compressionLevel"`
    MaxExportsPerProject   int    `json:"export.maxExportsPerProject"`
    ScheduledExportEnabled bool   `json:"export.scheduledExportEnabled"`
    ScheduledExportCron    string `json:"export.scheduledExportCron"`
}

var ValidExportConfig = ExportConfigFixture{
    RetentionDays:          30,
    MaxConcurrentJobs:      3,
    ...
    CompressionLevel:       6,
    MaxExportsPerProject:   10,
    ScheduledExportEnabled: false,
    ScheduledExportCron:    "0 2 * * 0",
}
```
...
func TestCrossKeyValidation_ZipSizeVsFileSize(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            config := CrossKeyConfig{
                MaxFileSizeBytes: tt.maxFileSize,
                MaxZipSizeBytes:  tt.maxZipSize,
            }
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            config := CronDependencyConfig{
                ScheduledExportEnabled: tt.enabled,
                ScheduledExportCron:    tt.cronExpr,
            }
...
func TestConcurrentValidation(t *testing.T) {
    ...
    for i := 0; i < 100; i++ {
        ...
        go func(idx int) {
            defer wg.Done()
            config := CrossKeyConfig{
                MaxFileSizeBytes: int64(1048576 + idx*1000),
                MaxZipSizeBytes:  int64(524288000),
            }
