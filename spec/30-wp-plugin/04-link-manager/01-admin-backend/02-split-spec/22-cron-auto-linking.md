...
CREATE INDEX IdxAutolinkJobsStatus ON AutoLinkJobs(Status);
CREATE INDEX IdxAutolinkJobsSchedule ON AutoLinkJobs(NextScheduledAt);
CREATE INDEX IdxAutolinkJobsCreated ON AutoLinkJobs(CreatedAt DESC);
...
CREATE INDEX IdxAutolinkQueueJob ON AutoLinkQueue(JobId, Processed);
...
CREATE INDEX IdxAutolinkSchedulesActive ON AutoLinkSchedules(IsActive, NextRunAt);
```
...
final class CronAutoLinkService implements CronAutoLinkServiceInterface
{
    ...
    public function createJob(
        ...
        $linksPerContent = $options['linksPerContent'] ?? DEFAULT_LINKS_PER_CONTENT;
        ...
        Logger::info('Auto-linking job created', [
            ...
            'LinksPerContent' => $linksPerContent
        ]);
        ...
        try {
            // Create job from schedule config
                ...
                [
                    ...
                    'LinksPerContent' => $schedule['LinksPerContent'],
                    'MaxInternalLinks' => $schedule['MaxInternalLinks'],
                    'CategoryIds' => json_decode($schedule['CategoryIds'] ?? '[]', true),
        ...
        foreach ($contentTypes as $contentType) {
            ...
            foreach ($orphans as $orphan) {
                $items[] = [
                    'Type' => $contentType,
                    'Id' => $orphan['WpId'],
                    'CurrentLinks' => $orphan['InternalLinkCount'] ?? 0
                ];
            }
        }
    ...
    private function completeJob(string $jobId): void
    {
        ...
        Logger::info('Auto-linking job completed', [
            ...
            'File' => __FILE__,
            'JobId' => $jobId,
            'LinksCreated' => $job['LinksCreated'] ?? 0,
            'ContentUpdated' => $job['ContentUpdated'] ?? 0
        ]);
    ...
    public function getProgress(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        return new \WP_REST_Response([
            ...
            'Progress' => [
                ...
                'LinksCreated' => $progress->getLinksCreated(),
                'ContentUpdated' => $progress->getContentUpdated(),
                'EtaSeconds' => $progress->getEstimatedTimeRemaining()
            ],
