...
CREATE INDEX IdxJobsStatus ON ScanJobs(Status);
CREATE INDEX IdxJobsCreated ON ScanJobs(CreatedAt DESC);
...
CREATE INDEX IdxQueueJob ON JobQueue(JobId, Processed);
```
...
final class CronService implements CronServiceInterface
{
    ...
    private function completeJob(string $jobId): void
    {
        ...
        $this->logger->info('Scan job completed', ['JobId' => $jobId]);
    ...
    public function getProgress(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        return new \WP_REST_Response([
            ...
            'progress' => [
                ...
                'linksFound' => $progress->getLinksFound(),
                'brokenFound' => $progress->getBrokenFound(),
