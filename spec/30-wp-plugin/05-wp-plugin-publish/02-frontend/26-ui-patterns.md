...
2. If step exists with `status === ExecutionStatus.Running`: **Update in-place** (don't append)
...
```
{Step: "AuthCheck", Status: "Running", Message: "Authenticating..."}
{Step: "AuthCheck", Status: "Success", Message: "Authenticated as admin"}
```
...
const existingIndex = prev.steps.findIndex(
  (s) => s.step === step && s.status === ExecutionStatus.Running
);
...
type CreateInput struct {
  // ... other fields
  ForceCreate bool // Skip validation errors
}

func (s *Service) Create(context stdctx.Context, input CreateInput) {
