...
const api = {
  ...
  postErrorHistory: (error: CapturedError) => 
    request<void>('/error-history', { method: HttpMethod.Post, body: JSON.stringify(error) }),
};
```
...
- [Error Modal Spec](./03-error-modal-reference.md) — Full modal structure, data model, and UX specification
- [Copy Format Samples](./01-copy-formats.md) — Complete samples for all copy/export formats
- [Error Handling Spec](../06-error-handling/02-error-handling-reference.md) — Cross-stack error architecture
- [Response Envelope Schema](../09-response-envelope/envelope.schema.json) — JSON Schema source of truth
