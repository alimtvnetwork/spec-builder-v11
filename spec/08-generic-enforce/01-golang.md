...
> For rules, rationale, and the canonical example, see [`00-overview.md`](./00-overview.md).
...
### `*string` is Prohibited — Use `string` with `omitempty`

Go pointer types for basic values (`*string`, `*int`, `*bool`, `*float64`) are **prohibited** in struct fields. Use the zero value with `json:",omitempty"` or `gorm:"default:null"` instead.

```go
// ❌ PROHIBITED — pointer types for nullable strings
type Config struct {
...
// ✅ REQUIRED — zero-value string with omitempty
type Config struct {
...
### `*time.Time` is Prohibited — Use `time.Time` with GORM defaults
...
type Record struct {
    ...
    LastSyncAt  time.Time `gorm:"default:null"`
}
```

**Rationale:** Pointer types add nil-check boilerplate, increase GC pressure, and obscure intent. The zero value (`""` for string, `time.Time{}` for time) combined with `omitempty` achieves the same semantics without pointers.
...
BAD:    *string / *int / *bool in struct fields  → Use zero value + omitempty
