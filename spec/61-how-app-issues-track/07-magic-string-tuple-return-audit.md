...
1. **Magic string HTTP methods** — Raw `"GET"`, `"POST"`, `"DELETE"`, `"PATCH"` literals instead of the `HttpMethod` enum defined in `spec/21-brun-cli/01-backend/19-enum-architecture.md`
...
Per memory `architecture/enum-standard`: Magic strings for HTTP methods (`"GET"`, `"POST"`) are **prohibited** and must be implemented as dedicated enums. The canonical `HttpMethod` enum exists in `spec/21-brun-cli/01-backend/19-enum-architecture.md`.
...
- `spec/21-brun-cli/01-backend/19-enum-architecture.md` — This is the **enum definition** itself (`Get: "GET"` in `variantStrings`)
- `spec/26-ai-transcribe-cli/01-backend/11-configuration.md` — CORS config array `Methods: ["GET", "POST", ...]` is a JSON/YAML config value, not Go code
- `spec/20-gsearch-cli/01-backend/12-testing-strategy.md` — `httpmock.RegisterResponder("GET", ...)` uses the mock library's string API (borderline; should still use `.String()`)
- `spec/20-gsearch-cli/02-frontend/02-frontend-architecture.md` — JSON fixture `"method": "POST"` is data, not Go code
