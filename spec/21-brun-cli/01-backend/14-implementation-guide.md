...
func main() {
    ...
    if cfgResult.HasError() {
        log.Error().Err(cfgResult.Error()).Msg("Failed to load configuration")
        os.Exit(1)
    }
    
    // Execute CLI
    execErr := cli.Execute(cfgResult.Value())
    if execErr != nil {
        log.Error().Err(execErr).Msg("Command failed")
        os.Exit(1)
    }
}
```
...
type Config struct {
    ...
    Apps       map[string]AppDef  `mapstructure:"applications"`
...
type Profile struct {
    ...
    PreCommands []string          `mapstructure:"preCommands"`
...
type AssetConfig struct {
    Mode   string   `mapstructure:"mode"` // clear, override, skip
...
type Defaults struct {
    Timeout    time.Duration `mapstructure:"timeout"`
    JsonOutput bool          `mapstructure:"jsonOutput"`
...
func (s RunStatus) String() string {
    ...
    case RunStatusCanceled:
        return "canceled"
    default:
        panic("unhandled RunStatus")
    }
}

type Run struct {
    gorm.Model
    RunId       string        `gorm:"uniqueIndex;size:50"`
    ProfileName string        `gorm:"size:100"`
    Runtime     string        `gorm:"size:20"`
    Command     string        `gorm:"type:text"`
    WorkDir     string        `gorm:"size:500"`
    Status      RunStatus     `gorm:"size:20"`
    ExitCode    int
    StartTime   time.Time
    EndTime     *time.Time    `json:",omitempty"`
    Duration    time.Duration
    Errors      []BuildError  `gorm:"foreignKey:RunId" json:",omitempty"`
}
```
    ...
    for _, line := range lines {
        for _, pattern := range patterns {
            if matches := pattern.FindStringSubmatch(line); matches != nil {
                buildErr := p.extractError(runtime, matches)
                if buildErr != nil {
...
func (c *Copier) copyDir(src, dst string) *apperror.AppError {
    walkErr := filepath.Walk(src, func(path string, info os.FileInfo, walkItemErr error) error {
        ...
        if info.IsDir() {
            return pathutil.MkdirAllRaw(dstPath, info.Mode())
        }
...
func (c *Copier) copyFile(src, dst string) error {
    srcFile, openErr := pathutil.OpenFile(src)
    if openErr != nil {
        return openErr
    }
    defer srcFile.Close()
    
    dstFileResult := pathutil.CreateFile(dst)
    if dstFileResult.HasError() {
        return dstFileResult.Error()
    }

    dstFile := dstFileResult.Value()
    defer dstFile.Close()
...
func (e *PowerShellExecutor) Execute(context context.Context, cmd *Command) apperror.Result[*Result] {
    ...
    execCmd := exec.CommandContext(context, e.executable(), args...)
...
func NewBuildCmd(cfg *config.Config) *cobra.Command {
    ...
    cmd := &cobra.Command{
        ...
        RunE: func(cmd *cobra.Command, args []string) error {
            ...
            eng := engine.New(cfg)
            result := eng.Execute(cmd.Context(), profile)
            if result.HasError() {
                return result.Error()
            }
            
            if jsonOutput {
                return json.NewEncoder(cmd.OutOrStdout()).Encode(result.Value())
            }
            
            // Console output
            fmt.Fprintf(cmd.OutOrStdout(), "Build %s: %s\n", 
                result.Value().Status, result.Value().RunId)
...
- [gsearch Implementation Guide](../../20-gsearch-cli/01-backend/13-implementation-guide.md) — Reference template
