...
CREATE TABLE Users (
    ...
    DisplayName TEXT,
    AvatarUrl TEXT,
    CompanyId TEXT,                            -- NULL for app-level users
...
CREATE TABLE UserDbRegistry (
    ...
    CompanyId TEXT,                            -- NULL for app-level
...
CREATE INDEX IdxUserDbUser ON UserDbRegistry(UserId);
...
func (m *UserDbManager) registerUserDb(userId, category, entityId, path string) error {
    _, err := m.rootDb.Exec(`
        ...
        (Id, UserId, Category, EntityId, Path, SequenceNum, UpdatedAt)
        VALUES (?, ?, ?, ?, ?, 1, CURRENT_TIMESTAMP)
    `, fmt.Sprintf("%s_%s_%s", userId, category, entityId), userId, category, entityId, path)
    return err
}
...
func (m *UserDbManager) ListUserDatabases(userId string) apperror.Result[[]UserDatabase] {
    rows, err := m.rootDb.Query(`
        SELECT Id, UserId, Category, EntityId, Path, SizeBytes, CreatedAt, LastAccessedAt
        FROM UserDbRegistry
    ...
    for rows.Next() {
        var db UserDatabase
        if err := rows.Scan(
            &db.Id, &db.UserId, &db.Category, &db.EntityId,
