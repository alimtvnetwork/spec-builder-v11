...
func validateResetRequest(db *sql.DB, resetId string) apperror.Result[*ResetRequest] {
    ...
    err := db.QueryRow(`
        SELECT Id, Scope, ExpiresAt, Status 
        FROM ResetRequests 
        WHERE Id = ?
    `, resetId).Scan(&req.Id, &req.Scope, &req.ExpiresAt, &req.Status)
    ...
    if time.Now().After(req.ExpiresAt) {
        ...
        db.Exec("UPDATE ResetRequests SET Status = 'expired' WHERE Id = ?", resetId)
