...
- `interface{}` / `any` is **prohibited** in exported APIs (see [Go Standards](../03-golang/04-golang-standards-reference.md))
...
- **Single return value:** Functions return `apperror.Result[T]` — never `(T, error)` or multi-value tuples (see [§7.1](./15-master-coding-guidelines.md#71-single-return-value-rule-go))
- **No type assertions:** `.(Type)` casting is prohibited in business logic — use concrete structs (see [§7.2](./15-master-coding-guidelines.md#72-no-type-assertions--casting-go))
...
- [PHP Standards](../04-php/07-php-standards-reference.md)
...
- [Master Guidelines — Section 6.1](./15-master-coding-guidelines.md#61--result-guard-rule-zero-silent-failures)
