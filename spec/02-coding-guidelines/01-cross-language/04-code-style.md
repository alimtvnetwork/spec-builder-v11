...
**Rationale:** Error wrapping must always be explicit — each `.WithX()` context value on its own line (see Rule 11). Never compress error handling to save line count.
...
- [Boolean Principles](./02-boolean-principles.md) — P1–P6 boolean naming rules (all languages)
- [PHP Coding Standards](../04-php/07-php-standards-reference.md) — PHP-specific rules that reference this spec
- [PHP Forbidden Patterns](../04-php/02-forbidden-patterns.md) — PHP checklist
- [PHP Enum Classes](../04-php/01-enums.md) — `ErrorChecker` examples
