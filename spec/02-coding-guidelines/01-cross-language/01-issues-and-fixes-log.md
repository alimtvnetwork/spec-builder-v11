...
**Prevention:** All log context array keys must use camelCase. See [naming-conventions.md § Array Key Conventions](../04-php/00-overview.md#array-key-conventions).
...
**Prevention:** Rule 4 in [code-style.md](./04-code-style.md). Exception: if `return`/`throw` is the only statement, no blank line needed.
