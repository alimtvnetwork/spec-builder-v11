...
> **Cross-Reference:** `spec/03-error-code-registry/01-registry.md`

---

## Purpose

Automated tool that parses the master error code registry (`01-registry.md`) and detects overlapping or duplicate range allocations. Prevents the class of collision bugs documented in `spec/61-how-app-issues-track/16-error-code-collision-remediation.md`.

---

## Input

The validator reads `spec/03-error-code-registry/01-registry.md` and extracts all rows from the **Registered Project Prefixes** table (Section 1) and all **Standalone Specification Error Ranges** detail tables.
...
RangeEntry {
    ...
    SpecPath   string   // e.g., "spec/11-spec-management-software/..."
...
Registry: spec/03-error-code-registry/01-registry.md
Entries parsed: {N}
...
|------|---------|-------------|
| `--registry` | `spec/03-error-code-registry/01-registry.md` | Path to registry file |
...
- **Prevention:** See `spec/61-how-app-issues-track/16-error-code-collision-remediation.md` for the class of bugs this tool prevents.
