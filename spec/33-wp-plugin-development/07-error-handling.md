...
> **Every catch block MUST pass the exception `$e` to the logger — never log only `$e->getMessage()` without the stack trace.**

Errors should never cause silent failures or unexplained crashes. Every error must be:
1. Caught (as `Throwable`)
2. Logged with **full stack trace** (by passing `$e` to the logger)
3. Handled gracefully

### CRITICAL: Never Drop the Stack Trace

When a `Throwable` is caught, the exception object contains the full stack trace — the single most important debugging artifact. Logging only `$e->getMessage()` discards this information and makes post-mortem debugging impossible.

```php
// ❌ FORBIDDEN — message-only, stack trace lost (rule 1.8)
catch (Throwable $e) {
    $this->fileLogger->error($e->getMessage(), __FILE__, __LINE__);
}

// ✅ REQUIRED — Throwable first, stack trace preserved automatically
catch (Throwable $e) {
    $this->fileLogger->error($e, __FILE__, __LINE__);
}
```

This rule applies to **every** catch block in the entire codebase, with no exceptions. See [Forbidden Patterns §1.8](../02-coding-guidelines/04-php/02-forbidden-patterns.md).
...
if (PathHelper::isDirReadonly($dir)) {
    $this->fileLogger->log(sprintf('[ERROR] Directory not writable: %s', $dir), __FILE__, __LINE__);
...
if (json_last_error() !== JSON_ERROR_NONE) {
    $this->fileLogger->log(
        sprintf('[ERROR] JSON decode failed: %s | Input: %s', json_last_error_msg(), substr($json, 0, 100)),
        __FILE__,
        __LINE__,
    );
...
| `->error($e->getMessage(), ...)` — message first | Stack trace lost | `->error($e, __FILE__, __LINE__)` — Throwable first |
...
| `error_log()` | No structure | `$this->fileLogger->error()` or `ErrorLog($e, 'context')` pre-FileLogger |
...
- [PHP Coding Standards](../02-coding-guidelines/04-php/07-php-standards-reference.md) — ErrorChecker, safeExecute, boolean rules
- [PHP Enum Spec](../02-coding-guidelines/04-php/01-enums.md) — ErrorTypeEnum, HookType full listings
- [Error Handling Cross-Stack](../04-error-resolution/06-error-handling/02-error-handling-reference.md) — Three-tier error architecture
