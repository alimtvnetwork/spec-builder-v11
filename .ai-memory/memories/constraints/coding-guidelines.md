...
| Object shapes | Explicit interfaces required (no `I` prefix) |
...
## API Client Rules (All Languages)

| Rule | Requirement |
|------|-------------|
| HTTP methods | Enum constants (`httpmethod.Post`), never string literals (`"POST"`) |
| Operation names | Per-domain enum constants (`snapshot.Cleanup`), never inline strings |
| Return type (Go) | `apperror.Result[T]` — never `(*T, error)` tuples |
| Blank line before return | Mandatory when preceded by `}` or other code |
