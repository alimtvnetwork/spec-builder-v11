...
|------|-------------------------------|-------------------|-------------------|
| 02-spec/11-spec-management-software | Heavy (shared-packages, features, automation-pipeline) | Heavy (25+ files) | Heavy |
| 02-spec/20-gsearch-cli | Moderate (maps search, scheduled) | Moderate (5+ files) | Moderate |
| 02-spec/24-nexus-flow-cli | ✅ **Remediated** (Phase 8c) — 0 remaining `interface{}`/`map[string]any` | Low | Low |
| 02-spec/30-wp-plugin | ✅ **Remediated** (Phase 8e) — 0 remaining `interface{}`/`map[string]any` | Moderate (10+ files) | Heavy |
| 02-spec/31-wp-plugin-builder | ✅ **Remediated** (Phase 8e) — 0 remaining `interface{}`/`map[string]any` | Low | Low |
| 02-spec/01-general-spec | Moderate (keyboard-shortcuts, testing) | Moderate | Heavy |
| 02-spec/14-wp-seo-publish-cli | ✅ **Remediated** (Phase 8e) — 0 remaining `interface{}`/`map[string]any` | Low | Moderate |

### Key Violations Requiring Attention

1. **`02-spec/02/.../13-shared-packages/03-pkg-types.md`**: `type Metadata map[string]any` — needs typed struct
2. **`02-spec/02/.../13-shared-packages/04-pkg-logging.md`**: `...any` in Logger interface — **ALLOWED** (slog standard)
3. **`02-spec/02/.../13-shared-packages/06-pkg-database.md`**: Multiple `map[string]any` in error context — needs `ErrorContext` struct
4. **`02-spec/24-nexus-flow-cli/`**: ✅ **Resolved in Phase 8c** — All `interface{}`/`map[string]any` replaced with typed structs (`BlockOutputData`, `PipelineInput`, `ComponentMetadata`, `IntegrationConfig`, `IntegrationParams`, `IntegrationResult`, `ProgressMetadata`, `EscalationInput`, `BlockSettings`, `WailsBinding`)
5. **`02-spec/02/.../05-features/27-automation-pipeline/05-validation-runtime.md`**: `Input/Output` structs with `map[string]interface{}` fields
6. **`02-spec/01-general-spec/05-ux/04-keyboard-shortcuts-ux.md`**: ✅ **Resolved** — Indexes renamed to IdxPascalCase
7. **`02-spec/30-wp-plugin/wp-plugin-publish/04-testing/40-e2e-test-spec.md`**: Full snake_case schema (`suite_id`, `started_at`, `run_id`)
8. **TypeScript `as any` casts**: `02-spec/02/.../25-ai-enhancements/06-04-sharing-ui.md` — needs generic Select component
...
|------|------|----------|------------|---------|
| Error Resolution | `02-spec/02/99-acceptance-criteria.md` | 7 | 5 | 1.1.0 |
| Shared CLI Frontend | `02-spec/03/99-acceptance-criteria.md` | 16 | 12 | 1.1.0 |
| Split DB Architecture | `02-spec/04/99-acceptance-criteria.md` | 8 | 10 | 1.1.0 |
| Seedable Config | `02-spec/05/99-acceptance-criteria.md` | 7 | 8 | 1.1.0 |
| PowerShell v2 | `02-spec/06/99-acceptance-criteria.md` | 8 | 6 | 1.1.0 |
| GSearch CLI | `02-spec/08/01-backend/99-acceptance-criteria.md` | 19 | 12 | 1.1.0 |
| BRun CLI | `02-spec/09/01-backend/11-acceptance-criteria.md` | 50+ | 12 | 2.1.0 |
| AI Bridge CLI | `02-spec/10/01-backend/99-acceptance-criteria.md` | 21 | 11 | 1.1.0 |
| Nexus Flow CLI | `02-spec/11/01-backend/99-acceptance-criteria.md` | 16 | 7 | 1.1.0 |
| WP Plugin Builder | `02-spec/13/99-acceptance-criteria.md` | 11 | 5 | 1.1.0 |
| WP SEO Publish | `02-spec/14-wp-seo/01-backend/99-acceptance-criteria.md` | 11 | 5 | 1.1.0 |
| Spec Reverse | `02-spec/25-spec-reverse-cli/01-backend/99-acceptance-criteria.md` | 13 | 8 | 1.1.0 |
| AI Transcribe | `02-spec/15/01-backend/99-acceptance-criteria.md` | 12 | 5 | 1.1.0 |
| **Totals** | | **~200** | **~129** | |
