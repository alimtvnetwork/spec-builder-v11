...
**Date:** 2026-02-06  
**Auditor:** AI  
**Scope:** `02-spec/01-general-spec/`, `02-spec/17-enum-specification/`, `02-spec/00-folder-structure-guideline.md`  
**Files Reviewed:** 37  
**Status:** Complete
...
|---|---------|-------------|----------|
| I-01 | `02-spec/17-enum-specification/00-overview.md` lines 76-84 | **Stale compliance table.** Shows all CLIs as "❌ Non-Compliant" with old scores (e.g., BRun 2/50, AI Bridge 8/50) but all 9 CLIs have been remediated to 50/50 per `.lovable/audits/*-remediation-phases.md`. The overview is outdated and misleading. | 🔴 Critical |
| I-02 | `02-spec/00-folder-structure-guideline.md` lines 30-121 | **Missing CLI folders.** Directory tree does not list `02-spec/25-spec-reverse-cli/`, `02-spec/14-wp-seo-publish-cli/` (dual numbering collision on 14), `02-spec/26-ai-transcribe-cli/`, `02-spec/60-ai-research/`, or `02-spec/17-enum-specification/`. The "Global Spec Directory Structure" is incomplete. | 🔴 Critical |
| I-03 | `02-spec/00-folder-structure-guideline.md` lines 156-166 | **Numbering range table outdated.** States "20-29: WordPress" but actual WP specs are at `30-wp-plugin/` and `31-wp-plugin-builder/`. Also states "08-19: CLI Tools" but `spec/14` is used by both Spec Reverse CLI and WP SEO Publish CLI (collision). | 🔴 Critical |
| I-04 | `02-spec/01-general-spec/01-foundation/01-coding-standards-foundation.md` line 23 | **ORM property naming conflicts with project-wide PascalCase mandate.** Spec says ORM properties use `camelCase` (e.g., `createdAt`), but the project-wide naming convention (memory: `style/naming-convention`) mandates PascalCase for all JSON transport fields and Go struct fields. This creates ambiguity for Go developers — do Go GORM models use `camelCase` or `PascalCase`? | 🟡 Warning |
| I-05 | `02-spec/01-general-spec/01-foundation/02-error-management-foundation.md` lines 277-287 | **Error code ranges conflict with CLI error code registry.** Foundation spec defines `ERR_1xxx` through `ERR_9xxx` with `ERR_` prefix. CLI tools use numeric ranges without prefix (e.g., GSearch 7000-7099, AI Bridge 9000-9499). No cross-reference explains the relationship between these two systems. | 🟡 Warning |
| I-06 | `02-spec/01-general-spec/01-foundation/error-resolution/` | **Stale files remain after migration.** Files `00-overview.md`, `01-health-endpoint-mismatch.md`, `02-coding-guidelines-php.md`, `03-debugging-php.md` still exist alongside `MOVED.md`. These should have been deleted after the move to `02-spec/04-error-resolution/`. | 🟡 Warning |
| I-07 | `02-spec/01-general-spec/99-meta/03-consistency-report-meta.md` lines 4-5 | **Document count incorrect.** States "Documents Reviewed: 32" and references old flat numbering (documents 30-35) instead of the current hierarchical folder structure. The consistency report itself is inconsistent with the current structure. | 🟡 Warning |
| I-08 | `02-spec/01-general-spec/00-overview.md` lines 139-148 | **Error code quick reference incomplete.** Shows generic ranges 1xxx-9xxx but doesn't mention the CLI-specific ranges (7000+, 9000+, 10000+, etc.) defined in `02-spec/03-error-code-registry/`. | 🟠 Minor |
| I-09 | `02-spec/17-enum-specification/02-required-methods.md` line 9 | **Method count discrepancy.** Overview says "9 mandatory methods" (from memory context) but the checklist in `04-validation-checklist.md` lists 7 methods (String, Label, IsValid, Is{Value}, All, ByIndex, Parse). MarshalJSON/UnmarshalJSON are listed as "Optional" in `02-required-methods.md` line 170 but all remediation audits treat them as mandatory. | 🟡 Warning |
| I-10 | `02-spec/17-enum-specification/03-folder-structure.md` lines 96-167 | **Enum category listings are outdated.** Shows simplified/incorrect enum lists for each CLI (e.g., BRun has `build_type`, `run_mode`, `log_level`, `profile` but remediation shows 7 different enums: `runtime`, `copy_mode`, `severity`, `package_manager`, `mod_tidy_mode`, `output_format`, `http_method`). | 🟡 Warning |
| I-11 | `02-spec/01-general-spec/04-advanced/03-database-conventions-advanced.md` lines 304-343 | **PostgreSQL enum style conflicts with Go enum spec.** Database conventions show `CREATE TYPE OrderStatus AS ENUM ('Pending', ...)` with PascalCase values, but Go enum spec mandates `variantStrings` use lowercase (e.g., `"pending"`). No guidance on how SQL enum values map to Go enum strings. | 🟠 Minor |
| I-12 | `02-spec/01-general-spec/03-quality/03-api-conventions-quality.md` lines 117-135 | **Response envelope uses `camelCase` field names.** Shows `success`, `data`, `error`, `meta` with `camelCase` keys (e.g., `request_id`, `token_type`), but project-wide PascalCase mandate requires `RequestId`, `TokenType`. The Go CLIs use PascalCase in their API responses. | 🟡 Warning |
| I-13 | `02-spec/01-general-spec/03-quality/01-testing-standards-quality.md` line 1 | **Version date uses 2025.** States "Last Updated: 2025-01-26" while other docs use 2026. Inconsistent year. | 🟠 Minor |
...
| File | Has AC? | Notes |
|------|---------|-------|
| `02-spec/00-folder-structure-guideline.md` | ❌ | No validation criteria beyond prose rules |
| `02-spec/01-general-spec/01-foundation/01-coding-standards-foundation.md` | ❌ | Has examples but no testable criteria |
| `02-spec/01-general-spec/01-foundation/02-error-management-foundation.md` | ❌ | Has code samples but no criteria |
| `02-spec/01-general-spec/02-systems/01-logging-system-systems.md` | ❌ | Has format examples but no test criteria |
| `02-spec/01-general-spec/02-systems/02-configuration-hierarchy-systems.md` | ❌ | Has tier definitions but no criteria |
| `02-spec/01-general-spec/02-systems/03-conditional-helpers-systems.md` | ❌ | Has usage examples but no criteria |
| `02-spec/01-general-spec/03-quality/01-testing-standards-quality.md` | ❌ | Defines test patterns but has no self-criteria |
| `02-spec/01-general-spec/03-quality/02-file-organization-quality.md` | ❌ | Has line limits but no enforcement criteria |
| `02-spec/01-general-spec/03-quality/03-api-conventions-quality.md` | ❌ | Has response formats but no criteria |
| `02-spec/01-general-spec/04-advanced/01-security-patterns-advanced.md` | ❌ | Has patterns but no criteria |
| `02-spec/01-general-spec/04-advanced/02-caching-patterns-advanced.md` | ❌ | No criteria |
| `02-spec/01-general-spec/04-advanced/03-database-conventions-advanced.md` | ❌ | No criteria |
| `02-spec/17-enum-specification/*` (5 files) | ❌ | Has checklist but not in GIVEN/WHEN/THEN format |

---

## 3. Detailed Acceptance Criteria

### 3.1 Folder Structure Guideline (`02-spec/00-folder-structure-guideline.md`)

---

**AC-P1-001: CLI Folder Structure Validation**

GIVEN: A new CLI tool specification folder is created under `spec/` with a numeric prefix (e.g., `02-spec/18-new-cli/`)
...
- Folders with numbers outside their designated category range MUST be flagged (e.g., a CLI tool at `02-spec/25-*` which is in the WordPress range)
...
### 3.2 Coding Standards (`02-spec/01-general-spec/01-foundation/01-coding-standards-foundation.md`)
...
### 3.3 Error Management (`02-spec/01-general-spec/01-foundation/02-error-management-foundation.md`)
...
- For **Go CLI applications**: the code MUST be a plain integer within the CLI's allocated range (e.g., 7000-7099 for GSearch, 9000-9499 for AI Bridge) as defined in `02-spec/03-error-code-registry/`
- No two distinct errors within the same codebase MAY share the same error code
- The error code registry (`02-spec/03-error-code-registry/01-registry.md`) MUST be updated whenever a new range is allocated
- The relationship between the `ERR_NXXX` pattern (general spec) and the numeric-only pattern (CLI tools) MUST be documented in both `02-spec/01-general-spec/01-foundation/02-error-management-foundation.md` and `02-spec/03-error-code-registry/`
...
### 3.4 Logging System (`02-spec/01-general-spec/02-systems/01-logging-system-systems.md`)
...
### 3.5 Configuration Hierarchy (`02-spec/01-general-spec/02-systems/02-configuration-hierarchy-systems.md`)
...
### 3.6 Conditional Helpers (`02-spec/01-general-spec/02-systems/03-conditional-helpers-systems.md`)
...
### 3.7 Testing Standards (`02-spec/01-general-spec/03-quality/01-testing-standards-quality.md`)
...
### 3.8 API Conventions (`02-spec/01-general-spec/03-quality/03-api-conventions-quality.md`)
...
### 3.9 Security Patterns (`02-spec/01-general-spec/04-advanced/01-security-patterns-advanced.md`)
...
### 3.10 Database Conventions (`02-spec/01-general-spec/04-advanced/03-database-conventions-advanced.md`)
...
### 3.11 Enum Specification (`02-spec/17-enum-specification/`)
...
- After remediation, the CLI's score in the enum specification overview (`02-spec/17-enum-specification/00-overview.md`) MUST be updated to reflect the new score
...
### 3.12 Caching Patterns (`02-spec/01-general-spec/04-advanced/02-caching-patterns-advanced.md`)
...
### 3.13 File Organization (`02-spec/01-general-spec/03-quality/02-file-organization-quality.md`)
...
|---|----------------|----------|----------------|
| R-01 | **Update enum spec overview table** to reflect all 9 CLIs at 50/50 compliance | 🔴 Critical | `02-spec/17-enum-specification/00-overview.md` |
| R-02 | **Update folder structure guideline** to include all current CLI folders (15-spec-reverse, 21-wp-seo-publish, 16-ai-transcribe, 60-ai-research) and resolve the `spec/14` numbering collision | 🔴 Critical | `02-spec/00-folder-structure-guideline.md` |
| R-03 | **Update numbering range table** to reflect actual folder assignments | 🔴 Critical | `02-spec/00-folder-structure-guideline.md` |
| R-04 | **Add cross-reference** between general-spec error code system (`ERR_NXXX`) and CLI error code registry (numeric ranges) explaining how they coexist | 🟡 Warning | `02-spec/01-general-spec/01-foundation/02-error-management-foundation.md`, `02-spec/03-error-code-registry/` |
| R-05 | **Delete stale files** in `02-spec/01-general-spec/01-foundation/error-resolution/` (keep only `MOVED.md` or delete the entire subfolder) | 🟡 Warning | `02-spec/01-general-spec/01-foundation/error-resolution/00-overview.md`, `01-health-endpoint-mismatch.md`, `02-coding-guidelines-php.md`, `03-debugging-php.md` |
| R-06 | **Clarify ORM property naming** — add a note to coding standards that Go structs use PascalCase (project convention) while PHP/TypeScript ORM properties MAY use camelCase per framework convention | 🟡 Warning | `02-spec/01-general-spec/01-foundation/01-coding-standards-foundation.md` |
| R-07 | **Update consistency report** to use current folder-based structure instead of flat document numbering | 🟡 Warning | `02-spec/01-general-spec/99-meta/03-consistency-report-meta.md` |
| R-08 | **Standardize MarshalJSON/UnmarshalJSON as mandatory** in the enum spec required methods document (not optional), since all CLIs implement them | 🟡 Warning | `02-spec/17-enum-specification/02-required-methods.md` |
| R-09 | **Update enum folder structure** enum category listings to match actual remediated enum sets | 🟡 Warning | `02-spec/17-enum-specification/03-folder-structure.md` |
| R-10 | **Clarify API response field casing** — add explicit note that Go CLI APIs use PascalCase JSON keys per project convention, while the general spec examples show framework-default casing | 🟡 Warning | `02-spec/01-general-spec/03-quality/03-api-conventions-quality.md` |
| R-11 | **Fix version date** from 2025 to 2026 in testing standards | 🟠 Minor | `02-spec/01-general-spec/03-quality/01-testing-standards-quality.md` |
| R-12 | **Add SQL-to-Go enum mapping guidance** explaining how PostgreSQL enum values relate to Go `variantStrings` | 🟠 Minor | `02-spec/01-general-spec/04-advanced/03-database-conventions-advanced.md` |
