...
func TestCompleteSearchFlow(t *testing.T) {
    ...
    httpmock.RegisterResponder(httpmethodtype.Get.HttpVerb(), `=~^https://www\.google\.com/search.*`,
...
func TestSearchWithMethodFallback(t *testing.T) {
    ...
    httpmock.RegisterResponder(httpmethodtype.Post.HttpVerb(), "https://html.duckduckgo.com/html/",
...
func TestNestedSearchExecution(t *testing.T) {
    ...
    httpmock.RegisterResponder(httpmethodtype.Get.HttpVerb(), "https://example.com/ml",
...
func TestRagExportJson(t *testing.T) {
    ...
    // Verify output
    data, readErr := pathutil.ReadFile(outputPath)
...
func (m *MockResponse) CreateHttpMock() func(req *http.Request) (*http.Response, error) { // EXEMPTED: http.RoundTripper signature
    return func(req *http.Request) (*http.Response, error) { // EXEMPTED: http.RoundTripper signature
        if m.Type == "error" {
            if m.Timeout {
...
func runFixturesCapture(cmd *cobra.Command, args []string) error {
    ...
    ensureErr := pathutil.EnsureDir(filepath.Dir(outputPath))
    if ensureErr != nil {
        return ensureErr
    }
    ...
    writeErr := pathutil.WriteFile(outputPath, append([]byte(header), body...), 0644)
    if writeErr != nil {
        return writeErr
    }
...
func loadTestdata(t *testing.T, filename string) string {
    ...
    if readErr != nil {
        t.Fatalf("load testdata %s: %v", filename, readErr)
    }
    return string(data)
}
