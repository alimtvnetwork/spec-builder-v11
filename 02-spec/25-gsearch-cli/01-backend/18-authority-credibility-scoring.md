...
Define the authority scoring, source weighting, and credibility classification system for evaluating search results. All configuration values follow the [Seedable Configuration Pattern](../../21-app/spec-management-software/04-coding-guidelines/05-seedable-config-pattern.md).
...
```go
// DomainScoreConfig holds typed domain authority scores
type DomainScoreConfig struct {
    Default float64
    Domains map[string]float64
}

// TLDBonusConfig holds typed TLD bonus values
type TLDBonusConfig struct {
...
func (as *AuthorityScorer) LookupAuthority(domain string) float64 {
    ...
    domainResult := GetTyped[DomainScoreConfig](as.settings, "authority_scores", "domain_scores")
    if domainResult.HasError() {
        return 0.5 // Default
    }

    domainConfig := domainResult.Value()
    
    // Exact match
    if score, ok := domainConfig.Domains[domain]; ok {
        return score
    }
    
    // TLD patterns (.edu, .gov)
    tldResult := GetTyped[TLDBonusConfig](as.settings, "authority_scores", "tld_bonuses")
    if tldResult.IsSuccess() {
        for tld, bonus := range tldResult.Value().Bonuses {
            if strings.HasSuffix(domain, tld) {
                return domainConfig.Default + bonus
            }
        }
    }
    
    // Check official docs patterns
    officialScoreResult := as.settings.GetFloat("authority_scores", "official_docs_score")
    officialScore := 0.95
    if officialScoreResult.IsSuccess() {
        officialScore = officialScoreResult.Value()
    }
    
    officialPatternsResult := as.settings.GetStringSlice("authority_scores", "official_docs_patterns")
    if officialPatternsResult.IsSuccess() {
        for _, pattern := range officialPatternsResult.Value() {
...
```go
// WeightFormulaConfig holds source weight formula parameters
type WeightFormulaConfig struct {
    AuthorityWeight  float64
    RecencyWeight    float64
    CitationsWeight  float64
}

// RecencyDecayConfig holds recency decay parameters
type RecencyDecayConfig struct {
    DaysHalfLife float64
    MinScore     float64
}

// CitationNormalizationConfig holds citation normalization parameters
type CitationNormalizationConfig struct {
    MaxCitations float64
    CapAt        float64
}
...
func (swc *SourceWeightCalculator) CalculateWeights(sources []Source) []Source {
    // Get weight formula from settings using typed accessors
    formula, _ := GetTyped[WeightFormulaConfig](swc.settings, "source_weights", "weight_formula")
    recencyConfig, _ := GetTyped[RecencyDecayConfig](swc.settings, "source_weights", "recency_decay")
    citationConfig, _ := GetTyped[CitationNormalizationConfig](swc.settings, "source_weights", "citation_normalization")
    ...
    for i := range sources {
        ...
        recency := swc.timeDecay(sources[i].PublishDate, recencyConfig.DaysHalfLife, recencyConfig.MinScore)
        
        // Citation score (normalized, capped)
        citations := math.Min(float64(sources[i].CitationCount)/citationConfig.MaxCitations, citationConfig.CapAt)
        
        // Combined weight
        sources[i].Weight = (formula.AuthorityWeight * authority) +
                           (formula.RecencyWeight * recency) +
                           (formula.CitationsWeight * citations)
    }
...
// CredibilityThresholdsConfig holds classification thresholds
type CredibilityThresholdsConfig struct {
    LowMax    float64
    MediumMax float64
}

// CredibilityCheckWeights holds per-check weights
type CredibilityCheckWeights struct {
    HttpsEnabled    float64
    DomainAuthority float64
    ContentQuality  float64
    CitationCount   float64
...
func (cc *CredibilityClassifier) Classify(source Source, checks CredibilityChecks) apperror.Result[CredibilityResult] {
    ...
    totalScore := (weights.HttpsEnabled * checks.HttpsEnabled) +
                  (weights.DomainAuthority * checks.DomainAuthority) +
                  (weights.ContentQuality * checks.ContentQuality) +
                  (weights.CitationCount * checks.CitationCount) +
                  (weights.Recency * checks.Recency) +
                  (weights.AuthorVerified * checks.AuthorVerified)
    
    // Classify based on thresholds
    var level CredibilityLevel
    if totalScore < thresholds.LowMax {
        level = CredibilityLow
    } else if totalScore < thresholds.MediumMax {
...
// ConfidenceWeightsConfig holds confidence metric weights
type ConfidenceWeightsConfig struct {
    SourceAgreementWeight    float64
    DataFreshnessWeight      float64
    SourceCountWeight        float64
    AuthorityDiversityWeight float64
}

// ConfidenceThresholdsConfig holds confidence thresholds
type ConfidenceThresholdsConfig struct {
    ContradictionPenaltyMultiplier float64
    MinSourcesForHighConfidence    float64
    FreshnessHalfLifeDays          float64
}

// ConfidenceWarningsConfig holds warning thresholds and messages
type ConfidenceWarningsConfig struct {
...
func (ca *ConfidenceAnalyzer) AnalyzeConfidence(sources []Source) apperror.Result[ConfidenceMetrics] {
    ...
    overallConfidence := (weights.SourceAgreementWeight * details.SourceAgreement) +
                        (weights.DataFreshnessWeight * details.DataFreshness) +
                        (weights.SourceCountWeight * details.SourceCountConfidence) +
                        (weights.AuthorityDiversityWeight * details.AuthorityDiversity)
    
    // Reduce confidence if contradictions found
    contradictionsFound := details.ContradictionPresence > 0.5
    if contradictionsFound {
        overallConfidence *= thresholds.ContradictionPenaltyMultiplier
    }
    
    // Determine warning message
    warning := ""
    if contradictionsFound || overallConfidence < warnings.LowConfidenceThreshold {
        warning = warnings.LowConfidenceMessage
    } else if overallConfidence < warnings.ModerateConfidenceThreshold {
...
// NOTE: FreshnessThresholds replaced by ConfidenceThresholdsConfig (see above)

func (ca *ConfidenceAnalyzer) calculateFreshness(sources []Source, thresholds ConfidenceThresholdsConfig) float64 {
    if len(sources) == 0 {
        return 0
    }
...
func (ca *ConfidenceAnalyzer) calculateSourceCountConfidence(sources []Source, thresholds ConfidenceThresholdsConfig) float64 {
...
func (ca *ConfidenceAnalyzer) calculateExpertAgreement(sources []Source) float64 {
    // Get expert sources from settings using typed accessor
    expertPatternsResult := GetTyped[[]string](ca.settings, "confidence_metrics", "expert_sources")
    var expertPatterns []string
    if expertPatternsResult.IsSuccess() {
        expertPatterns = expertPatternsResult.Value()
    }
    ...
    for _, s := range sources {
        ...
        for _, pattern := range expertPatterns {
            if matchPattern(s.Domain, pattern) {
                isExpert = true
                break
            }
        }
...
- [Seedable Config Pattern](../../21-app/spec-management-software/04-coding-guidelines/05-seedable-config-pattern.md) - Configuration pattern
- [Agentic Search](../../21-app/spec-management-software/05-features/25-ai-enhancements/10-offline-first-storage.md) - Search pipeline integration
