...
type SignalNormalization struct {
    GithubStarsMax            int64
    JobPostingsMax            int64
    StackOverflowQuestionsMax int64
...
type DataSourceWeights struct {
    GithubApiWeight         float64
    StackOverflowApiWeight  float64
    IndeedScraperWeight     float64
    NpmRegistryWeight       float64
    PypiRegistryWeight      float64
}
...
func LoadTrendConfig(settings SettingsService) appfault.Result[*TrendConfig] {
    ...
    config.CompositeWeights = weightsResult.Value
    ...
    config.SignalNormalization = normResult.Value
...
// CollectorMetadata holds source weight for result weighting
...
func (m *CollectorManager) CollectAll(context stdctx.Context, query CollectorQuery) appfault.Result[*AggregatedResult] {
    ...
    return m.aggregateResults(results, errChan)
}
```
...
func (c *GitHubCollector) Collect(context stdctx.Context, query CollectorQuery) appfault.Result[*CollectorResult] {
    ...
    for _, keyword := range query.Keywords {
        // Search repositories
        repos, _, err := c.client.Search.Repositories(context, keyword, &github.SearchOptions{
            Sort:  "stars",
            Order: "desc",
        })
        if err != nil {
...
func (c *StackOverflowCollector) Collect(context stdctx.Context, query CollectorQuery) appfault.Result[*CollectorResult] {
    ...
    for _, keyword := range query.Keywords {
        ...
        tagStats, err := c.fetchTagStats(context, keyword)
        if err != nil {
...
func (c *JobCollector) Collect(context stdctx.Context, query CollectorQuery) appfault.Result[*CollectorResult] {
    ...
    for _, keyword := range query.Keywords {
        ...
        for _, scraper := range c.scrapers {
            postingsResult := scraper.Scrape(context, keyword)
            if !postingsResult.IsSuccess {
                continue
            }
            totalCount += int64(len(postingsResult.Value))
        }
...
func (c *NPMCollector) Collect(context stdctx.Context, query CollectorQuery) appfault.Result[*CollectorResult] {
    ...
    for _, pkg := range query.Keywords {
        downloads, err := c.fetchDownloads(context, pkg, "last-month")
        if err != nil {
            continue
        }
...
func (m *CollectorManager) CollectWithFallback(context stdctx.Context, query CollectorQuery) appfault.Result[*AggregatedResult] {
    ...
    if result.IsSuccess && result.Value.SuccessCount > 0 {
        return result
    }
...
func TestTrendAnalyzer_FullPipeline(t *testing.T) {
    ...
    // Run analysis
    report := analyzer.Analyze(context.Background(), AnalyzeOptions{
        Type: TrendTypeLanguage,
    })
    
    require.True(t, report.IsSuccess)
    assert.Len(t, report.Value.TopItems, 10)
    assert.True(t, report.Value.TopItems[0].CompositeScore > report.Value.TopItems[1].CompositeScore)
}
```
...
- [Seedable Configuration Pattern](../../21-app/spec-management-software/04-coding-guidelines/05-seedable-config-pattern.md)
- [Trend Analysis Engine](./19-trend-analysis-engine.md)
- [Authority & Credibility Scoring](./18-authority-credibility-scoring.md)
- [Settings UI](../02-frontend/01-settings-ui-page.md)
