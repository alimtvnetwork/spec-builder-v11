# ⚠️ THIS FILE HAS BEEN SPLIT INTO A SUBFOLDER

> **Moved to:** [01-copy-formats/readme.md](./01-copy-formats/readme.md)
> **Date:** 2026-03-31
> **Reason:** 871-line file split into 10 focused files for easier AI consumption.

---

## File Index → [01-copy-formats/](./01-copy-formats/)

| File | Content |
|------|---------|
| [readme.md](./01-copy-formats/readme.md) | Index, format overview, copy/download menu patterns |
| [01-compact-report.md](./01-copy-formats/02-compact-report.md) | ⭐ DEFAULT — Compact Report (instant, includes delegated server info) |
| [02-full-report.md](./01-copy-formats/03-full-report.md) | Full Report — all sections, verbose |
| [03-full-report-with-backend-logs.md](./01-copy-formats/04-full-report-with-backend-logs.md) | Full Report + error.log.txt appended |
| [04-error-log-txt.md](./01-copy-formats/05-error-log-txt.md) | Raw error.log.txt format |
| [05-full-log-txt.md](./01-copy-formats/06-full-log-txt.md) | Raw log.txt format |
| [06-error-log-with-delegated-info.md](./01-copy-formats/07-error-log-with-delegated-info.md) | error.log.txt + Delegated Server Info |
| [07-envelope-error-response.md](./01-copy-formats/08-envelope-error-response.md) | Envelope Error JSON structure |
| [08-session-diagnostics.md](./01-copy-formats/09-session-diagnostics.md) | Session Diagnostics JSON |
| [09-generator-code-reference.md](./01-copy-formats/10-generator-code-reference.md) | Source files, signatures, replication guide |

---

*This file is kept as a redirect. All content lives in the subfolder above.*
