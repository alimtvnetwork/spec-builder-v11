...
func parseInput(input string, target *PipelineInput) *appfault.AppError {
    // Try as file path first
    isExists := pathutil.Exists(input)
    if isExists {
        data, readErr := pathutil.ReadFile(input)
        if readErr != nil {
            return readErr
        }
        unmarshalErr := json.Unmarshal(data, target)
        if unmarshalErr != nil {
            return appfault.Wrap(unmarshalErr, appfault.ErrInputParse, "unmarshal pipeline input").
                WithPath(input)
        }
        return nil
    }
    ...
    // Try as JSON string
    unmarshalErr := json.Unmarshal([]byte(input), target)
    if unmarshalErr != nil {
        return appfault.Wrap(unmarshalErr, appfault.ErrInputParse, "unmarshal pipeline input from string")
    }
    return nil
}

func writeOutput(path string, result *model.ExecutionResult) *appfault.AppError {
    data, marshalErr := json.MarshalIndent(result, "", "  ")
    if marshalErr != nil {
        return appfault.Wrap(marshalErr, appfault.ErrOutputWrite, "marshal execution result")
    }
    writeErr := pathutil.WriteFile(path, data, 0644)
    if writeErr != nil {
        return writeErr
    }
    return nil
...
// MessageType uses messagetype.Variant from internal/enums/messagetype/
// See: 10-enum-architecture.md §17 for full definition
//
// Usage: messagetype.SessionConfigure, messagetype.PipelineExecute, etc.
// Methods: .String(), .IsClientMessage(), .IsServerMessage()
import "nexus-flow/internal/enums/messagetype"
...
// PipelineInput holds typed pipeline input data
type PipelineInput struct {
    Parameters map[string]string `json:",omitempty"`
    FilePaths  []string          `json:",omitempty"`
    RawJson    json.RawMessage   `json:",omitempty"`
}
...
type PipelineExecuteMessage struct {
    ...
    Input       PipelineInput    `json:",omitempty"`
...
// ProgressMetadata holds typed progress metadata
type ProgressMetadata struct {
    TokenCount   int    `json:",omitempty"`
    ChunkIndex   int    `json:",omitempty"`
...
type BlockProgressMessage struct {
    ...
    Metadata    ProgressMetadata `json:",omitempty"`
}

// BlockOutputData holds typed block output
...
// EscalationInput holds typed escalation response data
type EscalationInput struct {
    SelectedOption string `json:",omitempty"`
    TextResponse   string `json:",omitempty"`
...
type InputProvideMessage struct {
    ...
    Input       EscalationInput `json:",omitempty"`
}
...
// BlockType uses blocktype.Variant from internal/enums/blocktype/
// See: 10-enum-architecture.md §1 for full definition
//
// Usage: blocktype.Prompt, blocktype.Search, blocktype.CodeGen, etc.
// Methods: .String(), .SupportsStreaming()
import "nexus-flow/internal/enums/blocktype"
...
// BlockSettings holds typed block-specific settings
type BlockSettings struct {
    Model          string            `json:",omitempty"`
    PromptTemplate string            `json:",omitempty"`
    Temperature    float64           `json:",omitempty"`
    MaxTokens      int               `json:",omitempty"`
    Query          string            `json:",omitempty"`
    SearchType     string            `json:",omitempty"`
...
type SearchSettings struct {
    Query       string
    SearchType  string   // fts, vss, hybrid
    TopK        int
    MinScore    float64
...
func (b *SearchBlock) Validate() *appfault.AppError {
...
func (b *SearchBlock) Execute(context stdctx.Context, input BlockInput) appfault.Result[BlockOutput] {
    ...
    output := BlockOutputData{
        Results: mustMarshalJson(resp.Results),
    }
    ...
    if settings.IncludeRag {
        ...
        if err == nil {
            output.Response = ragResp.FormattedPrompt
        }
    }
...
func (b *CodeGenBlock) Validate() *appfault.AppError {
...
func (c *LoopController) executeForEach(
    ...
    collection, ok := getNestedCollection(input.Data, config.Collection)
    if !ok {
        return nil, errors.New(errors.CodeValidationError,
    ...
    for i, item := range collection {
        ...
        go func(idx int, itemData json.RawMessage) {
            defer wg.Done()
            ...
            iterInput := input
            iterInput.Data = BlockOutputData{
                Results: itemData,
            }
