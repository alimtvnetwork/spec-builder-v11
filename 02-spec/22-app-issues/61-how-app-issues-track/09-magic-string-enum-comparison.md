...
| File | Violation | Fix |
|------|-----------|-----|
| `02-spec/01-general-spec/01-foundation/01-coding-standards-foundation.md:133` | `hasMismatch($participant->status, 'active')` | `hasMismatch($participant->status, ParticipantStatus::Active->value)` |
| `02-spec/01-general-spec/02-systems/01-logging-system-systems.md:126` | `hasMismatch(getenv('APP_ENV'), 'development')` | `hasMismatch(getenv('APP_ENV'), AppEnvType::Development->value)` |
...
- [x] Issue documented in `02-spec/61-how-app-issues-track/09-magic-string-enum-comparison.md`
