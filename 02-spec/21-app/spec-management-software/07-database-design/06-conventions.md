...
| Unique Index | `Idx{Table}{Column}` | `IdxUserEmail` |
| Foreign Key Index | `Idx{Table}{FKColumn}` | `IdxProjectOwnerId` |
| Composite Index | `Idx{Table}{Col1}{Col2}` | `IdxFileProjectPath` |
| Functional Index | `Idx{Table}{Purpose}` | `IdxSessionActive` |
...
```go
// Single column index
Status string `gorm:"type:text;index:IdxInstructionStatus"`

// Composite index
ProjectId string `gorm:"type:text;index:IdxFileProjectPath"`
Path      string `gorm:"type:text;index:IdxFileProjectPath"`
```
...
```go
// Use typed struct for Updates — NEVER use map[string]interface{} or map[string]any
type ProjectUpdate struct {
    Name        string `gorm:"column:name"`
    Description string `gorm:"column:description"`
}

db.Model(&project).Updates(ProjectUpdate{
    Name:        newName,
...
> **Convention:** All JSON wire-format fields (API request/response payloads, serialized JSON columns) use **camelCase**.
> This applies to Go struct `json:"..."` tags, OpenAPI schemas, and any JSON stored in TEXT/JSONB columns.
> Database columns and GORM struct tags remain **PascalCase** per the global naming standard.
> See: `.ai-memory/memories/style/naming-convention.md` § Frontend Interoperability Exemption.
...
- [Relationships](./04-relationships.md) — FK constraints
