...
func SeedPromptPresets(db *gorm.DB, promptsDir string) error {
    ...
    for _, category := range categories {
        ...
        entries, err := pathutil.ReadDir(categoryPath)
        if err != nil {
        ...
        for _, entry := range entries {
            if stringutil.IsMissingSuffix(entry.Name(), ".md") {
                continue
            }
...
func seedPresetFromFile(db *gorm.DB, category, filePath string) error {
    ...
    if err != nil {
        return appfault.Wrap(
            ...
            ErrSeedPresetFailed,
            "seed preset",
        ).WithContext("name", frontmatter.Name)
    }
    ...
    for _, file := range files {
        ...
        if err != nil {
            return appfault.Wrap(
                ...
                ErrSeedFileFailed,
                "seed file",
            ).WithContext("path", file.RelativePath)
        }
    }
    ...
    for i, snapshot := range snapshots {
        ...
        if err != nil {
            return appfault.Wrap(
                ...
                "seed snapshot",
            ).WithContext("name", snapshot.Name)
        }
    }
...
func (f *FixtureFactory) CreateChatMessage(sessionId string, role string, content string) *ChatMessage {
    ...
    // Update session counts — typed struct per coding standard (no map[string]any)
    type ChatSessionCountsUpdate struct {
        MessageCount  gorm.Expr `gorm:"column:MessageCount"`
        TotalTokens   gorm.Expr `gorm:"column:TotalTokens"`
        LastMessageAt time.Time `gorm:"column:LastMessageAt"`
    }
    f.db.Model(&ChatSession{}).Where("Id = ?", sessionId).
        UpdateColumns(ChatSessionCountsUpdate{
...
- [Unified Schema](./03-unified-schema.md)
- [Database Design Overview](./00-overview.md)
- [Prompt Preset System](../05-features/06-ai-integration/02-presets-guidelines.md)
