...
func TestVoiceToSpecPipeline(t *testing.T) {
    ...
    t.Run("upload_and_transcribe", func(t *testing.T) {
        ...
        // Upload audio file
        audioData, err := pathutil.ReadFile(VoiceToSpecFixtures.AudioFile)
...
Implement user authentication with...`,
    ...
    t.Run("verify_artifacts_saved", func(t *testing.T) {
        ...
        for _, expectedPath := range VoiceToSpecFixtures.ExpectedArtifacts {
            ...
            assert.True(t, pathutil.Exists(fullPath), "Artifact not found: %s", expectedPath)
            
            // Verify content
            content, _ := pathutil.ReadFile(fullPath)
...
func TestIdeaPromotionFlow(t *testing.T) {
    ...
    t.Run("setup_project_with_specs", func(t *testing.T) {
        ...
        for _, specPath := range IdeaPromotionFixtures.RelatedSpecs {
            ...
            pathutil.EnsureDir(filepath.Dir(fullPath))
            pathutil.WriteFile(fullPath, []byte(sampleSpecContent(specPath)), 0644)
        }
    ...
    t.Run("create_idea", func(t *testing.T) {
        ...
        assert.True(t, pathutil.Exists(ideaPath))
    })
    ...
    t.Run("retrieve_related_context", func(t *testing.T) {
        ...
        // EXEMPTED: wiremock test framework — JSON stub requires map[string]interface{} (§7.2)
...
4. Add accessibility announcements`,
    ...
    t.Run("verify_instruction_file", func(t *testing.T) {
        ...
        content, err := pathutil.ReadFile(instructionPath)
        require.NoError(t, err)
...
func TestRAGRetrievalAccuracy(t *testing.T) {
    ...
    t.Run("index_documents", func(t *testing.T) {
        ...
        for _, doc := range RAGAccuracyFixtures.IndexedDocuments {
            ...
            pathutil.EnsureDir(filepath.Dir(fullPath))
            pathutil.WriteFile(fullPath, []byte(doc.Content), 0644)
        }
...
func TestConsistencyLoopTo99Percent(t *testing.T) {
    ...
    t.Run("setup_inconsistent_project", func(t *testing.T) {
        ...
        for path, content := range ConsistencyLoopFixtures.InitialSpecs {
            ...
            pathutil.EnsureDir(filepath.Dir(fullPath))
            pathutil.WriteFile(fullPath, []byte(content), 0644)
        }
    })
    ...
    t.Run("verify_fixes_applied", func(t *testing.T) {
        ...
        assert.True(t, pathutil.Exists(missingFilePath), "Missing file should have been created")
        
        // Check naming was fixed
        userModelPath := filepath.Join(env.WorkDir, "consistency-loop-test",
            "07-database-design/01-users.md")
        content, _ := pathutil.ReadFile(userModelPath)
...
func TestFileSyncConflictResolution(t *testing.T) {
    ...
    t.Run("external_file_modification", func(t *testing.T) {
        ...
        pathutil.WriteFile(filePath, []byte(newContent), 0644)
    ...
    t.Run("resolve_keep_filesystem", func(t *testing.T) {
        ...
        content, _ := pathutil.ReadFile(filePath)
    ...
    t.Run("create_db_priority_conflict", func(t *testing.T) {
        ...
        filePath := filepath.Join(env.WorkDir, "sync-conflict-test", "docs/readme.md")
        pathutil.WriteFile(filePath, []byte("# Filesystem Version\n\nVersion 4 - FS Edit"), 0644)
    })
    ...
    t.Run("resolve_with_merge", func(t *testing.T) {
        ...
        pathutil.WriteFile(filePath, []byte("# Header\n\n## Section A\nOriginal\n\n## Section B\nFS content"), 0644)
    ...
    t.Run("manual_conflict_resolution", func(t *testing.T) {
        ...
        filePath := filepath.Join(env.WorkDir, "sync-conflict-test", "docs/readme.md")
        fsContent, _ := pathutil.ReadFile(filePath)
    ...
    t.Run("detect_new_files", func(t *testing.T) {
        ...
        pathutil.WriteFile(newFilePath, []byte("# New File\n\nCreated externally"), 0644)
    ...
    t.Run("syncignore_patterns", func(t *testing.T) {
        ...
        pathutil.WriteFile(ignorePath, []byte("*.tmp\n.cache/\nnode_modules/"), 0644)
        
        // Create ignored files
        pathutil.WriteFile(filepath.Join(env.WorkDir, "sync-conflict-test", "test.tmp"), []byte("temp"), 0644)
        pathutil.EnsureDir(filepath.Join(env.WorkDir, "sync-conflict-test", ".cache"))
        pathutil.WriteFile(filepath.Join(env.WorkDir, "sync-conflict-test", ".cache/data"), []byte("cached"), 0644)
...
func computeFileHash(path string) string {
    content, _ := pathutil.ReadFile(path)
...
- [Unified Database Schema](../07-database-design/03-unified-schema.md)
- [Seed Data & Fixtures](../07-database-design/05-seed-data.md)
