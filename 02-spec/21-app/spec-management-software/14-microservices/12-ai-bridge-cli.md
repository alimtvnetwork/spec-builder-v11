...
- [Shared Pkg Modules](./08-shared-pkg-modules.md)
...
CREATE INDEX IdxAppName ON ApplicationIndex(Name);
CREATE INDEX IdxAppActive ON ApplicationIndex(IsActive);
```
...
CREATE INDEX IdxModelCategory ON ModelRegistry(Category);
CREATE INDEX IdxModelProvider ON ModelRegistry(ProviderId);
CREATE INDEX IdxModelDefault ON ModelRegistry(IsDefault);
```
...
CREATE INDEX IdxConvStatus ON Conversation(Status);
CREATE INDEX IdxConvUpdated ON Conversation(UpdatedAt DESC);
```
...
CREATE INDEX IdxMsgConv ON Message(ConversationId, SequenceNum);
CREATE INDEX IdxMsgRole ON Message(Role);
```
...
CREATE INDEX IdxMemorySource ON Memory(SourceType, SourceId);
CREATE INDEX IdxMemoryHash ON Memory(ContentHash);
...
CREATE INDEX IdxTaskType ON Task(Type);
CREATE INDEX IdxTaskConv ON Task(ConversationId);
```
...
type AppError struct {
    ...
    Constant   string
    Message    string
    Details    AppErrorDetails        `json:",omitempty"`
    Retryable  bool
    Cause      error                  `json:"-"` // EXEMPTED: AppError internal cause (I-2)
    Stack      []StackFrame           `json:",omitempty"`
    StackStr   []string               `json:",omitempty"` // Compact format
}

// AppErrorDetails holds typed error context data (no map[string]interface{})
type AppErrorDetails struct {
    Path         string   `json:",omitempty"`
    Pattern      string   `json:",omitempty"`
    AllowedRoots []string `json:",omitempty"`
...
func (e *AppError) WithDetails(details AppErrorDetails) *AppError {
    e.Details = details
    return e
}
...
func LogWithSource(context context.Context, level slog.Level, msg string, args ...any) {
    ...
    logger.Log(context, level, msg, args...)
}

// Convenience methods
func Info(context context.Context, msg string, args ...any) {
    LogWithSource(context, slog.LevelInfo, msg, args...)
}

func Error(context context.Context, msg string, err error, args ...any) {
    var appErr *AppError
    if errors.As(err, &appErr) {
        args = append(args, 
            LogKeyErrorCode, appErr.Code,
            LogKeyErrorConstant, appErr.Constant,
            LogKeyStackTrace, appErr.StackStr,
        )
    }
    args = append(args, "error", err)
    LogWithSource(context, slog.LevelError, msg, args...)
}
```
...
```json
{
    ...
    "line": 87,
    "ErrorCode": 6001,
    "ErrorConstant": "ERR_AI_PROVIDER_UNAVAILABLE",
    "Stack": [
...
func (m *HealthMonitor) StartMonitoring(context stdctx.Context) {
...
func (w *WindowsFirewall) AddException(context stdctx.Context, port int, name string) error {
    cmd := exec.CommandContext(context, "netsh", "advfirewall", "firewall", "add", "rule",
...
func (w *WindowsFirewall) RemoveException(context stdctx.Context, port int) error {
    // Find and remove rule by port
    cmd := exec.CommandContext(context, "netsh", "advfirewall", "firewall", "delete", "rule",
        fmt.Sprintf("localport=%d", port),
        "protocol=tcp",
    )
    return cmd.Run()
...
func (l *LinuxFirewall) AddException(context stdctx.Context, port int, name string) error {
    if l.useUFW {
        cmd := exec.CommandContext(context, "ufw", "allow", fmt.Sprintf("%d/tcp", port))
        return cmd.Run()
    }
    // iptables fallback
    cmd := exec.CommandContext(context, "iptables", "-A", "INPUT", "-p", "tcp",
