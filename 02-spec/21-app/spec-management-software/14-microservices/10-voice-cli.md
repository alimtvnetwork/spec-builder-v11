...
- [AI-Bridge Service](./04-ai-bridge.md) — LLM provider abstraction
...
CREATE INDEX IdxProjectExternal ON ProjectIndex(ExternalProjectId, ExternalSystem);
...
CREATE INDEX IdxConversationProject ON ConversationIndex(ProjectId);
CREATE INDEX IdxConversationStatus ON ConversationIndex(Status);
...
CREATE INDEX IdxSessionConversation ON ActiveSession(ConversationId);
...
CREATE INDEX IdxSessionConversation ON RecordingSession(ConversationId);
...
CREATE INDEX IdxSegmentSession ON TranscriptSegment(SessionId);
CREATE INDEX IdxSegmentTime ON TranscriptSegment(StartTime, EndTime);
CREATE INDEX IdxSegmentText ON TranscriptSegment(TextNormalized);
...
CREATE INDEX IdxWordSegment ON WordTimestamp(SegmentId);
...
CREATE INDEX IdxEntitySegment ON TranscriptEntity(SegmentId);
CREATE INDEX IdxEntityType ON TranscriptEntity(Type);
...
CREATE INDEX IdxCommandSegment ON VoiceCommand(SegmentId);
CREATE INDEX IdxCommandType ON VoiceCommand(CommandType);
CREATE INDEX IdxCommandStatus ON VoiceCommand(Status);
...
CREATE INDEX IdxEventSession ON AudioEvent(SessionId);
CREATE INDEX IdxEventType ON AudioEvent(Type);
...
type ProjectIndex struct {
    ...
    CreatedAt             time.Time
    UpdatedAt             time.Time
    LastAccessedAt        time.Time `gorm:"default:null"`
...
type ConversationIndex struct {
    ...
    CreatedAt      time.Time
    UpdatedAt      time.Time
    LastActivityAt time.Time `gorm:"default:null"`
...
type VoiceCommand struct {
    ...
    ExecutedAt      time.Time `gorm:"default:null"`
...
func (w *WhisperProvider) Transcribe(context stdctx.Context, audio io.Reader, opts TranscribeOptions) appfault.Result[TranscriptResult] {
    ...
    result, err := w.processor.Process(context, samples, whisper.ProcessOptions{
        ...
        for {
            ...
            case chunk, ok := <-audioStream:
                if !ok {
                    ...
                    if buffer.Len() > 0 {
                        w.processChunk(context, buffer.Flush(), opts, events, true)
                    }
                    return
                }
                ...
                if buffer.Duration() >= buffer.chunkDuration {
                    w.processChunk(context, buffer.Flush(), opts, events, false)
                }
            }
        }
    }()
...
func (o *OpenAIRealtimeProvider) connect(context stdctx.Context) error {
    o.mu.Lock()
    defer o.mu.Unlock()
...
func (o *OpenAIRealtimeProvider) StreamTranscribe(context stdctx.Context, audioStream <-chan []byte, opts TranscribeOptions) appfault.Result[<-chan TranscriptEvent] {
    ...
    go o.handleMessages(context, events)
    
    // Send audio chunks
    // AudioBufferAppendMessage is the typed WebSocket message for audio input.
    type AudioBufferAppendMessage struct {
        Type  string
        Audio string
    }

    // SessionUpdateMessage is the typed WebSocket message for session configuration.
    ...
    type SessionUpdateConfig struct {
        Modalities               []string
        InputAudioFormat         string                     `json:"input_audio_format"`         // External API: snake_case
        OutputAudioFormat        string                     `json:"output_audio_format"`        // External API: snake_case
        InputAudioTranscription  TranscriptionConfig        `json:"input_audio_transcription"`  // External API: snake_case
        TurnDetection            TurnDetectionConfig        `json:"turn_detection"`             // External API: snake_case
        Temperature              float64
    }
    ...
    type TurnDetectionConfig struct {
        Type              string
        Threshold         float64
        PrefixPaddingMs   int     `json:"prefix_padding_ms"`   // External API: snake_case
        SilenceDurationMs int     `json:"silence_duration_ms"` // External API: snake_case
    }
    ...
    config := SessionUpdateMessage{
        ...
        Session: SessionUpdateConfig{
            ...
            TurnDetection: TurnDetectionConfig{
                ...
                PrefixPaddingMs:   300,
                SilenceDurationMs: o.config.SilenceDuration,
            },
...
func (o *OpenAIRealtimeProvider) handleMessages(context stdctx.Context, events chan<- TranscriptEvent) {
    defer close(events)
    ...
    for {
        ...
        default:
            ...
            // --- Typed OpenAI Realtime Messages (no map[string]interface{}) ---
            
            // OpenAIRealtimeMessage is the envelope for all OpenAI Realtime API messages
            // EXEMPTED: External API (OpenAI Realtime) — snake_case/lowercase keys
            type OpenAIRealtimeMessage struct {
                Type       string               `json:"type"`
                Transcript string               `json:"transcript,omitempty"`
                Error      *OpenAIRealtimeError `json:"error,omitempty"`
            }
            
            // OpenAIRealtimeError is the typed error payload from OpenAI Realtime API
            // EXEMPTED: External API (OpenAI Realtime)
            type OpenAIRealtimeError struct {
                Type    string `json:"type"`
                Message string `json:"message"`
            }
            
            var msg OpenAIRealtimeMessage
...
func (e *ElevenLabsProvider) StreamTranscribe(context stdctx.Context, audioStream <-chan []byte, opts TranscribeOptions) appfault.Result[<-chan TranscriptEvent] {
    ...
    // --- Typed ElevenLabs WebSocket Messages (no map[string]interface{}) ---
    
    // ElevenLabsConfigMessage is the typed config message for ElevenLabs Scribe API
    // EXEMPTED: External API (ElevenLabs) — snake_case keys
    type ElevenLabsConfigMessage struct {
        Type           string `json:"type"`
    ...
    // ElevenLabsAudioMessage is the typed audio chunk message for ElevenLabs Scribe API
    // EXEMPTED: External API (ElevenLabs)
    type ElevenLabsAudioMessage struct {
        Type  string `json:"type"`
        Audio string `json:"audio"`
    }
    
    // Send config
    configMsg := ElevenLabsConfigMessage{
        Type:           "config",
        ModelId:        e.config.Model,
        CommitStrategy: e.config.CommitStrategy,
        SampleRate:     opts.SampleRate,
    }
    conn.WriteJson(configMsg)
    
    // Handle incoming messages
    go e.handleMessages(context, events)
    
    // Send audio
    go func() {
        for chunk := range audioStream {
            msg := ElevenLabsAudioMessage{
                Type:  "audio",
                Audio: base64.StdEncoding.EncodeToString(chunk),
            }
...
func (e *ElevenLabsProvider) handleMessages(context stdctx.Context, events chan<- TranscriptEvent) {
    defer close(events)
    ...
    for {
        ...
        default:
            ...
            // --- Typed ElevenLabs Response Messages (no map[string]interface{}) ---
            
            // ElevenLabsTranscriptMessage is the typed response from ElevenLabs Scribe API
            // EXEMPTED: External API (ElevenLabs)
            type ElevenLabsTranscriptMessage struct {
                Type  string               `json:"type"`
                Text  string               `json:"text"`
                Words []ElevenLabsWordTime  `json:"words,omitempty"`
            }
            
            // ElevenLabsWordTime is a word-level timestamp from ElevenLabs
            // EXEMPTED: External API (ElevenLabs)
            type ElevenLabsWordTime struct {
            ...
            var msg ElevenLabsTranscriptMessage
            if err := json.Unmarshal(message, &msg); err != nil {
                continue
            }
            ...
            case "committed_transcript_with_timestamps":
                words := e.parseElevenLabsWords(msg.Words)
                events <- TranscriptEvent{
                    Type: "final",
                    Segment: &TranscriptSegment{
                        Text:     msg.Text,
...
// CommandParameters holds typed parameters extracted from voice commands
...
func (p *CommandParser) Parse(context stdctx.Context, text string) appfault.Result[*ParsedCommand] {
    ...
    if p.useLlm {
        return p.parseWithLlm(context, text)
    }
...
func (p *CommandParser) buildCommand(cmdType CommandType, raw, normalized string, matches []string) appfault.Result[*ParsedCommand] {
    ...
    case CmdDeleteStage, CmdRenameStage:
        ...
        if len(matches) > 2 {
            cmd.Parameters.NewName = matches[2]
        }
...
If the command is unclear or not a valid command, return {"type": null, "confidence": 0}`, text)
    ...
    response, err := p.llmClient.Complete(context, prompt)
    if err != nil {
        return nil, err
    }
    
    // LLMCommandResult is the typed response from LLM command parsing
    // EXEMPTED: LLM output format — keys controlled by prompt
...
// NexusFlowCommandPayload is the typed request body for executing voice commands
type NexusFlowCommandPayload struct {
...
func (c *NexusFlowClient) ExecuteCommand(context stdctx.Context, flowId string, cmd *ParsedCommand) appfault.Result[*CommandResult] {
    ...
    resp, err := c.post(context, fmt.Sprintf("/flows/%s/commands", flowId), payload)
    if err != nil {
        return nil, err
    }
...
// NexusFlowTranscriptMessage is the typed WebSocket message for streaming transcripts
type NexusFlowTranscriptMessage struct {
    Type      string
    SegmentId string
    Text      string
    IsFinal   bool
...
func (c *NexusFlowClient) StreamTranscripts(context stdctx.Context, flowId string, transcripts <-chan *TranscriptSegment) error {
    wsUrl := strings.Replace(c.baseUrl, "http", "ws", 1) + fmt.Sprintf("/flows/%s/voice", flowId)
    
    conn, _, err := websocket.DefaultDialer.DialContext(context, wsUrl, nil)
    if err != nil {
        return err
    }
    defer conn.Close()
    ...
    for transcript := range transcripts {
        msg := NexusFlowTranscriptMessage{
            Type:      "transcript",
...
// VoiceLinkPayload is the typed request body for linking conversations
type VoiceLinkPayload struct {
    ConversationId string
    Source         string
}

func (c *SpecManagementClient) LinkConversation(context stdctx.Context, projectId, conversationId string) error {
    payload := VoiceLinkPayload{
        ConversationId: conversationId,
        Source:         "voice-cli",
    }
    
    _, err := c.post(context, fmt.Sprintf("/projects/%s/voice-links", projectId), payload)
    return err
}
...
func (c *SpecManagementClient) GetProjectContext(context stdctx.Context, projectId string) appfault.Result[*ProjectContext] {
    ...
    var projCtx ProjectContext
    if err := json.Unmarshal(resp, &projCtx); err != nil {
        return nil, err
    }
