...
CREATE INDEX IdxMigrationsVersion ON Migrations(Version);
```
...
CREATE INDEX IdxEmbeddingModelsProvider ON EmbeddingModels(ProviderId);
```
...
CREATE INDEX IdxProjectsDeleted ON Projects(DeletedAt) WHERE DeletedAt IS NOT NULL;
```
...
CREATE INDEX IdxProjectsPurge ON Projects(PurgeAt) WHERE PurgeAt IS NOT NULL;
```
...
CREATE INDEX IdxAuditTimestamp ON AuditLog(Timestamp DESC);
CREATE INDEX IdxAuditService ON AuditLog(ServiceName, Action);
CREATE INDEX IdxAuditEntity ON AuditLog(EntityType, EntityId);
CREATE INDEX IdxAuditUser ON AuditLog(UserId) WHERE UserId IS NOT NULL;
...
CREATE INDEX IdxServiceEventsTime ON ServiceEvents(Timestamp DESC);
CREATE INDEX IdxServiceEventsType ON ServiceEvents(ServiceName, EventType);
```
...
CREATE INDEX IdxProjectRegistryStatus ON ProjectRegistry(Status);
...
CREATE UNIQUE INDEX IdxModelRegistryUnique ON ModelRegistry(Provider, ModelName);
```
...
CREATE INDEX IdxFoldersParent ON Folders(ParentId);
...
CREATE INDEX IdxSpecsFolder ON Specs(FolderId);
CREATE INDEX IdxSpecsStatus ON Specs(Status);
CREATE INDEX IdxSpecsPath ON Specs(FilePath);
CREATE INDEX IdxSpecsDeleted ON Specs(DeletedAt) WHERE DeletedAt IS NOT NULL;
...
CREATE INDEX IdxSpecTagsTag ON SpecTags(TagId);
```
...
CREATE INDEX IdxCrossrefSource ON CrossReferences(SourceId);
CREATE INDEX IdxCrossrefTarget ON CrossReferences(TargetId);
CREATE INDEX IdxCrossrefValid ON CrossReferences(IsValid) WHERE IsValid = 0;
```
...
CREATE INDEX IdxCommitsTime ON Commits(Timestamp DESC);
CREATE INDEX IdxCommitsAuto ON Commits(IsAutoCommit);
...
CREATE INDEX IdxFileVersionsCommit ON FileVersions(CommitId);
CREATE INDEX IdxFileVersionsPath ON FileVersions(FilePath);
CREATE INDEX IdxFileVersionsHash ON FileVersions(ContentHash);
...
CREATE UNIQUE INDEX IdxDiffCacheHashes ON DiffCache(PreviousHash, CurrentHash);
CREATE INDEX IdxDiffCacheVersion ON DiffCache(FileVersionId);
...
CREATE INDEX IdxRollbackTime ON RollbackLog(RolledBackAt DESC);
```
...
CREATE INDEX IdxChunksHash ON Chunks(FileHash);
...
CREATE INDEX IdxEmbeddingsModel ON Embeddings(ModelId);
```
...
CREATE INDEX IdxSearchStatsTime ON SearchStats(CreatedAt DESC);
```
...
CREATE INDEX IdxMessagesTime ON Messages(CreatedAt);
...
CREATE INDEX IdxRagContextMessage ON RAGContext(MessageId);
CREATE INDEX IdxRagContextChunk ON RAGContext(ChunkId);
...
### Migration Execution
```go
func (r *Runner) Migrate(context stdctx.Context, db *sql.DB, dbType string) error {
    migrations, err := r.loadMigrations(dbType)
    if err != nil {
        return errors.Wrap(err, 9001, "failed to load migrations")
    }
    
    applied, err := r.getAppliedMigrations(context, db)
    if err != nil {
    ...
    for _, m := range migrations {
        ...
        start := time.Now()
        if err := r.executeMigration(context, db, m); err != nil {
            return errors.Wrap(err, 9003, "migration failed: "+m.Version)
        }
        
        duration := time.Since(start).Milliseconds()
        if err := r.recordMigration(context, db, m, duration); err != nil {
