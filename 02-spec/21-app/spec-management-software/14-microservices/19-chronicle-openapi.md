...
CREATE INDEX IdxCommitFilesCommit ON CommitFiles(CommitId);
CREATE INDEX IdxCommitFilesPath ON CommitFiles(Path);
CREATE INDEX IdxSnapshotsCommit ON Snapshots(CommitId);
CREATE INDEX IdxAuditTimestamp ON AuditLog(Timestamp DESC);
CREATE INDEX IdxAuditAction ON AuditLog(Action);
CREATE INDEX IdxAuditResource ON AuditLog(ResourceType, ResourceId);
CREATE INDEX IdxAuditActor ON AuditLog(Actor);
```
