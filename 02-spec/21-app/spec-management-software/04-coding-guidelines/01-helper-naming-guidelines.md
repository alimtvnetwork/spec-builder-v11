...
// EXEMPTED: generic helper boundary — .(type) switch on interface{} for multi-type dispatch (§7.2)
...
func (StringHelpers) IsAlphanumeric(s string) bool {
    for _, r := range s {
        if charutil.IsNonLetter(r) && charutil.IsNonDigit(r) {
            return false
        }
    }
    return true
...
func (FileHelpers) IsFileMissing(path string) bool {
    return pathutil.IsMissing(path)
}
...
func (FileHelpers) IsDirectoryEmpty(path string) bool {
    entries, err := pathutil.ReadDir(path)
    if err != nil {
        return true
    }
...
- [Implementation Guidelines](../08-roadmap-overview/02-implementation-order-guide.md)
- [Coding Standards](../../01-spec-authoring-guide/01-foundation/01-coding-standards-foundation.md)
