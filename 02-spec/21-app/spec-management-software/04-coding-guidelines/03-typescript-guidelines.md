...
**Cross-References:**
- [General Coding Standards](../../01-spec-authoring-guide/01-foundation/01-coding-standards-foundation.md)
...
### 0. Generics First (Highest Priority)

> 🔴 **CRITICAL:** Generics are the **#1 priority** for type safety. Always prefer generics over `any`, `unknown`, `Record<string, any>`, or untyped containers.

See [General Coding Standards §9](../../01-spec-authoring-guide/01-foundation/01-coding-standards-foundation.md) for the universal rule. TypeScript-specific patterns:
...
function handleEvent<T extends EventPayload>(event: TypedEvent<T>): void;
```

#### ❌ INCORRECT — Untyped Containers

```typescript
// NEVER use any/unknown/Record<string, any> for custom code
...
> ⚠️ **MANDATORY:** Never use `any` or `unknown` (except in type guards). Use generics instead.

Every type must be explicitly defined with proper interfaces, type aliases, or generic parameters.
