...
> **Cat 8 Triage Note (Wave 88):** This file contains ~8 `interface{}` usages in the settings
> service's generic Get/Update/Cache accessors. These are **EXEMPTED** as a settings framework
> boundary — the service stores JSON-decoded dynamic values internally and exposes typed accessors
> (GetString, GetFloat, GetInt, GetBool) for business logic consumption. The newer `SettingValue`
> union struct pattern (see `02-spec/26-brun-cli/01-backend/17-settings-service.md` and
> `02-spec/33-shared-cli-frontend/03-settings-service.md`) supersedes this approach for new implementations.
...
│  │                         PUBLIC INTERFACE                                │ │
│  │                                                                         │ │
│  │  Get(category, key) → any (EXEMPTED: settings framework boundary)       │ │
...
│  │  GetMap(category, key) → SettingMap                                    │ │
...
```go
// SettingMap is a typed alias for settings object values (replaces map[string]interface{})
type SettingMap map[string]SettingValue

// SettingValue is a discriminated union for setting values
type SettingValue struct {
    ...
    MapVal     SettingMap `json:"mapVal,omitempty"`
}
...
```go
// GetMap retrieves an object setting as SettingMap
//
// Type Handling:
// - JSON object → SettingMap (parsed into typed SettingValue fields)
// - Nested objects preserved as SettingValue.MapVal
// - Arrays preserved as SettingValue.SliceVal
//
// Errors:
// - ErrSettingNotFound: Key doesn't exist
...
// Get a typed float value directly
sourceAgreement, err := service.GetFloat(CategoryConfidenceMetrics, "weights.source_agreement")
if err != nil {
    log.Printf("Error: %v", err)
}
...
func NewTrendAnalyzer(settings SettingsService) *TrendAnalyzer {
    // Load typed configuration — no bare casts
    var weights TrendWeights
    if err := settings.GetTyped(CategoryTrendAnalysis, "composite_score_weights", &weights); err != nil {
        log.Printf("failed to load trend weights: %v", err)
    }
    
    var normalization NormalizationConfig
    if err := settings.GetTyped(CategoryTrendAnalysis, "signal_normalization", &normalization); err != nil {
        log.Printf("failed to load normalization: %v", err)
    }
...
func NewConfidenceAnalyzer(settings SettingsService) *ConfidenceAnalyzer {
    ...
    if err := settings.GetTyped(CategoryConfidenceMetrics, "weights", &weights); err != nil {
        log.Printf("failed to load confidence weights: %v", err)
    }
    
    var thresholds ConfidenceThresholds
    if err := settings.GetTyped(CategoryConfidenceMetrics, "thresholds", &thresholds); err != nil {
