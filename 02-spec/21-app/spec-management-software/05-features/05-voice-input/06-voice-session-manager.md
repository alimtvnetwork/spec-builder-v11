...
function useVoiceSessionManager(options: UseVoiceSessionManagerOptions): UseVoiceSessionManagerReturn {
  ...
  return {
    state,
    isRecording: state.phase === SessionPhase.Recording,
    isPaused: state.phase === SessionPhase.Paused,
    isProcessing: state.phase === SessionPhase.Processing,
    canRecord: state.permissions.microphone === 'granted' && state.connection.status === ConnectionStatus.Connected,
    startSession,
    ...
    playbackState: state.phase === SessionPhase.Reviewing ? playback.state : null,
  ...
  return (
    <SessionContext.Provider value={manager}>
      <div className={cn('voice-session-manager', modeStyles[mode], props.className)}>
        ...
        {manager.state.phase === SessionPhase.Idle && (
          <SessionStarter onStart={manager.startSession} />
        )}
        
        {(manager.state.phase === SessionPhase.Recording || manager.state.phase === SessionPhase.Paused) && (
          <RecordingInterface
        ...
        {manager.state.phase === SessionPhase.Processing && (
          <ProcessingIndicator 
            progress={/* transcription progress */}
          />
        )}
        
        {manager.state.phase === SessionPhase.Reviewing && (
          <ReviewInterface
...
- **Voice-CLI Service**: [/memories/architecture/microservices/voice-cli-service](../../../../.ai-memory/memories/features/ai-transcribe-backend.md)
- **Split Database System**: [/memories/architecture/split-database-system](../../../../.ai-memory/memories/architecture/01-split-db-architecture.md)
- **Resilient Execution**: [/memories/features/resilient-execution-system](../../../../.ai-memory/memories/features/resilient-execution-system.md)
