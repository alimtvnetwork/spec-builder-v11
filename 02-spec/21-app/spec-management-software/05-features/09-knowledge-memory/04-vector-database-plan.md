...
type InstructionSegment struct {
    ...
    Status             string    `gorm:"type:text;not null;default:'pending'"` // pending, executing, completed
    SummaryForNext     string    `gorm:"type:text"` // Compressed output for next segment
    ExecutedAt         time.Time `gorm:"type:text;default:null"`
...
type VectorIndexMetadata struct {
    ...
    IndexType      string    `gorm:"type:text;not null"` // "vss" | "hnsw" | "ivf"
