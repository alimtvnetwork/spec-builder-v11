...
type Artifact struct {
    ...
    FileId        string         `gorm:"type:text;not null;index:IdxArtifactFileId"`
    ArtifactType  string         `gorm:"type:text;not null;index:IdxArtifactType"` // "idea" | "instruction"
    Title         string         `gorm:"type:text;not null"`
    Summary       string         `gorm:"type:text"`
    Status        string         `gorm:"type:text;not null;index:IdxArtifactStatus"`
    RelativePath  string         `gorm:"type:text;not null;uniqueIndex:IdxArtifactPath"`
    ContentHash   string         `gorm:"type:text;not null"`
    IsPinned      bool           `gorm:"default:false;index:IdxArtifactPinned"`
    CreatedAt     time.Time      `gorm:"not null"`
    UpdatedAt     time.Time      `gorm:"not null;index:IdxArtifactUpdatedAt"`
    IndexedAt     time.Time      `gorm:"type:text;default:null"`
...
type Embedding struct {
    ...
    ModelId       string    `gorm:"type:text;not null;index:IdxEmbeddingModelId"`
    Dimensions    int       `gorm:"not null"`
    Vector        []byte    `gorm:"type:blob;not null"` // Little-endian float32 array
    CreatedAt     time.Time `gorm:"not null"`
...
type RetrievalSession struct {
    Id            string         `gorm:"type:text;primaryKey"`
    ProjectId     string         `gorm:"type:text;not null;index:IdxRetrievalSessionProjectId"`
    UserId        string         `gorm:"type:text;not null;index:IdxRetrievalSessionUserId"`
    ...
    RetrievedAt   time.Time      `gorm:"not null;index:IdxRetrievalSessionRetrievedAt"`
...
type RetrievalSessionChunk struct {
    SessionId     string    `gorm:"type:text;primaryKey"`
    ChunkId       string    `gorm:"type:text;primaryKey"`
    Score         float64   `gorm:"not null"`
    RankPosition  int       `gorm:"not null"`
    SourceType    string    `gorm:"type:text;not null"` // "semantic" | "keyword" | "topk"
      ...
      "semanticChunks": [
        {
          ...
          "score": 0.89,
          "sourcePath": "02-spec/backend/07-authentication.md",
