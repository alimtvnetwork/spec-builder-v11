...
type VectorSearchService interface {
    // Initialization
    Initialize(context stdctx.Context) error
    InitializeForProject(context stdctx.Context, projectId string) error
    ...
    // Health
    HealthCheck(context stdctx.Context) error
    IsVssAvailable() bool
}
...
func (v *VectorSearchServiceImpl) Initialize(context stdctx.Context) error {
    // 1. Load sqlite-vss extension
    if err := v.loadVssExtension(context); err != nil {
...
func (v *VectorSearchServiceImpl) loadVssExtension(context stdctx.Context) error {
    // Extension loading is platform-specific
    // The path should be configured in config table
    result := v.db.WithContext(context).Exec(`SELECT load_extension('vss0')`)
    return result.Error
}
...
func (v *VectorSearchServiceImpl) IndexEmbedding(context stdctx.Context, chunkId string, embedding []float32) error {
    if !v.vssLoaded {
...
func (v *VectorSearchServiceImpl) IndexBatch(context stdctx.Context, embeddings map[string][]float32) error {
    if !v.vssLoaded || len(embeddings) == 0 {
        return nil
    }
    
    return v.db.WithContext(context).Transaction(func(tx *gorm.DB) error {
...
func (v *VectorSearchServiceImpl) RemoveEmbedding(context stdctx.Context, chunkId string) error {
    if !v.vssLoaded {
        return nil
    }
    
    return v.db.WithContext(context).Exec(
        `DELETE FROM VssEmbedding WHERE chunk_id = ?`, chunkId,
    ).Error
}

// RemoveByArtifact removes all embeddings for an artifact
func (v *VectorSearchServiceImpl) RemoveByArtifact(context stdctx.Context, artifactId string) error {
    if !v.vssLoaded {
        return nil
    }
...
func (v *VectorSearchServiceImpl) SearchSemantic(
    ...
    err := v.db.WithContext(context).Raw(`
        SELECT 
            chunk_id,
            distance
        FROM VssEmbedding
...
func (v *VectorSearchServiceImpl) SearchHybrid(
    ...
    go func() {
        if v.vssLoaded && len(queryEmbedding) > 0 {
            scores, err := v.SearchSemantic(context, queryEmbedding, fetchLimit)
    ...
    go func() {
        if queryText != "" {
            scores, err := v.SearchKeyword(context, queryText, fetchLimit)
...
func (v *VectorSearchServiceImpl) SearchHybridWeighted(
    ...
    keywordScores, _ := v.SearchKeyword(context, queryText, fetchLimit)
...
func (v *VectorSearchServiceImpl) ReindexProject(context stdctx.Context, projectId string) error {
    ...
    for _, chunk := range chunks {
        ...
        err := v.db.WithContext(context).Table("Embedding").
        ...
        if err := v.IndexEmbedding(context, chunk.Id, embeddingVec); err != nil {
...
func (v *VectorSearchServiceImpl) GetIndexStats(context stdctx.Context, projectId string) appfault.Result[*VectorIndexStats] {
    ...
    if v.vssLoaded {
        var count int64
        if err := v.db.WithContext(context).Raw(`
    ...
    v.db.WithContext(context).Table("VectorIndexMetadata").
...
func (v *VectorSearchServiceImpl) HealthCheck(context stdctx.Context) error {
    // Test basic query
    if v.vssLoaded {
        return v.db.WithContext(context).Exec(`SELECT 1 FROM VssEmbedding LIMIT 1`).Error
    }
    return v.db.WithContext(context).Exec(`SELECT 1 FROM ChunkFts LIMIT 1`).Error
}
```
...
func (r *RAGService) RetrieveContext(
    ...
    if err != nil {
        ...
        return r.fallbackKeywordSearch(context, query, limit)
    }
    ...
    for _, score := range scores {
        var chunk Chunk
        if err := r.db.WithContext(context).
            Preload("Artifact").
