...
type ContextWindowConfig struct {
    // Model limits
    ModelContextSize int    // Total model context window (tokens)
    ModelName        string // Model identifier for lookup
    
    // Fixed reservations (guaranteed allocation)
    SystemPromptReserve  int // Layer 1: System prompt
    CriticalReserve      int // Layer 2: Critical context
    ResponseReserve      int // Layer 5: Response buffer
    SafetyMargin         int // Tokenization variance buffer
    
    // Dynamic allocation (flexible based on content)
    UserQueryMaxTokens     int // Layer 3: Max for user query
    InstructionMaxTokens   int // Layer 3: Max for instruction
    RetrievedContextTokens int // Layer 4: Retrieved chunks
    
    // Priority weights for overflow handling
    SemanticChunkWeight  float64 // Priority for semantic chunks
    KeywordChunkWeight   float64 // Priority for keyword chunks
    RecentArtifactWeight float64 // Priority for recent items
    PinnedArtifactWeight float64 // Priority for pinned items
    
    // Behavior flags
    AllowTruncation       bool   // Allow content truncation
    TruncationStrategy    string // "tail" | "head" | "middle"
    EnableOverflowWarning bool   // Log warnings on overflow
}
...
type ContextBlock struct {
    Layer       ContextLayer // Layer assignment
    Type        string       // "system" | "metadata" | "query" | "chunk" | etc
    Content     string
    TokenCount  int
    Priority    float64      // 0.0-1.0, higher = keep first
    Source      string       // Origin identifier
    CanTruncate bool         // Whether this block can be truncated
    MinTokens   int          // Minimum tokens if truncated
}
...
type AssembledContext struct {
    ...
    SourceChunks   []string // Chunk IDs included
}
```
...
type AssembleRequest struct {
    ...
    PinnedArtifacts []ContextBlock
    UserQuery       string
    Instruction     string
    RetrievedChunks []ContextBlock
    MemoryContext   string // Compressed previous turns
}
...
func (b *BudgetAllocator) CalculateBudget(
    ...
    // Count actual sizes
    systemTokens := b.tokenCounter.Count(systemPrompt).Value()
    criticalTokens := b.tokenCounter.Count(criticalContent).Value()
    queryTokens := b.tokenCounter.Count(userQuery).Value()
    instrTokens := b.tokenCounter.Count(instruction).Value()
...
type ContentSummarizer interface {
    Summarize(context stdctx.Context, blocks []ContextBlock, targetTokens int) ContextBlockSlice
}
```
...
|------|----------|-------------|
| 5060 | `ErrContextOverflow` | Context exceeds model limit |
| 5061 | `ErrBudgetExceeded` | Token budget allocation exceeded |
| 5062 | `ErrSystemPromptTooLarge` | System prompt exceeds reserve |
| 5063 | `ErrInvalidConfig` | Context window configuration invalid |
| 5064 | `ErrTokenCountFailed` | Token counting operation failed |
| 5065 | `ErrAssemblyFailed` | Context assembly failed |
| 5066 | `ErrTruncationFailed` | Content truncation failed |
...
| ID | Criterion | Priority | Validation Method |
|----|-----------|----------|-------------------|
| EH-001 | `ErrContextOverflow` (5060) for exceeded limit | Critical | Error code test |
| EH-002 | `ErrBudgetExceeded` (5061) for allocation exceeded | Critical | Error code test |
| EH-003 | `ErrSystemPromptTooLarge` (5062) for oversized system | High | Error code test |
| EH-004 | `ErrInvalidConfig` (5063) for bad configuration | High | Error code test |
