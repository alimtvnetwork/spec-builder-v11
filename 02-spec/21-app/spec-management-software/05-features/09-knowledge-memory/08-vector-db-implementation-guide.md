...
func (v *VectorSearchService) Initialize(context stdctx.Context) error {
    ...
    `
    return v.db.WithContext(context).Exec(createFts).Error
}

func (v *VectorSearchService) loadVssExtension(context stdctx.Context) error {
    return v.db.WithContext(context).Exec(`SELECT load_extension('vss0')`).Error
}

// IndexEmbedding adds a single embedding
func (v *VectorSearchService) IndexEmbedding(context stdctx.Context, chunkId string, embedding []float32) error {
    if !v.vssLoaded {
    ...
    // Upsert pattern
    v.db.WithContext(context).Exec(`DELETE FROM VssEmbedding WHERE chunk_id = ?`, chunkId)
...
func (v *VectorSearchService) SearchHybrid(
    ...
    semanticResults, _ := v.SearchSemantic(context, queryEmbedding, limit*2)

    // Get keyword results
    keywordResults, _ := v.SearchKeyword(context, queryText, limit*2)
...
type TokenCounter struct {
    ...
    // TypedCache replaces raw sync.Map — eliminates .(int) casts
...
func (t *TokenCounter) Count(text string) appfault.Result[int] {
    ...
    // Check cache — no cast needed, TypedCache[int] returns int directly
...
func (s *MemoryCompressionService) Compress(
    ...
    for attempt := 0; attempt <= s.config.MaxRetries; attempt++ {
        summary, err = s.aiService.Generate(context, prompt)
        if err == nil {
            break
        }
...
func (s *MemoryCompressionService) IncrementalMerge(
    ...
    return s.aiService.Generate(context, prompt)
}
...
func (p *RAGPipeline) ExecuteInstruction(
    ...
    assembled := assembledResult.Value()
    ...
    for _, idx := range order {
        ...
        output, err := p.aiService.Generate(context, execPrompt)
        if err != nil {
        ...
        compressed, err := p.memoryCompress.Compress(context, output, 300)
        if err == nil {
