...
func (c *SpecCopier) CopyToRepo(projectId, repoPath string) error {
    specDir := filepath.Join(repoPath, "02-spec")
    err := pathutil.EnsureDir(specDir)
    if err != nil {
        return err
    }
...
func (g *StructureGenerator) Generate(repoPath string, config StructureConfig) error {
    ...
    for _, dir := range dirs {
        err := pathutil.EnsureDir(filepath.Join(repoPath, dir))
        if err != nil {
            return err
        }
    }
...
|------|----------|-------------|
| 8700 | `ErrRepoCreateDirFailed` | Failed to create repository directory |
| 8701 | `ErrRepoTemplateFailed` | Failed to generate template file |
| 8702 | `ErrRepoInvalidStructure` | Invalid custom structure definition |
| 8703 | `ErrRepoCopySpecFailed` | Failed to copy specification files |
