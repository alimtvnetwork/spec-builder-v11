...
- [Project Editor UI](./20-project-editor-ui.md) - File tree integration
- [Long Chain Events](./29-long-chain-events.md) - Event streaming
...
│                                                                                  │
│       // Existing routes                                                        │
│       api.HandleFunc("/users", userHandler.List).Methods(httpmethod.Get.String())                │
│  -    api.HandleFunc("/users/{id}", userHandler.Get).Methods(httpmethod.Get.String())            │
│  +                                                                              │
│  +    // Authentication routes                                                  │
│  +    auth := api.PathPrefix("/auth").Subrouter()                              │
│  +    auth.HandleFunc("/login", authHandler.Login).Methods(httpmethod.Post.String())             │
│  +    auth.HandleFunc("/logout", authHandler.Logout).Methods(httpmethod.Post.String())           │
│  +    auth.HandleFunc("/refresh", authHandler.Refresh).Methods(httpmethod.Post.String())         │
│  +    auth.HandleFunc("/register", authHandler.Register).Methods(httpmethod.Post.String())       │
│  +                                                                              │
│  +    // Protected routes                                                       │
...
type FileChange struct {
    ...
    ChangeType  string    `gorm:"not null"` // create, modify, delete, rename, copy
    ...
    DiffPatch       *string `gorm:"type:text" json:",omitempty"`
    
    CreatedAt time.Time
    AppliedAt *time.Time `json:",omitempty"`
...
type FileChangeGroup struct {
    ...
    IsExpanded  bool         `gorm:"default:true"`
    AllApplied  bool         `gorm:"default:false"`
    HasErrors   bool         `gorm:"default:false"`
    
    CreatedAt   time.Time
    
    Changes []FileChange     `gorm:"foreignKey:MessageId;references:MessageId" json:"-"`
}
```
