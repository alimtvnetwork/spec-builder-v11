...
- [Implementation Guide](./19-implementation-guide.md)
- [Frontend Testing Strategy](../20-testing/01-test-strategy.md)
...
func TestGenerationPipeline_FullFlow(t *testing.T) {
    ...
    result, err := service.Generate(context.Background(), GenerationRequest{
        ...
        UserId:         user.Id,
        SpecReferences: []string{"02-spec/01-feature.md"},
    })
...
func TestGitIntegration_LocalOperations(t *testing.T) {
    ...
    pathutil.WriteFile(testFile, []byte("package main"), 0644)
...
func TestBuildVerification_WithMockBrun(t *testing.T) {
    ...
    mockBrun.On("Check", mock.Anything, CheckOptions{Language: "go"}).
        Return(`{"ExitCode": 1, "Errors": [{"File": "main.go", "Line": 10, "Message": "undefined: foo"}]}`, nil).
        Once()
    
    // After fix, check succeeds
    mockBrun.On("Check", mock.Anything, CheckOptions{Language: "go"}).
        Return(`{"ExitCode": 0, "Errors": []}`, nil).
        Once()
...
- [Architecture](./01-architecture.md)
- [Implementation Guide](./19-implementation-guide.md)
- [E2E Integration Scenarios](../20-testing/01-test-strategy.md)
