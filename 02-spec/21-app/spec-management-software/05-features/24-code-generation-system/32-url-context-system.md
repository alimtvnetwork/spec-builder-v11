...
func (s *URLContextService) runCrawl(
    ...
    cmd := exec.CommandContext(context, s.gsearchPath, args...)
...
func (s *URLContextService) SearchSite(
    ...
    if req.UseVectors {
        return s.vectorSearch(context, siteDb, req.Query, req.Limit)
    }
        ...
        <div className="max-h-24 overflow-y-auto text-xs space-y-1">
          {progress.recentActivity.slice(0, 5).map((activity, i) => (
            <div key={i} className="flex items-center gap-1">
              {activity.status === ActivityStatus.Success && <span className="text-green-500">✓</span>}
              {activity.status === ActivityStatus.Skipped && <span className="text-yellow-500">⊘</span>}
              {activity.status === ActivityStatus.Failed && <span className="text-red-500">✗</span>}
...
|------|------|-------------|
| 12850 | `ErrUrlContextDisabled` | URL context feature disabled |
| 12851 | `ErrCrawlAlreadyRunning` | Crawl already in progress for domain |
| 12852 | `ErrSiteNotCached` | Requested site not in cache |
| 12853 | `ErrSiteSearchFailed` | Site search query failed |
