...
|------|-------------|----------|
| `ErrGitCommitFailed` | Commit failed | Log and notify user |
| `ErrGitPushFailed` | Push failed | Retry with backoff |
| `ErrGitRepoNotFound` | Repository not found | Check spec path config |
| `ErrGitAuthFailed` | Auth failed | Check credentials |
| `ErrGitMergeConflict` | Merge conflict | Require manual resolution |
...
| PL-003 | `ErrGitMergeConflict` returned on merge conflict | Critical | Error code test |
...
| AU-003 | `ErrGitAuthFailed` returned on auth failure | Critical | Auth error test |
...
| ID | Criterion | Priority | Validation Method |
|----|-----------|----------|-------------------|
| EH-001 | `ErrGitCommitFailed` for commit failure | Critical | Error code test |
| EH-002 | `ErrGitPushFailed` for push failure | Critical | Error code test |
| EH-003 | `ErrGitRepoNotFound` for repository not found | Critical | Error code test |
| EH-004 | `ErrGitAuthFailed` for auth failure | Critical | Error code test |
| EH-005 | `ErrGitMergeConflict` for merge conflict | Critical | Error code test |
...
- [General Spec: Logging](../../../01-spec-authoring-guide/02-systems/01-logging-system-systems.md)
