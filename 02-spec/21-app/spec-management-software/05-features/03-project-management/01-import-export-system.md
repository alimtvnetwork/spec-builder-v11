...
```json
{
  ...
  "checksums": {
    "algorithm": "sha256",
    "files": {
      "02-spec/00-overview.md": "abc123...",
      "02-spec/01-backend/01-database.md": "def456..."
    }
  },
  "exportOptions": {
    "includeHistory": true,
...
type ExportOptions struct {
    ...
    SelectedFiles       []string `json:",omitempty"` // Empty = all
}
...
func (s *ExportService) ExportProject(
    ...
    if err != nil {
        pathutil.Remove(zipPath) // Cleanup on error
...
func DetectImportSource(filePath string) appfault.Result[ImportSourceType] {
    ...
    info, err := pathutil.Stat(filePath)
...
func (s *ImportService) PreviewImport(context stdctx.Context, sourcePath string) appfault.Result[*ImportPreview] {
    ...
    case ImportSourceZip:
        return s.previewZipImport(context, sourcePath)
    case ImportSourceMarkdown, ImportSourcePRD:
        return s.previewMarkdownImport(context, sourcePath, sourceType)
...
func (s *ImportService) ImportFromSource(
    ...
    switch sourceType {
    case ImportSourceZip:
        return s.importFromZip(context, userId, sourcePath, options)
    case ImportSourceMarkdown:
        return s.importFromMarkdown(context, userId, sourcePath, options, false)
    case ImportSourcePRD:
        return s.importFromMarkdown(context, userId, sourcePath, options, true)
    case ImportSourceFolder:
...
func (s *ImportService) importFromZip(
    ...
    for _, file := range reader.File {
        ...
        pathutil.EnsureDir(filepath.Dir(destPath))
        if err := pathutil.WriteFile(destPath, content, 0644); err != nil {
            result.Errors = append(result.Errors,
        ...
        relPath := file.Name
        s.fileService.CreateFileRecord(context, project.Id, relPath, content)
        result.FilesImported++
    }
    
    // 5. Generate or update metadata
    if projectJson != nil {
        s.applyProjectMetadata(context, &project, projectJson)
    } else if options.GenerateMetadata {
        s.generateProjectMetadata(context, &project, projectDir)
...
func (s *ImportService) importFromMarkdown(
    ...
    projectDir := s.pathManager.ToAbsolute(project.Id, "")
    specDir := filepath.Join(projectDir, "02-spec")
    pathutil.EnsureDir(specDir)
    result.FoldersCreated++
    ...
    if isPRD && options.ParsePRDSections {
        ...
        for i, section := range sections {
            ...
            if err := pathutil.WriteFile(filePath, []byte(section.Content), 0644); err != nil {
                result.Errors = append(result.Errors,
                    fmt.Sprintf("Failed to write %s: %v", fileName, err))
                continue
            }
            
            relPath := filepath.Join("02-spec", fileName)
            s.fileService.CreateFileRecord(context, project.Id, relPath, []byte(section.Content))
            result.FilesImported++
        }
    } else {
        // 3b. Import as single file (00-overview.md)
        overviewPath := filepath.Join(specDir, "00-overview.md")
        if err := pathutil.WriteFile(overviewPath, content, 0644); err != nil {
            return nil, NewError(ERR_FILE_WRITE, "Failed to write overview file")
        }
        
        s.fileService.CreateFileRecord(context, project.Id, "02-spec/00-overview.md", content)
...
func (s *ImportService) generateProjectMetadata(
    ...
    overviewPath := filepath.Join(projectDir, "02-spec", "00-overview.md")
    if content, err := pathutil.ReadFile(overviewPath); err == nil {
...
// Response
{
  ...
  "data": {
    ...
    "fileTree": [
      {"path": "02-spec/00-overview.md", "size": 2048, "isDir": false},
      {"path": "02-spec/01-backend/", "size": 0, "isDir": true}
    ]
  }
}
```
