...
func (tm *TrashManager) Restore(context stdctx.Context, entryId string) error {
    ...
    pathutil.Remove(entry.TrashPath + ".meta")
...
func (tm *TrashManager) EmptyTrash(context stdctx.Context, beforeDate *time.Time) apperror.Result[int] {
    ...
    for _, entry := range entries {
        if err := tm.PermanentDelete(context, entry.Id); err == nil {
            deletedCount++
        }
    }
...
```json
{
  ...
  "data": {
    "entries": [
      {
        "id": "abc123",
        "originalPath": "02-spec/01-feature/03-old-spec.md",
...
```json
{
  "success": true,
  "data": {
    "restoredPath": "02-spec/01-feature/03-old-spec.md",
...
func (fs *FileService) DeleteFile(context context.Context, fileId string, isPermanent bool) error {
    file, err := fs.GetFile(context, fileId)
    if err != nil {
        return err
    }
    ...
    if isPermanent {
        ...
        return fs.permanentDelete(context, file)
    }
    
    // Default: soft delete to trash
    _, err = fs.trashManager.SoftDelete(context, file.Path, ctxutil.GetUserId(context), "User deleted")
    return err
}
```
