...
|------|------|-------------|-------------------|
| `ErrWs4001` | ConnectionLost | WebSocket disconnected unexpectedly | Exponential backoff reconnect |
| `ErrWs4002` | ConnectionTimeout | Connection attempt timed out | Retry with increased timeout |
| `ErrWs4003` | ConnectionRefused | Server refused connection | Check auth, retry later |
| `ErrWs4004` | HandshakeFailed | WebSocket handshake failed | Verify protocol, retry |
| `ErrWs4005` | HeartbeatTimeout | No pong received | Force reconnect |
| `ErrWs4006` | InvalidMessage | Malformed message received | Log, continue |

### SSE Errors

| Code | Name | Description | Recovery Strategy |
|------|------|-------------|-------------------|
| `ErrSse4101` | StreamClosed | EventSource closed | Auto-reconnect |
| `ErrSse4102` | StreamError | Stream error event | Reconnect with last event ID |
| `ErrSse4103` | ParseError | Invalid event data | Log, skip event |

### State Errors

| Code | Name | Description | Recovery Strategy |
|------|------|-------------|-------------------|
| `ErrState4201` | SyncConflict | State conflict detected | Request server resolution |
| `ErrState4202` | StaleState | Local state outdated | Full sync |
| `ErrState4203` | InvalidState | State validation failed | Reset to last valid |
