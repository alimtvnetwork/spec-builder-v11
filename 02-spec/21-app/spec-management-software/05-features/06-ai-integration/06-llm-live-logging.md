...
// LogEntryDetails holds structured detail data for log entries
...
type LLMLogEntry struct {
    ...
    Level       LLMLogLevel
    Source      LLMLogSource
    ModelId     *string          `json:",omitempty"`      // Model being used
    ModelName   *string          `json:",omitempty"`    // Human-readable model name
    SlotIndex   *int             `json:",omitempty"`    // Which slot (port)
    RequestId   *string          `json:",omitempty"`    // Correlation ID
    Message     string
    Details     *LogEntryDetails `json:",omitempty"`    // Structured data
    
    // Shell command specific
    Command     *string          `json:",omitempty"`      // Shell command executed
    ExitCode    *int             `json:",omitempty"`     // Exit code if completed
    DurationMs  *int64           `json:",omitempty"`   // Execution time
    
    // Error specific
    ErrorCode   *string          `json:",omitempty"`    // Error classification
    StackTrace  *string          `json:",omitempty"`   // Full stack trace
    FilePath    *string          `json:",omitempty"`     // Source file
    LineNumber  *int             `json:",omitempty"`   // Source line
}
```
...
func (m *LogStreamManager) LogShellCommand(
    ...
    return func(exitCode int, err error) {
        ...
        if err != nil {
            errStr := err.Error()
            entry.Details = &LogEntryDetails{Error: &errStr}
        }
...
func (m *LogStreamManager) CaptureProcessOutput(
    ...
    // Stream stdout
    go m.streamPipe(context, stdout, LLMLogLevelStdout, modelId, modelName, slotIndex, requestId)
    
    // Stream stderr
    go m.streamPipe(context, stderr, LLMLogLevelStderr, modelId, modelName, slotIndex, requestId)
...
func (m *LogStreamManager) streamPipe(
    ...
    if err := scanner.Err(); err != nil {
        m.Log(LLMLogEntry{
            ...
            Details:   &LogEntryDetails{Error: strPtr(err.Error())},
        })
    }
}
...
type LogFilter struct {
    Levels    []LLMLogLevel  `json:",omitempty"`
    Sources   []LLMLogSource `json:",omitempty"`
    ModelId   *string        `json:",omitempty"`
    SlotIndex *int           `json:",omitempty"`
...
func (m *LogStreamManager) matchesFilter(entry LLMLogEntry, filter *LogFilter) bool {
    ...
    if filter.Search != nil && *filter.Search != "" {
        ...
        if stringutil.IsMissingSubstring(strings.ToLower(entry.Message), searchLower) {
            if entry.Command == nil || stringutil.IsMissingSubstring(strings.ToLower(*entry.Command), searchLower) {
                return false
            }
        }
    }
...
CREATE INDEX IdxLlmlogTimestamp ON LLMLog(Timestamp DESC);
CREATE INDEX IdxLlmlogLevel ON LLMLog(Level);
CREATE INDEX IdxLlmlogSource ON LLMLog(Source);
CREATE INDEX IdxLlmlogModel ON LLMLog(ModelId);
CREATE INDEX IdxLlmlogRequest ON LLMLog(RequestId);
...
func (sm *SlotManager) startModel(context stdctx.Context, slot *ModelSlot, model *ModelInfo) error {
    config, _ := sm.configService.GetLLaMAConfig(context)
    
    cmd := exec.CommandContext(context,
        config.ServerPath,
    ...
    done := sm.logManager.LogShellCommand(context, commandStr, &model.Id, &slot.SlotIndex)
    ...
    if err := sm.waitForReady(context, slot.Port); err != nil {
        return err
    }
...
All configuration keys use dot.notation format (see [Seeding Configuration](../../../06-seedable-config-architecture/00-overview.md)):
...
type LLMLogEntry struct {
    ...
    // Multi-server fields
    ServerId   *string `json:",omitempty"` // Server identifier from config
    ServerType *string `json:",omitempty"` // "ollama", "llama", "llama-swap"
    ServerPort *int    `json:",omitempty"` // Port the server is running on
}
```
...
type LogFilter struct {
    ...
    ServerId   *string   `json:",omitempty"`
    ServerType *string   `json:",omitempty"` // "ollama", "llama", "llama-swap"
    ServerIds  []string  `json:",omitempty"`  // Multiple servers
}
...
type LogStatsMessage struct {
    Type       string              // "llm:log_stats"
    ByServer   map[string]LogStats
    TotalLogs  int
    ErrorCount int
}
```
...
6. **Error Management** (`../../../01-spec-authoring-guide/02-systems/01-logging-system-systems.md`): Follows dual-file strategy
...
| ID | Criterion | Priority | Validation Method |
|----|-----------|----------|-------------------|
| EC-001 | Connection refused → `ErrLlmConnection` | Critical | Error mapping test |
| EC-002 | Out of memory → `ErrLlmOom` | Critical | Error mapping test |
| EC-003 | Context canceled → `ErrLlmCanceled` | Critical | Error mapping test |
| EC-004 | Timeout → `ErrLlmTimeout` | Critical | Error mapping test |
| EC-005 | Model not found → `ErrLlmModelNotFound` | Critical | Error mapping test |
| EC-006 | Unknown errors → `ErrLlmUnknown` | High | Error mapping test |
