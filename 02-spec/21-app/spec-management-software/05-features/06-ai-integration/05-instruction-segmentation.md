...
type ParsedSection struct {
    ...
    EndLine     int
    TokenCount  int
    Pattern     string          // Pattern that detected this section
    Subsections []ParsedSection `json:",omitempty"`
    Keywords    []string        // Extracted keywords for dependency detection
}
...
type SegmentationConfig struct {
    MaxTokensPerSegment   int   // Default: 4000
    MinTokensPerSegment   int   // Default: 500
    OverlapTokens         int   // Context overlap between segments
    MergeSmallSections    bool  // Combine tiny sections
    PreserveCodeBlocks    bool  // Don't split code blocks
    ExtractKeywords       bool  // Extract keywords for dependencies
}
...
type ExecutionConfig struct {
    ...
    SummarizeAfterComplete  bool
    ContinueOnFailure       bool
    ParallelIndependent     bool
    MaxParallelSegments     int
    TimeoutPerSegment       time.Duration
}
...
func (e *SegmentExecutionEngine) Execute(context stdctx.Context, plan *ExecutionPlan) *appfault.AppError {
    ...
    for _, segmentIndex := range plan.ExecutionOrder {
        ...
        // Check dependencies
        if depErr := e.checkDependencies(context, segment, plan); depErr != nil {
            if e.config.ContinueOnFailure {
                segment.Status = string(SegmentStatusBlocked)
                segment.LastError = depErr.Error()
                continue
            }
        ...
        // Execute segment
        if execErr := e.executeSegment(context, segment, plan); execErr != nil {
            if e.config.ContinueOnFailure {
                segment.Status = string(SegmentStatusFailed)
                segment.LastError = execErr.Error()
        ...
        if e.config.SummarizeAfterComplete && segmentIndex < len(plan.Segments)-1 {
            if sumErr := e.summarizeSegment(context, segment); sumErr != nil {
                // Non-fatal, log and continue
                segment.LastError = "summarization failed: " + sumErr.Error()
            }
        }
...
func (e *SegmentExecutionEngine) executeSegment(
    ...
    for attempt := 1; attempt <= segment.MaxAttempts; attempt++ {
        ...
        assembled, err := e.contextManager.Assemble(context, contextReq)
        if err != nil {
        ...
        execCtx, cancel := stdctx.WithTimeout(context, e.config.TimeoutPerSegment)
        defer cancel()
...
func (e *SegmentExecutionEngine) summarizeSegment(
    ...
    summary, err := e.memoryService.Compress(context, segment.Output, 500) // 500 token target
    if err != nil {
        return err
    }
...
// --- Typed Update Struct for GORM (no map[string]interface{}) ---

// SegmentStatusUpdate is the typed struct for segment status GORM updates
type SegmentStatusUpdate struct {
    Status         string     `gorm:"column:Status"`
    SummaryForNext *string    `gorm:"column:SummaryForNext"`
...
func (e *SegmentExecutionEngine) updateSegmentStatus(context stdctx.Context, segment *ExecutionSegment) {
    e.db.WithContext(context).Model(&segment.InstructionSegment).Updates(SegmentStatusUpdate{
...
func (s *InstructionSegmentationServiceImpl) Segment(
    ...
    return s.engine.CreateExecutionPlan(context, instructionId, sections, graphResult.Value())
}
...
|------|----------|-------------|
| 5070 | `ErrSegmentationParseFailed` | Failed to parse instruction into sections |
| 5071 | `ErrSegmentationNoSections` | No logical sections detected |
| 5072 | `ErrDependencyCycle` | Circular dependency detected |
| 5073 | `ErrDependencyResolutionFailed` | Could not resolve dependencies |
| 5074 | `ErrSegmentExecutionFailed` | Segment execution failed |
| 5075 | `ErrSegmentBlocked` | Segment blocked by failed dependency |
| 5076 | `ErrSegmentTimeout` | Segment execution timed out |
| 5077 | `ErrPlanNotFound` | Execution plan not found |
...
| DR-007 | `ErrDependencyCycle` (5072) for unbreakable cycles | High | Error code test |
...
| ID | Criterion | Priority | Validation Method |
|----|-----------|----------|-------------------|
| EH-001 | `ErrSegmentationParseFailed` (5070) for parse failure | Critical | Error code test |
| EH-002 | `ErrSegmentationNoSections` (5071) for empty parse | Critical | Error code test |
| EH-003 | `ErrSegmentExecutionFailed` (5074) for exec failure | Critical | Error code test |
| EH-004 | `ErrSegmentBlocked` (5075) for blocked segments | High | Error code test |
| EH-005 | `ErrSegmentTimeout` (5076) for timeout | High | Error code test |
