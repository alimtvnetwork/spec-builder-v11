...
CREATE INDEX IdxCheckpointTask ON ExecutionCheckpoint(TaskId);
CREATE INDEX IdxCheckpointExpires ON ExecutionCheckpoint(ExpiresAt);
...
CREATE INDEX IdxAttemptTask ON ExecutionAttempt(TaskId);
CREATE INDEX IdxAttemptStatus ON ExecutionAttempt(Status);
...
CREATE INDEX IdxVoteAttempt ON ConsensusVote(AttemptId);
...
CREATE INDEX IdxEscalationStatus ON EscalationRequest(Status);
CREATE INDEX IdxEscalationPriority ON EscalationRequest(Priority);
...
CREATE INDEX IdxTelemetryTask ON ExecutionTelemetry(TaskId);
```
...
type FailureAnalysis struct {
    ...
    SuggestedFix  CorrectionStrategy 
}
```
...
func (a *RootCauseAnalyzer) Analyze(context stdctx.Context, failure *TaskFailure) appfault.Result[FailureAnalysis] {
    ...
    result, err := a.thinkingModel.GenerateStructured(context, GenerateRequest{
...
// --- Typed Template Context Structs (no interface{} or map[string]any) ---

type RewritePromptContext struct {
...
func (r *PromptRewriter) Rewrite(context stdctx.Context, task *InstructionTask, guidance string) appfault.Result[CorrectedTask] {
    ...
    result, err := r.writingModel.Generate(context, GenerateRequest{
        UserPrompt:  prompt,
        Temperature: 0.3,
    })
...
type ConsensusConfig struct {
    Enabled              bool    
    MinModels            int     // Minimum models required (default: 2)
    RequiredAgreement    float64 // 0.0-1.0 (default: 0.66 = 2/3)
    VerifierRequired     bool    // Require thinking model verification
    ParallelExecution    bool    // Execute models in parallel
    TimeoutMs            int     // Per-model timeout
    ConflictResolution   string  // merge, vote, escalate
}
...
type ConsensusResult struct {
    ...
    Votes           []ConsensusVote   
    Conflicts       []ConflictDetail  `json:",omitempty"`
    Resolution      string            
}
...
func (e *ConsensusEngine) Execute(context stdctx.Context, task *InstructionTask) appfault.Result[ConsensusResult] {
    ...
    if config.ParallelExecution {
        outputs = e.executeParallel(context, task, selectedModels, config.TimeoutMs)
    } else {
        outputs = e.executeSequential(context, task, selectedModels, config.TimeoutMs)
    }
    ...
    votes := e.collectVotes(context, task, outputs, comparison)
    
    // 5. Determine consensus
    return e.resolveConsensus(context, outputs, votes, comparison, config)
}
...
type ComparisonResult struct {
    ...
    MajorityOutput    int              // Index of majority cluster
    HasConflicts      bool
    ConflictRegions   []ConflictRegion
}

type OutputCluster struct {
    Indices     []int    // Which outputs are in this cluster
    Similarity  float64  // Internal similarity
    Centroid    string   // Representative output
}

type ConflictRegion struct {
    Section     string   // Which part conflicts
...
func (e *ConsensusEngine) collectVotes(
    ...
    prompt := interpolate(verifierPrompt, VerifierPromptContext{
        TaskDescription: task.Description,
        Outputs:         outputs,
        Comparison:      comparison,
    })
    
    result, err := e.thinkingModel.GenerateStructured(context, GenerateRequest{
...
type StateSnapshot struct {
    ...
    DatabaseRows   map[string][]byte     `json:",omitempty"`
...
type FileState struct {
    Path         string
    ContentHash  string
    Content      string `json:",omitempty"`  // Full content for small files
...
func (m *CheckpointManager) CreateCheckpoint(context stdctx.Context, task *InstructionTask) appfault.Result[Checkpoint] {
    ...
    for _, path := range affectedFiles {
        ...
        if err != nil && pathutil.IsExistingPathError(err) {
            return nil, err
        }
    ...
    // 4. Store checkpoint
    if err := m.store.Save(context, checkpoint); err != nil {
        return nil, err
    }
...
func (m *CheckpointManager) Rollback(context stdctx.Context, checkpointId string) error {
    checkpoint, err := m.store.Get(context, checkpointId)
    if err != nil {
        return err
    }
    ...
    for path, state := range checkpoint.StateSnapshot.Files {
        if state.Content == "" && state.ContentRef != "" {
            ...
            state.Content, _ = m.store.GetContent(context, state.ContentRef)
        }
    ...
    m.restoreInstructionState(context, checkpoint.StateSnapshot.InstructionState)
...
type RetryStrategy struct {
    ...
    ContextLevel   string  // normal, expanded, full, maximum
...
type EscalationContext struct {
    ...
    Task            *InstructionTask
    FailureHistory  []FailureAnalysis
    ConsensusResult *ConsensusResult  `json:",omitempty"`
...
func (m *EscalationManager) Escalate(context stdctx.Context, trigger EscalationTrigger, escalationContext EscalationContext) appfault.Result[EscalationRequest] {
    ...
    if resolver, ok := m.autoResolvers[trigger]; ok {
        if resolution, err := resolver.TryResolve(context, escalationContext); err == nil && resolution != nil {
            return m.createAutoResolvedRequest(escalationContext, resolution)
        }
    }
    ...
    request := &EscalationRequest{
        Id:             uuid.New().String(),
        InstructionId:  escalationContext.Task.InstructionId,
        TaskId:         &escalationContext.Task.Id,
...
```go
func (m *EscalationManager) Resolve(context stdctx.Context, requestId string, resolution Resolution) error {
    request, err := m.queue.Get(context, requestId)
    if err != nil {
        return err
    }
    ...
    if err := m.queue.Update(context, request); err != nil {
        return err
    }
    
    // 2. Resume task execution with resolution
    return m.resumeExecution(context, request, resolution)
}

func (m *EscalationManager) resumeExecution(context stdctx.Context, request *EscalationRequest, resolution Resolution) error {
    var escalationContext EscalationContext
    json.Unmarshal([]byte(request.ContextJson), &escalationContext)
    
    // Find selected option
    var selectedOption *EscalationOption
    for _, opt := range escalationContext.Options {
...
func (c *TelemetryCollector) RecordExecution(context stdctx.Context, task *InstructionTask, result *ExecutionResult) {
...
func (e *TaskExecutor) executeTaskResilient(context stdctx.Context, task *InstructionTask) error {
    ...
    result, err := res.Execute(context, task, ResilientConfig{
    ...
    if err != nil {
        return e.handleFinalFailure(context, task, err)
    }
    
    // 4. Apply result
    return e.applyTaskResult(context, task, result)
}
```
