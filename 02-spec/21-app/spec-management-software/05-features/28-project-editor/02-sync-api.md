...
type SyncState struct {
    ...
    UserId      string    `gorm:"index:IdxUserKey,priority:1"`
    Key         string    `gorm:"index:IdxUserKey,priority:2"`
    Value       string
...
type SyncStateRequest struct {
    ...
    Value     string `validate:"required,max=1048576"` // 1MB max
...
type BatchSyncRequest struct {
    States []SyncStateRequest `validate:"required,max=50,dive"`
}

// BatchSyncResponse with results and conflicts
type BatchSyncResponse struct {
    Results  []SyncStateResponse
    Errors   []SyncError         `json:",omitempty"`
}
...
CREATE INDEX IdxSyncUserKey ON SyncStates(UserId, Key);
CREATE INDEX IdxSyncDevice ON SyncStates(DeviceId);
CREATE INDEX IdxSyncExpires ON SyncStates(ExpiresAt);
...
CREATE INDEX IdxDevicesUser ON SyncDevices(UserId);
```
