...
type SyncResult struct {
	Status          string
	ChangesPushed   int          `json:"changes_pushed"` // snake_case: API contract
	ChangesPulled   int          `json:"changes_pulled"`
	Conflicts       []ConflictInfo `json:",omitempty"`
...
func (s *SyncService) SyncShare(context stdctx.Context, shareId string, direction string) appfault.Result[SyncResult] {
	...
	case sourceChanged && targetChanged:
		...
		if resolved {
			...
			// Save conflict state
			s.saveConflictState(context, shareId, conflict)
		}
	}
	
	// Update sync state
	s.updateSyncState(context, &SyncState{
		ShareId:         shareId,
...
func (s *SyncService) pushToTarget(context stdctx.Context, share *MemoryShare, content string) error {
	return s.files.WriteContent(context, share.TargetProjectId, share.ResourcePath, content)
}

// pullFromTarget copies content from target to source
func (s *SyncService) pullFromTarget(context stdctx.Context, share *MemoryShare, content string) error {
	return s.files.WriteContent(context, share.SourceProjectId, share.ResourcePath, content)
}
...
func (s *SyncService) ResolveConflict(
	...
	share, _ := s.getShare(context, shareId)
	
	switch resolution {
	case "use_source":
		content, _, _ := s.getContentWithHash(context, share.SourceProjectId, share.ResourcePath)
		return s.pushToTarget(context, share, content)
		
	case "use_target":
		content, _, _ := s.getContentWithHash(context, share.TargetProjectId, share.ResourcePath)
		return s.pullFromTarget(context, share, content)
	...
	case "merge":
		...
		s.files.WriteContent(context, share.SourceProjectId, share.ResourcePath, *mergedContent)
		s.files.WriteContent(context, share.TargetProjectId, share.ResourcePath, *mergedContent)
		
	case "use_both":
		// Create a copy in target with conflict suffix
		conflictPath := s.generateConflictPath(share.ResourcePath)
		targetContent, _, _ := s.getContentWithHash(context, share.TargetProjectId, share.ResourcePath)
		s.files.WriteContent(context, share.TargetProjectId, conflictPath, targetContent)
		
		// Then sync source to original path
		sourceContent, _, _ := s.getContentWithHash(context, share.SourceProjectId, share.ResourcePath)
		return s.pushToTarget(context, share, sourceContent)
	}
...
func (w *SyncWorker) Start(context stdctx.Context) {
...
func (w *SyncWorker) runSyncCycle(context stdctx.Context) {
	// Get all shares with sync permission
	shares, err := w.getAutoSyncShares(context)
	if err != nil {
	...
	for _, share := range shares {
		...
		// Run sync
		result, err := w.syncService.SyncShare(context, share.Id, share.SyncDirection)
		if err != nil {
		...
		if result.Status == "conflict" {
			w.notifyConflict(context, share, result.Conflicts)
		}
	}
}
...
func (w *SyncWorker) notifyConflict(context stdctx.Context, share ShareWithSyncInfo, conflicts []ConflictInfo) {
...
func (w *FileWatcher) Start(context stdctx.Context) {
	go func() {
		for {
			select {
			case <-context.Done():
			...
			case event, ok := <-w.watcher.Events:
				...
				if event.Op&(fsnotify.Write|fsnotify.Create) != 0 {
					w.handleFileChange(context, event.Name)
				}
...
func (w *FileWatcher) handleFileChange(context stdctx.Context, path string) {
	...
	shares, err := w.getSharesForFile(context, projectId, path)
	if err != nil {
		return
	}
	...
	for _, share := range shares {
		if share.Permissions == "sync" {
			go w.syncService.SyncShare(context, share.Id, share.SyncDirection)
		}
	}
}
...
- [Sharing Architecture](./61-sharing-architecture.md)
