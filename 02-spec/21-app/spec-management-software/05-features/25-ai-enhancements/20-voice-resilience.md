...
|---|-----------|-------------|
| 01 | [Audio Capture](./21-audio-capture.md) | Browser MediaRecorder, WebM/Opus encoding, IndexedDB storage |
| 02 | [Transcription Service](./22-transcription-service.md) | Go backend with whisper.cpp integration |
| 03 | [Audio Sync](./23-audio-sync.md) | Resilient upload queue, chunked uploads, retry logic |
| 04 | [Voice UI Components](./24-voice-ui-components.md) | React components with waveform, transcription display |
...
func (s *Service) Transcribe(context stdctx.Context, audioData []byte, format string) appfault.Result[TranscriptionResult] {
	...
	// Run whisper.cpp
	cmd := exec.CommandContext(context, "./whisper",
		"--model", s.modelPath,
		"--language", "auto",
		"--output-json",
		"-f", "-", // Read from stdin
	...
	cmd.Stdin = bytes.NewReader(wavResult.Value())
...
func (s *Service) TranscribeStream(
	...
	for {
		...
		if buffer.Len() >= 16000*2*5 {
			result := s.Transcribe(context, buffer.Bytes(), "pcm")
			if result.HasError() {
				continue
			}
			
			for _, seg := range result.Value().Segments {
				onSegment(seg)
...
CREATE INDEX IdxAudioStatus ON audio_recordings(transcription_status);
CREATE INDEX IdxAudioProject ON audio_recordings(project_id);
```
