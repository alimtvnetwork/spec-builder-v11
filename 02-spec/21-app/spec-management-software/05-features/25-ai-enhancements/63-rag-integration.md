...
// EmbeddingApiRequest is the typed request for the OpenAI embeddings endpoint.
...
func (s *EmbeddingService) EmbedQuery(context stdctx.Context, query string) float64Slice {
...
func (s *EmbeddingService) storeChunks(context stdctx.Context, chunks []EmbeddedChunk) error {
...
func (s *SearchService) Search(context stdctx.Context, query string, opts SearchOptions) SearchResultSlice {
	...
	chunks, err := s.getCandidateChunks(context, opts)
	if err != nil {
		return nil, err
	}
...
func (s *SearchService) getCandidateChunks(context stdctx.Context, opts SearchOptions) EmbeddedChunkSlice {
	query := `
		...
		WHERE project_id = ?
	`
	args := []string{string(opts.ProjectId)}
...
func (w *IndexingWorker) Start(context stdctx.Context) {
	ticker := time.NewTicker(w.interval)
	defer ticker.Stop()
	
	// Initial indexing
	w.runIndexingCycle(context)
...
func (w *IndexingWorker) runIndexingCycle(context stdctx.Context) {
	// Get files that need indexing
	files, err := w.getFilesNeedingIndexing(context)
	if err != nil {
	...
	for _, file := range files {
		if err := w.indexFile(context, file); err != nil {
			fmt.Printf("Failed to index %s: %v\n", file.Path, err)
		}
	}
	
	// Also index shared memories
	w.indexSharedMemories(context)
}

func (w *IndexingWorker) indexFile(context stdctx.Context, file FileInfo) error {
	content, err := w.files.GetContent(context, file.ProjectId, file.Path)
	if err != nil {
		return err
	}
	...
	_, err = w.embedding.EmbedDocument(context, doc)
	if err != nil {
		return err
	}
	
	// Mark as indexed
	return w.markIndexed(context, file.ProjectId, file.Path, file.Hash)
}

func (w *IndexingWorker) indexSharedMemories(context stdctx.Context) {
	// Get all active shares with content that needs indexing
	shares, err := w.shares.GetActiveSharesNeedingIndexing(context)
	if err != nil {
		return
	}
	
	for _, share := range shares {
		content, err := w.shares.GetShareContent(context, share.ID, "system")
		if err != nil {
			continue
		}
		...
		w.embedding.EmbedDocument(context, doc)
	}
}
...
CREATE INDEX IdxChunksProject ON embedded_chunks(project_id);
CREATE INDEX IdxChunksSource ON embedded_chunks(source_id);
CREATE INDEX IdxChunksType ON embedded_chunks(source_type);
CREATE INDEX IdxChunksShare ON embedded_chunks(share_id);
