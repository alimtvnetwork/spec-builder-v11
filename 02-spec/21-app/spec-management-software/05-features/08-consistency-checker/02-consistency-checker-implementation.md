...
type ReportSummary struct {
	...
	CategoryCounts map[IssueCategory]int
}
...
type ConsistencyReport struct {
	...
	TriggeredBy string     // "manual", "auto", "commit-hook"
...
type ScanProgress struct {
	Status          ReportStatus
	CurrentFile     *string `json:",omitempty"`
	FilesScanned    int
	TotalFiles      int
	IssuesFound     int
...
// --- Typed Log Context Structs (no map[string]any) ---

type ConsistencyCheckLogContext struct {
	ProjectId string
	ReportId  string
}
...
type ConsistencyCheckCompletedContext struct {
	ReportId    string
	Score       float64
	Grade       string
	TotalIssues int
...
func (s *CheckerService) RunCheck(
	...
	utils.Info("Starting consistency check", ConsistencyCheckLogContext{
		ProjectId: projectId,
		ReportId:  reportId,
	})
	...
	case err := <-errCh:
		if err != nil {
			utils.Error("Validation error", ValidationErrorContext{Error: err.Error()})
	...
	utils.Info("Consistency check completed", ConsistencyCheckCompletedContext{
		ReportId:    reportId,
		Score:       report.Score,
		Grade:       report.Grade,
...
func (s *Scanner) ScanDirectory(
	...
	err := filepath.Walk(rootPath, func(path string, info os.FileInfo, err error) error {
		...
		if stringutil.IsMissingSuffix(strings.ToLower(path), ".md") {
			return nil
		}
...
func (v *LinkValidator) validateLink(
	...
	if pathutil.IsRelativePath(targetPath) {
...
func (v *NamingValidator) registerDefaultRules() {
	...
	v.rules = append(v.rules, models.NamingRule{
		...
		Applies: func(path string) bool {
			...
			return stringutil.IsMissingSubstring(filepath.Base(path), ".")
		},
	})
	...
	v.rules = append(v.rules, models.NamingRule{
		...
		Applies: func(path string) bool {
			return strings.HasSuffix(path, ".md") && 
			       stringutil.IsMissingSubstring(path, "README") &&
			       stringutil.IsMissingSubstring(path, "ideas/")
		},
	})
	...
	c.requiredPatterns = append(c.requiredPatterns, requiredPattern{
		...
		AppliesTo: func(path string) bool {
			return strings.HasSuffix(path, ".md") &&
			       stringutil.IsMissingSubstring(path, "README") &&
			       stringutil.IsMissingSubstring(path, "ideas/") &&
			       stringutil.IsMissingSubstring(path, "glossary")
		},
	})
...
func NewConsistencyRepo(db *gorm.DB) *ConsistencyRepo {
...
func (r *ConsistencyRepo) SaveReport(
	...
	err := r.db.WithContext(context).Create(report).Error
