...
type ChunkAnalysis struct {
    TotalChunks       int
    AverageChunkSize  int      // words
    OversizedChunks   []string // section IDs
    UndersizedChunks  []string // section IDs
    OrphanedHeadings  []string // H3/H4 without H2
    RecommendedSplits []string // where to add H2
}
```
...
**Indexes:**
- `IdxConsistencyReportProjectId` — on ProjectId
- `IdxConsistencyReportCreatedAt` — on CreatedAt (DESC)
...
CREATE INDEX IdxConsistencyReportProjectId ON ConsistencyReport(ProjectId);
CREATE INDEX IdxConsistencyReportCreatedAt ON ConsistencyReport(CreatedAt DESC);
```
...
CREATE INDEX IdxConsistencyFixReportId ON ConsistencyFix(ReportId);
CREATE INDEX IdxConsistencyFixStatus ON ConsistencyFix(Status);
```
...
|------|----------|-------------|
| 5080 | `ErrReportGenerationFailed` | Report generation error |
| 5081 | `ErrProjectNotFound` | Project ID doesn't exist |
| 5082 | `ErrInvalidReportType` | Unknown report type |
| 5083 | `ErrFixConflict` | File changed since fix generated |
| 5084 | `ErrFixAlreadyApplied` | Fix was already applied |
| 5085 | `ErrReportNotFound` | Report ID doesn't exist |
| 5086 | `ErrLowConfidenceFix` | Fix confidence below threshold |
...
type IterativeLoopConfig struct {
    TargetScore           int  // Default: 99
    MaxIterations         int  // Default: 10
    AutoApplyFixes        bool // Default: false
    MinImprovementPerIter int  // Default: 2
    StallThreshold        int  // Default: 3
    ReportEachIteration   bool // Default: true
}
...
type IterationResult struct {
    ...
    FixesApplied    int
    DurationMs      int
    Report          *ConsistencyReport `json:",omitempty"`
}
...
func (s *ConsistencyService) RunIterativeLoop(
    ...
    for i := 1; i <= config.MaxIterations; i++ {
        ...
        fixes, err := s.GenerateAutoFixes(context, report)
        if err != nil {
        ...
        if config.AutoApplyFixes && len(fixes) > 0 {
            appliedCount, err = s.ApplyHighConfidenceFixes(context, fixes)
            if err != nil {
        ...
        s.emitLoopProgress(context, loopId, iterResult)
    }
    ...
    // Persist loop result
    s.loopRepo.SaveLoopResult(context, result)
