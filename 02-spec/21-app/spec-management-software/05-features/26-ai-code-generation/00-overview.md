...
`code`, `features`, `generation`, `management`, `software`
...
|------|------|-------------|
| `ErrCodegenComplexity` | Complexity Analysis | Failed to determine task complexity |
| `ErrCodegenGenerate` | Code Generation | LLM failed to generate valid code |
| `ErrCodegenCompile` | Compilation | Generated code failed to compile |
| `ErrCodegenExecute` | Execution | Runtime error during execution |
| `ErrCodegenReuse` | Reuse Matching | Failed to find reusable code |
| `ErrCodegenApproval` | Approval | User rejected code execution |
| `ErrCodegenHistory` | History Logging | Failed to log operation |
