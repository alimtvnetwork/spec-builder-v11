...
// ValidatorArgs holds typed arguments for a validator
type ValidatorArgs struct {
    MaxLength    int      `json:",omitempty"`
    RequiredTags []string `json:",omitempty"`
    StrictMode   bool     `json:",omitempty"`
}

// ValidatorContext holds pipeline execution context
...
// ValidatorMetadata holds input metadata
...
// HtmlCheckDetails holds the typed details for HTML validation
type HtmlCheckDetails struct {
    HtmlLength int
    HasDoctype bool
}
...
type Result struct {
    ...
    Details  HtmlCheckDetails `json:",omitempty"`
}
...
func validate(input Input) Result {
    ...
    if stringutil.IsMissingSubstring(html, "<!DOCTYPE html>") {
        errors = append(errors, "Missing DOCTYPE declaration")
    }
    
    if stringutil.IsMissingSubstring(html, "<html") {
        errors = append(errors, "Missing <html> tag")
    }
    
    if strings.Contains(html, "<img") && stringutil.IsMissingSubstring(html, "alt=") {
...
func main() {
    ...
    // Read input file
    data, readErr := pathutil.ReadFile(os.Args[1])
    if readErr != nil {
