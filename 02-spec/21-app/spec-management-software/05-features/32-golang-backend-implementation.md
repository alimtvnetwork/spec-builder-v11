...
| 1.6 | Registry overlap validator | `02-spec/03-error-code-registry/08-overlap-validator.md` |
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| POST | `/api/v1/export` | Export project data |
...
**Rationale:** This backend is primarily a service orchestrator — it coordinates file I/O, database access, AI providers, and Git operations. Pure business logic (unit-testable) is a minority of the codebase. The majority of defects will occur at integration boundaries (DB queries, HTTP handlers, auth middleware, AI provider calls), making integration tests the highest-value investment. E2E tests cover only critical user-facing flows (auth → file CRUD → AI chain) to catch cross-cutting regressions without excessive maintenance cost.
...
| SM-RV | Registry overlap validator (`02-spec/03-error-code-registry/08-overlap-validator.md`) — error codes 2850-2859 |
...
- [ ] Registry overlap validator detects collisions and returns exit code 1; passes clean registries with exit code 0
