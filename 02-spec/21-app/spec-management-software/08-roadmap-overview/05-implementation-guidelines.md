...
- [ ] Create `internal/models/user.go`:
  ...
  type File struct {
      Id          string    `gorm:"primaryKey;type:text"`
      ProjectId   string    `gorm:"type:text;not null;index;uniqueIndex:IdxFileProjectPath,priority:1"`
      ParentId    *string   `gorm:"type:text;index"`
      Name        string    `gorm:"type:text;not null"`
      Path        string    `gorm:"type:text;not null;uniqueIndex:IdxFileProjectPath,priority:2"`
...
- [ ] Create `internal/models/project.go`:
  ...
  const (
      ProjectTypeSpec     ProjectType = "02-spec"
...
- [ ] Create `internal/config/config.go`:
  ...
  func Load() appfault.Result[Config] {
      ...
      if data, err := pathutil.ReadFile("internal/config/seed.json"); err == nil {
...
- [ ] Create `internal/utils/jwt.go`:
  ...
  type Claims struct {
      ...
      UserId    string    `json:"uid"`  // EXEMPTED: JWT standard short claim names
      TokenType TokenType `json:"typ"`  // EXEMPTED: JWT standard short claim names
  }
...
- [ ] Create `internal/repository/user_repo.go`:
  ...
  // UserLastLoginUpdate is a typed GORM update struct — never use map[string]any
...
- [Testing & Deployment](./07-testing-deployment.md)
