...
```go
// In pkg/database
// DatabaseErrorContext provides typed context for database errors.
type DatabaseErrorContext struct {
    Path string
}
...
if err := db.Ping(); err != nil {
    return errors.NewDatabase(
        ...
        DatabaseErrorContext{Path: db.path},
    ).WithCause(err)
}
```
...
```go
func (db *DB) QueryRow(context stdctx.Context, query string, args ...any) *Row {
