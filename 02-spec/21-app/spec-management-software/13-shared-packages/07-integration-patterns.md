...
func (s *SpecService) GetSpec(context stdctx.Context, id types.SpecId) appfault.Result[Spec] {
    ...
    if err != nil {
        ...
        s.logger.ErrorContext(context, "failed to get spec",
            logging.Err(err),
            "specId", id,
        )
        return nil, err
...
func HandleCreateSpec(w http.ResponseWriter, r *http.Request) error {
    ...
    // Context already has requestId from middleware
    ...
    logger.InfoContext(ctx, "spec created", "specId", spec.Id)
...
// HealthCheckResult is the typed health check response
...
type DatabaseHealthDetail struct {
    OpenConnections int
    InUse           int
    Idle            int
}

func (h *HealthChecker) Check(context stdctx.Context) HealthCheckResult {
    health := h.db.HealthCheck(context)
    
    return HealthCheckResult{
        Status:  health.Status,
        Latency: health.Latency.String(),
        Database: DatabaseHealthDetail{
            OpenConnections: health.OpenConns,
            InUse:           health.InUse,
            Idle:            health.Idle,
        },
    }
...
func TestSpecService_CreateSpec(t *testing.T) {
    ...
    repo := &MockSpecRepository{
        CreateFunc: func(context stdctx.Context, spec *Spec) error {
            return nil
        },
    }
