...
// SpecMetadata holds parsed frontmatter and document metadata
...
func (sp *SpecParser) Import(path string) appfault.Result[*ParsedSpec] {
    ...
    default:
        return nil, errors.New(10441, "unsupported spec format")
    }
    ...
    for _, file := range files {
        ...
        if err := sp.projectDb.Create(dbSpec).Error; err != nil {
            return nil, errors.Wrap(err, 10442, "spec storage failed")
        }
...
func (sp *SpecParser) extractAndReadZip(path string) SpecFileSlice {
    ...
    if err != nil {
        return nil, errors.Wrap(err, 10443, "temp directory creation failed")
    }
    defer pathutil.RemoveAll(tempDir)
    
    // Extract zip
    if err := extractZip(path, tempDir); err != nil {
        return nil, errors.Wrap(err, 10444, "zip extraction failed")
    }
...
func (sp *SpecParser) readFolder(path string) SpecFileSlice {
    ...
    if err != nil {
        return nil, errors.Wrap(err, 10445, "folder reading failed")
    }
