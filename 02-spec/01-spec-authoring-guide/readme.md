...
### How to Write an Overview (`readme.md`)

Every module's `readme.md` must follow this structure:
...
Every `readme.md` MUST include these three scores:
...
|----------|---------|----------|
| **Overview** | Module entry point with metadata and file index | `readme.md` |
...
│   ├── readme.md                   # Issue index with status/severity
│   ├── 01-score-rounding/
│   │   ├── readme.md
│   │   ├── 01-investigation.md
│   │   └── 02-resolution.md
...
> **Key insight:** App/WP projects split features into `01-backend.md`, `02-frontend.md`, `03-wp-admin.md` inside each feature folder. Issues follow the same `{NN}-{kebab-name}` convention with `readme.md` required for multi-file issues. See [07-app-project-template.md](./07-app-project-template.md) for the full template.
...
>
> 1. **Error Management is the highest priority** — error handling patterns from [03-error-manage/](../03-error-manage/readme.md) must be implemented from the very first line of code. Never defer error handling to "later."
> 2. **Boolean and if/else naming** — follow the strict boolean naming conventions (`is`/`has`/`should` prefixes, positive-only names, no negatives). Extract complex conditions into named variables. See [Coding Guidelines](../02-coding-guidelines/readme.md).
> 3. **Database conventions** — singular table names, PascalCase, `{TableName}Id` integer PKs, exact FK name matching. See [Database Conventions](../04-database-conventions/readme.md).
...
9. **Score the module** — set AI Confidence and Ambiguity percentages in `readme.md`
...
4. If any are missing, create them with at least `readme.md`
...
1. Every folder has `readme.md`
2. Top-level folders have `99-consistency-report.md`
3. Root `02-spec/` has `readme.md` and `99-consistency-report.md` (plus `folder-structure-root.md` as a redirect)
...
**Then** Every folder MUST contain a valid `readme.md`, follow kebab-case numeric prefixes, and resolve all internal links.
