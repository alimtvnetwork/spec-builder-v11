...
func (e *ExecutionEngine) executeWithPort(context context.Context, cmd *Command, requestedPort int) appfault.Result[ExecutionResult] {
    ...
    if e.config.Ports.Firewall.AutoEnable {
        firewallErr := e.portManager.firewallManager.EnablePort(
            port.AvailablePort, 
            e.config.Ports.Firewall.RuleName,
            "tcp",
        )
        if firewallErr != nil {
            e.logger.Warn("Failed to enable firewall port", "error", firewallErr)
        }
    }
    
    // Set PORT environment variable
    cmd.Env["PORT"] = strconv.Itoa(port.AvailablePort)
    
    // Execute command
    result := e.execute(context, cmd)
    if result.IsOk() {
        value := result.Value()
