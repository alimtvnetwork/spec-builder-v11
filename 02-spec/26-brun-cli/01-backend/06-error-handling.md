...
- [**Error Code Registry (Centralized)**](../../21-app/spec-management-software/06-error-management/01-error-code-registry.md) — Authoritative source for all 7xxx error codes
...
type BuildError struct {
    ...
    Message    string
    Severity   severity.Variant // Type-safe severity enum
...
func (p *StackTraceParser) ParseGoStack(stack string) []StackFrame {
    ...
    for i := 0; i < len(lines)-1 && len(frames) < p.maxDepth; i += 2 {
        ...
        if stringutil.IsMissingPrefix(locLine, "\t") && stringutil.IsMissingPrefix(locLine, "    ") {
            continue
        }
...
func (l *FileLogger) Initialize() *apperror.AppError {
    ...
    if mkErr := pathutil.MkdirAll(l.runDir, 0755); mkErr != nil {
        return mkErr
    }
    
    // Open log files
    stdoutResult := pathutil.CreateFile(filepath.Join(l.runDir, "log.txt"))
    if stdoutResult.HasError() {
        return stdoutResult.Error()
    }

    l.stdoutFile = stdoutResult.Value()
    
    stderrResult := pathutil.CreateFile(filepath.Join(l.runDir, "error.txt"))
    if stderrResult.HasError() {
        return stderrResult.Error()
    }

    l.stderrFile = stderrResult.Value()
    
    combinedResult := pathutil.CreateFile(filepath.Join(l.runDir, "combined.txt"))
    if combinedResult.HasError() {
        return combinedResult.Error()
    }

    l.combinedFile = combinedResult.Value()
...
> **📋 Canonical Reference:** The authoritative error code definitions are maintained in the [Central Error Code Registry](../../21-app/spec-management-software/06-error-management/01-error-code-registry.md). This section provides a local reference; for the latest codes and cross-domain consistency, always consult the central registry.
...
func NewConfigNotFoundError(path string) *BrunError {
    return &BrunError{
        ...
        Constant:  "ErrBrunConfigNotFound",
...
func NewPortUnavailableError(port int, fallbackTried []int) *BrunError {
    return &BrunError{
        ...
        Constant:  "ErrBrunPortUnavailable",
