...
| 15 | [Seedable Config](./01-backend/15-seedable-config.md) | JSON seeding and version control |
| 16 | [Split DB Architecture](./01-backend/16-split-db-architecture.md) | Database architecture |
| 17 | [Session Management](./01-backend/17-session-management.md) | Operation session logging |
| **Frontend Specs** | | |
| 05 | [WebSocket Reconnection](./02-frontend/05-websocket-reconnection-recovery.md) | Reconnect and state recovery |
...
| 26 | [UI Patterns](./02-frontend/26-ui-patterns.md) | Shared UI patterns |
| 27 | [Quick Publish](./02-frontend/27-quick-publish.md) | One-click deployment |
| 28 | [Remote Plugins](./02-frontend/28-remote-plugins.md) | Remote plugin management |
| **Shared** | | |
| 05 | [Endpoint Mapping](./05-endpoint-mapping.md) | Centralized endpoint mapping system |
...
All debugging and verification MUST follow the Error Resolution specification:

| Document | Purpose |
|----------|---------|
| [Error Resolution Overview](../../03-error-manage/01-error-resolution/00-overview.md) | Frontend-backend verification patterns |
| [Go Debugging Guide](../../03-error-manage/03-debugging-guides/02-debugging-go.md) | Initialization order, zerolog, response envelope |
| [TypeScript Debugging Guide](../../03-error-manage/03-debugging-guides/03-debugging-typescript.md) | React frontend debugging |

### Mandatory Verification

Before claiming any API endpoint works:
1. Verify backend response format matches `{success: true, data: {...}}`
2. Verify frontend detection uses HTTP status codes as primary indicator
3. Never assume—always read the actual handler code
