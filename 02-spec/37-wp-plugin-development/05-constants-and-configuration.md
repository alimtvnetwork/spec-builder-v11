...
enum DateFormatType: string {
    case Iso8601  = 'c';              // ISO 8601 full (2026-02-25T14:30:00+00:00)
    case DateTime = 'Y-m-d H:i:s';   // Database storage format
    case DateOnly = 'Y-m-d';         // Date without time
    case LogTimestamp = 'Y-m-d\TH:i:s'; // Log file timestamp
...
**Usage:**
```php
// ❌ WRONG — hardcoded format string
$timestamp = gmdate('c');
$createdAt = gmdate('Y-m-d H:i:s');

// ✅ CORRECT — enum-backed format
$timestamp = gmdate(DateFormatType::Iso8601->value);
$createdAt = gmdate(DateFormatType::DateTime->value);
```
...
Values used only within a single class are defined as `private const`. Date format strings are **not** allowed as private constants — use `DateFormatType` instead:
...
class LogWriter {
    ...
    // ❌ WRONG — hardcoded date format as class const
    // private const TIMESTAMP_FORMAT = 'Y-m-d\TH:i:s';

    // ✅ CORRECT — use DateFormatType::LogTimestamp->value
}
```
